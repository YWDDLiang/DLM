...
PLANNER_TEMPERATURES="${PLANNER_TEMPERATURES:-0.8 0.9}"
...
REFINE_MAX_PROPOSALS="${REFINE_MAX_PROPOSALS:-${GATE_SAMPLES}}"
...
SOURCE_CHECKPOINT="runs/${SOURCE_RUN_ID}/outputs/h1a2_llama_rich_sft/final"
mkdir -p "${OUT_DIR}" "${NOTES_DIR}" "${LOG_DIR}"

if [ ! -d "${SOURCE_CHECKPOINT}" ]; then
  echo "Missing source planner checkpoint: ${SOURCE_CHECKPOINT}" >&2
  exit 2
fi

BASE_MASTER_PORT="${MASTER_PORT:-$((27000 + (${SLURM_JOB_ID:-0} % 16000)))}"
PORT_OFFSET=0
next_port() {
...
trap 'status=$?; echo "${status}" > "${NOTES_DIR}/exit_status.txt"; date "+%F %T %Z" > "${NOTES_DIR}/end_time.txt"; exit "${status}"' EXIT
...
RUN_ID="${RUN_ID}" SOURCE_RUN_ID="${SOURCE_RUN_ID}" SOURCE_CHECKPOINT="${SOURCE_CHECKPOINT}" PLANNER_TEMPERATURES="${PLANNER_TEMPERATURES}" NOTES_DIR="${NOTES_DIR}" python - <<'PY'
import json
import os
...
payload = {
    ...
    "stage": "h1a2_extra_temperature_continuation",
    "source_run_id": os.environ["SOURCE_RUN_ID"],
    "source_checkpoint": os.environ["SOURCE_CHECKPOINT"],
    "planner_temperatures": os.environ["PLANNER_TEMPERATURES"].split(),
    "de_novo_constraints": {
        "temperature_sweep_is_method_level": True,
        "per_sample_retry_or_filter": False,
        "gold_plan_at_sampling": False,
    },
}
Path(os.environ["NOTES_DIR"], "h1a2_temperature_continuation_config.json").write_text(
...
for path in sorted(notes.glob("h1a2_temp*_planner256_gate.json")):
    ...
    items.append({
        "path": str(path),
        "stem": path.stem.replace("_planner256_gate", ""),
        "score": score,
        "passed_acceptable": bool(payload.get("passed_acceptable")),
        "acceptable_failures": payload.get("acceptable_failures") or [],
        "metrics": metrics,
    })
...
PY
  ...
  echo "H1-A2 extra-temperature planner did not pass acceptable gate; stopping before hybrid." | tee -a "${LOG_DIR}/h1a2_decision.log"
fi
...
lines = ["# H1-A2 Extra Temperature Continuation Report", "", f"- RUN_ID: {os.environ['RUN_ID']}", f"- best_label: {payload['best_label']}", ""]
lines += ["## Summary JSON", "", "```json", json.dumps(payload, indent=2, ensure_ascii=False, sort_keys=True), "```", ""]
(notes / "h1a2_temperature_continuation_report.md").write_text("\n".join(lines), encoding="utf-8")
PY
