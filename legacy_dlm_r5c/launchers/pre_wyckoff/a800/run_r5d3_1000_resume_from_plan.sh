...
SOURCE_PLAN_RUN_ID="${SOURCE_PLAN_RUN_ID:-20260530_024012-r5d3-1000-refine}"
SOURCE_PLAN_SAMPLE_DIR="${SOURCE_PLAN_SAMPLE_DIR:-runs/${SOURCE_PLAN_RUN_ID}/outputs/r5d3_plan_sample1000}"
...
if [ ! -f "${SOURCE_PLAN_SAMPLE_DIR}/raw_generations.jsonl" ]; then
  echo "Missing source plan sample: ${SOURCE_PLAN_SAMPLE_DIR}/raw_generations.jsonl" >&2
  exit 1
fi
cp -a "${SOURCE_PLAN_SAMPLE_DIR}/." "${PLAN_SAMPLE_DIR}/"
...
payload = {
    "run_id": "${RUN_ID}",
    "stage": "r5d3_resume_from_validated_plan_to_refined1000",
    "source_plan_run_id": "${SOURCE_PLAN_RUN_ID}",
    "source_plan_sample_dir": "${SOURCE_PLAN_SAMPLE_DIR}",
...
Path("${NOTES_DIR}/r5d3_1000_resume_config.json").write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
PY
...
lines = [
    "# R5-D3 Resume Plan-Conditioned Exact Body 1000 Refined Gate",
    "",
    f"- RUN_ID: ${RUN_ID}",
    f"- source plan RUN_ID: ${SOURCE_PLAN_RUN_ID}",
