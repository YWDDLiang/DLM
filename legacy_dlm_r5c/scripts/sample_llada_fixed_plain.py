#!/usr/bin/env python3
"""Sample fixed-schema plain-text MP-20 proposals with staged LLaDA generation."""
...
from crystal_dlm.transformers_compat import ensure_create_bidirectional_mask, ensure_llada2_rope_parameters
...
def merge_distributed_outputs(output_dir: Path, world_size: int) -> None:
    ...
    if merged_metrics["target_mode"]:
        ...
        merged_metrics["target_reached"] = merged_metrics["target_reached_ranks"] == world_size and merged_metrics["graph_success"] >= target
    else:
        ...
        json.dumps(
            {
                ...
                "message": str(exc),
                "raw_count_text": record.get("raw_count_text", ""),
                "count_text": record.get("count_text", ""),
                "raw_lattice_text": record.get("raw_lattice_text", ""),
                "lattice_text": record.get("lattice_text", ""),
                "raw_elements_text": record.get("raw_elements_text", ""),
                "elements_text": record.get("elements_text", ""),
...
def parse_count_batch(raw_texts: List[str], current_indices: List[int], metrics, failure_handle, raw_handle) -> tuple[List[Dict[str, Any]], List[str]]:
    ...
    for sample_idx, raw_text in zip(current_indices, raw_texts):
        ...
        record: Dict[str, Any] = {
            ...
            "generation_mode": "fixed_plain_staged",
            "raw_count_text": raw_text,
        }
        try:
            parsed = parse_count_text(raw_text)
            metrics["count_parse_success"] += 1
            record["count_text"] = parsed["count_text"]
            record["num_atoms"] = int(parsed["num_atoms"])
            records.append(record)
            prompts.append(lattice_prompt(parsed["count_text"]))
        except Exception as exc:
            add_failure(metrics, failure_handle, int(sample_idx), "count", exc, record)
            raw_handle.write(json.dumps(record, ensure_ascii=False) + "\n")
    return records, prompts


def parse_lattice_batch(raw_texts: List[str], records: List[Dict[str, Any]], metrics, failure_handle, raw_handle) -> tuple[List[Dict[str, Any]], List[str]]:
    out_records: List[Dict[str, Any]] = []
    prompts: List[str] = []
    for record, raw_text in zip(records, raw_texts):
        record["raw_lattice_text"] = raw_text
        try:
            parsed = parse_lattice_text(raw_text)
            metrics["lattice_parse_success"] += 1
            record["lattice_text"] = parsed["lattice_text"]
            out_records.append(record)
            prompts.append(elements_prompt(record["count_text"], parsed["lattice_text"]))
        except Exception as exc:
...
def parse_elements_batch(raw_texts: List[str], records: List[Dict[str, Any]], metrics, failure_handle, raw_handle) -> tuple[List[Dict[str, Any]], List[str]]:
    out_records: List[Dict[str, Any]] = []
    prompts: List[str] = []
    for record, raw_text in zip(records, raw_texts):
        record["raw_elements_text"] = raw_text
        try:
            parsed = parse_elements_text(raw_text, int(record["num_atoms"]))
            metrics["elements_parse_success"] += 1
            record["elements_text"] = parsed["elements_text"]
            record["composition"] = parsed["composition"]
            out_records.append(record)
            prompts.append(coords_prompt(record["count_text"], record["lattice_text"], parsed["elements_text"]))
        except Exception as exc:
            add_failure(metrics, failure_handle, int(record["sample_idx"]), "elements", exc, record)
            raw_handle.write(json.dumps(record, ensure_ascii=False) + "\n")
    return out_records, prompts
...
def finish_records(
    ...
    *,
    ...
    for record, raw_text in zip(records, raw_texts):
        ...
        try:
            coords = parse_coords_text(raw_text, int(record["num_atoms"]), deduplicate_pbc=True)
            metrics["coords_parse_success"] += 1
            record["coords_text"] = coords["coords_text"]
            arrays = assemble_plain_answer(
                record["count_text"],
                record["lattice_text"],
                record["elements_text"],
                coords["coords_text"],
            )
            ...
            record.update(
                {
                    ...
                    "composition": arrays.get("composition", record.get("composition")),
                }
            )
        except Exception as exc:
...
def main() -> None:
    ...
    parser.add_argument("--count-gen-length", type=int, default=8)
    parser.add_argument("--count-block-length", type=int, default=4)
    parser.add_argument("--lattice-gen-length", type=int, default=48)
    parser.add_argument("--lattice-block-length", type=int, default=8)
    parser.add_argument("--elements-tokens-per-site", type=int, default=6)
    parser.add_argument("--elements-extra-tokens", type=int, default=8)
    parser.add_argument("--elements-block-length", type=int, default=4)
    parser.add_argument("--coords-tokens-per-site", type=int, default=12)
    parser.add_argument("--coords-extra-tokens", type=int, default=8)
    parser.add_argument("--coords-block-length", type=int, default=8)
    parser.add_argument("--coords-max-gen-length", type=int, default=288)
    ...
    for name, length, block in (
        ("count", args.count_gen_length, args.count_block_length),
    ...
    run_config["representation"] = "fixed_plain"
    run_config["generation_mode"] = "fixed_plain_staged"
    ...
    with raw_path.open("w", encoding="utf-8") as raw_handle, failure_path.open("w", encoding="utf-8") as failure_handle:
        progress = tqdm(total=len(sample_indices), desc=f"fixed-plain sampling rank{rank}", disable=distributed and not is_main)
        for batch_start in range(0, len(sample_indices), args.batch_size):
            ...
            count_texts = generate_texts(
                model,
                tokenizer,
                [count_prompt()] * len(current_indices),
                gen_length=args.count_gen_length,
                block_length=args.count_block_length,
                temperature=args.temperature,
                cfg_scale=args.cfg_scale,
                remasking=args.remasking,
            )
            count_records, lattice_prompts = parse_count_batch(count_texts, current_indices, metrics, failure_handle, raw_handle)
            ...
            lattice_records, elements_prompts = parse_lattice_batch(lattice_texts, count_records, metrics, failure_handle, raw_handle)
            if not lattice_records:
                progress.update(len(current_indices))
                continue

            element_groups: Dict[int, List[tuple[Dict[str, Any], str]]] = defaultdict(list)
            for record, prompt in zip(lattice_records, elements_prompts):
                element_groups[int(record["num_atoms"])].append((record, prompt))
            coord_records: List[Dict[str, Any]] = []
            coord_prompts: List[str] = []
            for num_atoms, grouped in sorted(element_groups.items()):
                records = [item[0] for item in grouped]
                prompts = [item[1] for item in grouped]
                gen_length = rounded_length(
                    int(num_atoms) * int(args.elements_tokens_per_site) + int(args.elements_extra_tokens),
                    int(args.elements_block_length),
                )
                element_texts = generate_texts(
                    model,
                    tokenizer,
                    prompts,
                    gen_length=gen_length,
                    block_length=args.elements_block_length,
                    temperature=args.temperature,
                    cfg_scale=args.cfg_scale,
                    remasking=args.remasking,
                )
                parsed_records, parsed_prompts = parse_elements_batch(element_texts, records, metrics, failure_handle, raw_handle)
                coord_records.extend(parsed_records)
                coord_prompts.extend(parsed_prompts)
            if not coord_records:
                progress.update(len(current_indices))
                continue

            coord_groups: Dict[int, List[tuple[Dict[str, Any], str]]] = defaultdict(list)
            for record, prompt in zip(coord_records, coord_prompts):
                coord_groups[int(record["num_atoms"])].append((record, prompt))
            for num_atoms, grouped in sorted(coord_groups.items()):
                records = [item[0] for item in grouped]
                prompts = [item[1] for item in grouped]
                raw_length = int(num_atoms) * int(args.coords_tokens_per_site) + int(args.coords_extra_tokens)
                gen_length = min(args.coords_max_gen_length, rounded_length(raw_length, int(args.coords_block_length)))
