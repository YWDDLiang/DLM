...
from crystal_dlm.ordinal_rng import sha256_text
...
H1_PLANNER_PROMPT_STYLE_RICH_NOCHARGE = H1_RICH_NOCHARGE_PLAN_FORMAT
...
def build_planner_user_prompt(*, sample_idx: int | None = None, prompt_style: str | None = None) -> str:
    ...
    if style == H1_PLANNER_PROMPT_STYLE_RICH_NOCHARGE:
        return (
            f"{CRYSLLMGEN_TEXT_PROMPT.rstrip()}\n\n"
            ...
            "- Use valid element symbols only.\n"
            "- Use a chemically plausible MP-20-like bulk composition.\n"
            "- Do not emit charge, oxidation states, ions, N, elements, counts, coordinates, lattice lengths, angles, CIF, candidates, or explanations.\n"
            "- Do not include any extra text before or after the six lines."
            f"{sample_line}"
        )
    prefill_hint = (
...
def canonical_plan_record_for_style(
    ...
    *,
    ...
    style = normalize_prompt_style(prompt_style)
    plan_style = _plan_style_for_prompt_style(style)
    ...
    return {
        ...
        "raw_plan_text_sha256": sha256_text(str(raw_plan_text)),
        "plan_text_sha256": sha256_text(canonical_plan_text),
        "parsed_plan": dict(plan),
        "plan_state": dict(plan),
        "prompt": body_prompt,
        "body_prompt_sha256": sha256_text(body_prompt),
...
def _plan_style_for_prompt_style(prompt_style: str) -> str:
    if prompt_style == H1_PLANNER_PROMPT_STYLE_RICH_PLAN:
        return H1_RICH_PLAN_FORMAT
    if prompt_style == H1_PLANNER_PROMPT_STYLE_RICH_NOCHARGE:
        return H1_RICH_NOCHARGE_PLAN_FORMAT
    return R5C_FORMULA_END_PLAN_FORMAT
