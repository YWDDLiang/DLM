...
def verify_published_dataset(data_dir: str | Path) -> Dict[str, Any]:
    ...
    if (body_dir / "manifest.json").is_file():
        root = body_dir
        layout = "h1a2_sidecar_root"
    else:
        root = body_dir.parent
        layout = "plangraph_v1_body_subdir"
    manifest_path = root / "manifest.json"
    success_path = root / "_SUCCESS"
    if layout == "plangraph_v1_body_subdir" and body_dir.name != "body":
        raise PlannedPreflightError(
            "preflight expects either a published H1-A2 sidecar root or the "
            "PlanGraph v1 dataset's body directory"
        )
    ...
    if manifest.get("dataset_version") not in allowed_versions:
        raise PlannedPreflightError("unexpected planned dataset version")
    if (
        manifest.get("dataset_version") == H1A2_SIDECAR_DATASET_VERSION
        and layout != "h1a2_sidecar_root"
    ):
        raise PlannedPreflightError(
            "H1-A2 sidecar dataset must be passed as its dataset root"
        )
    if (
        manifest.get("dataset_version") == PLANGRAPH_DATASET_VERSION
        and layout != "plangraph_v1_body_subdir"
    ):
        raise PlannedPreflightError(
            "PlanGraph v1 dataset must be passed as its body directory"
        )
    if manifest.get("published") is not True:
        raise PlannedPreflightError("dataset manifest is not marked published")
    if manifest.get("dataset_version") == H1A2_SIDECAR_DATASET_VERSION:
        if (
            manifest.get("source_plan_provenance")
            != "structure_derived_teacher_plan_state"
            or manifest.get("model_proposed_plan") is not False
            or manifest.get("eligible_as_end_to_end_planner_evidence") is not False
        ):
            raise PlannedPreflightError(
                "H1-A2 sidecar must remain teacher-plan body supervision and "
                "must not claim model-proposed Planner evidence"
            )
...
def preflight_planned_data(
    *,
    ...
    if normalized_policy not in {"d1", "d2"}:
        raise PlannedPreflightError(
            "preflight policy must be d1 or d2"
        )
    if int(max_length) <= 0:
