...
**最新用户要求（先完成文档登记）：默认最后用4小时分析和寻找提升；若发现很好的机制，允许延长验证时间，随后整理发布仓库，再执行固定1200 Plan的三轮完整链路。** 发布目标为 [YWDDLiang/DLM_ICLR](https://github.com/YWDDLiang/DLM_ICLR)。公开仓库要求高复用，只保留当前推理/训练主流程及有有效性证据的改动，清理失败实验实现、冗余和杂乱内容，禁止设置各种人为 hard gate。Plan 来源须可参数化选择复用现有 R03 256 条、H1A2 1200 条或重新调用 Plan 模型；推理、自提升、训练和数据处理使用统一入口，支持 `.sh` 与 Slurm 一键启动。训练默认超参数对应现有实际运行配置；数据层从 MP20 起步，保留接入多个其它数据集的统一接口。仓库整理发布后，固定1200 Plan按原顺序只看前1000条有效记录，自提升完整执行3轮。详细原文、顺序与实现要求见[最新要求登记](docs/r03_paper_story_20260907/EXPERT_REPAIR_DISCUSSION.md#release-final-run-20260910)。本次仅登记要求，四小时窗口的起止时间在实际启动时单独记录，不从之前已完成的实验追溯计时。
