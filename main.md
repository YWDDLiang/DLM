...
输入边界纠正：前述SUN排序和“已测当前态作KEEP基准”的多组结果使用了当前态物理评价信息，只能作为带当前反馈的离线对照，不能当作纯模型自主推理结果。原数字、权重和生成改动保留；正在训练与核验仅使用Plan/当前结构/候选结构及模型特征的自主评分器，物理信息仅作为离线标签或事后评测。正式发布和运行按自主输入边界执行。见[输入边界回执](docs/r03_paper_story_20260907/receipts/INFERENCE_INPUT_BOUNDARY_CORRECTION_20260911.json)。
...
用户最新补充：后续验证以证明KEEP/EDIT机制可行为目标，要求不宜苛刻；正式实验使用另外整理的干净训练数据，重点确保来源/组成隔离、去重、可靠标签及验证与训练边界。最终固定1200 Plan清单取原始顺序前1050个合法Plan，三轮集合一致。

**用户追加的后续解决任务：有效训练不足、候选位置集中、接受条件误拒绝、失败输入修复，四项均须继续处理。** 这些不只是历史诊断；更新排序器不等于内容/定位训练完成，扩候选不等于学会选位置，开放尝试不等于修复成功，失败人数少也不能将其移出任务。逐项工作与未解决状态已写入[最新要求登记§0.21](docs/r03_paper_story_20260907/EXPERT_REPAIR_DISCUSSION.md#release-final-run-20260910)，与默认四小时、好机制可延长、发布仓库整理及最终三轮运行一并执行。

**最新用户要求已开始执行（2026-09-10 13:02:01 UTC）：默认最后用4小时分析和寻找提升；若发现很好的机制，允许延长验证时间，随后整理发布仓库，再执行固定1200 Plan的三轮完整链路。** 发布目标为 [YWDDLiang/DLM_ICLR](https://github.com/YWDDLiang/DLM_ICLR)。公开仓库要求高复用，只保留当前推理/训练主流程及有有效性证据的改动，清理失败实验实现、冗余和杂乱内容，禁止设置各种人为 hard gate。Plan 来源须可参数化选择复用现有 R03 256 条、H1A2 1200 条或重新调用 Plan 模型；推理、自提升、训练和数据处理使用统一入口，支持 `.sh` 与 Slurm 一键启动。训练默认超参数对应现有实际运行配置；数据层从 MP20 起步，保留接入多个其它数据集的统一接口。仓库整理发布后，固定1200 Plan按原顺序取前1050个合法Plan，自提升完整执行3轮。详细原文、顺序与实现要求见[最新要求登记](docs/r03_paper_story_20260907/EXPERT_REPAIR_DISCUSSION.md#release-final-run-20260910)。四小时窗口已于13:02:01 UTC开始，默认截止17:02:01 UTC（北京时间次日01:02:01）；发现好机制可延长。实际记录见[分析窗口](docs/r03_paper_story_20260907/execution/FINAL_IMPROVEMENT_WINDOW_20260910.json)。

2026-09-10 的 SUN 范围／排序阶段已完成。DEV 预先选定的终态概率模型在 **256 个新组成来源上把 SUN 从 13 提到 19（+6，+2.344 个百分点），没有损失原 SUN**；MSUN 为129，持平；Stable 为19，相对KEEP少1。另一项固定差值对照得到Stable22、SUN15、MSUN139，即+2/+2/+10，但未通过此前DEV的SUN增长条件。因此已获得新的SUN收益和两种不同取向的真实模型，完整默认采用门槛仍未通过，未在新终测后更换赢家或追调阈值。

本轮先核实旧E三轮各只有1次实际更新、384条实际更新行中仅24条局部位置监督、约90%的旧提案集中site0，再补充其它位置候选。TRAIN的新颖Stable晋升机会从7个来源扩大到19个；新数据1917行/492来源，两个300参数判别器各完整训练64遍、512步。原增量模型暴露“当前已经MSUN、期望增量上界为0”的候选级门槛问题，随后在新终测打开前登记终态概率对照，DEV选定阈值0.2。

旧246条严格组成子集的探索性复核为Stable32→34、SUN28→33、MSUN121→128。新256条的SUN增量95%来源配对bootstrap区间为[+0.781,+4.297]个百分点；尚未测量重复随机流波动。新增6条SUN有5条原本已Stable，4条弛豫终态前后仍匹配，不能等同于发现6种新稳定物相。差值对照选中的Bi₂Se₃Br₆则从hull+0.121355降至-0.006238 eV/atom，且弛豫终态不再匹配原结构。

两个模型分别通过5020个真实完整E判断视图重载核验，分数差异与选择差异均为0；输出字节与绑定候选/原KEEP一致。24个Slurm任务全部成功结束，分配4.4453 GPU小时、19.3056 CPU小时，峰值5GPU/28CPU/3任务，最后计算结束于12:19:16 UTC。旧预算、G/F和暂停在S1的1000主流程均未更改。详见[完整结果与论证](docs/r03_paper_story_20260907/SUN_RANK_SCOPE_RESULTS_20260910.md)、[完整回执](docs/r03_paper_story_20260907/receipts/SUN_RANK_SCOPE_FINAL_20260910.json)、[两组权重与使用说明](docs/r03_paper_story_20260907/artifacts/sun_rank_scope_20260910/README.md)。

以下为上一阶段已归档记录。

连续KEEP、四轮有界接受训练/策略尝试，以及单候选、两候选、四候选的实际物理评价在上一阶段完成。**当时已有小幅收益，但没有证明SUN稳定、大幅提升。** 所有原始失败、固定模型和评分均保留，1000主流程仍停在S1，没有恢复G/F训练或采样。
