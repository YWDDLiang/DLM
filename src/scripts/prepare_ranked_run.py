...
def prepare(old,root):
    ...
    spec.update(run_id='ranked_train_rsi_20260909:S0',run_root=str(fit),ranked_training=True,
        round_index=0,training_parent_root=str(fit/'cohort'),collect_training_proposals=False,
        selection_tag='same_256_TRAIN_Plans_ranked_v2')
    ...
    spec['resources']={'wall_hours':12,'max_gpus':6,'base_gpus':4,'max_submitted_slurm_jobs':3,
    ...
    write_json(twin/'cohort/MANIFEST.json',{'source_split':'train','requests':256,
    ...
    receipt={'source_split':'train','requests':256,'plans_sha256':file_hash(fit/'cohort/plans.jsonl'),
...
if __name__=='__main__':
    ...
    args=parser.parse_args();print(json.dumps(prepare(args.previous_root,args.root)))
