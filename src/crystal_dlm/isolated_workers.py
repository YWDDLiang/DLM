"""Killable persistent processes for bounded scientific evaluations.

Workers acknowledge readiness, receive one payload, and return one result.
Only active work is timed; queued work never inherits an earlier deadline.
"""
...
def isolated_results(tasks, *, worker_target, worker_arguments, task_timeout=180.,
                     startup_timeout=120., admitted=None):
    if (not worker_arguments or not math.isfinite(task_timeout) or task_timeout <= 0
    ...
    def start(arguments):
        ...
        process.start()
        child.close()
        slot = {'arguments': arguments, 'process': process, 'connection': parent,
                'ready': False, 'active': None, 'started': time.monotonic()}
    ...
    try:
        ...
        while pending or any(slot['active'] is not None for slot in slots):
            ready = set(wait([slot['connection'] for slot in slots], timeout=.1))
            replacements = []
                ...
                if slot['connection'] in ready:
                    try:
                        ...
                        if message.get('ready'):
                            ...
                            restart = isinstance(completed, dict) and completed.get('status') == 'worker_error'
