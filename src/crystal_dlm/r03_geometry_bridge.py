"""Optional existing geometry support at the frozen R03 candidate-logit hook.

The frozen constructor keeps its weights, prompt, prefill, reveal schedule and
paired-noise function. Only current-group coordinate logits receive periodic
alias aggregation and the existing 0.5 A / 125-image inter-site support. No
lattice-system, space-group or volume-bin projection is introduced.
"""
...
def _file_identity(function_or_path: Any) -> dict[str, Any]:
    location = function_or_path if isinstance(function_or_path, Path) else inspect.getsourcefile(function_or_path)
    if not location or not Path(location).is_file():
        return {"path": location, "sha256": None}
...
def _checked_groups(groups: Sequence[Sequence[int]]) -> tuple[tuple[int, ...], ...]:
    ...
    if (length < 11 or (length - 7) % 4 or not 1 <= (length - 7) // 4 <= 20
            or sorted(flattened) != list(range(length)) or any(not group for group in normalized)):
    ...
    if stages[:2] != ["prefilled_composition", "lattice"] or [order[s] for s in stages] != sorted(order[s] for s in stages):
        raise GeometryBridgeContractError("the frozen R03 lattice-to-X-to-Y-to-Z dependency order is required")
    return normalized
...
class ConstructionGeometryMonitor:
    def __init__(self, *, enabled: bool, tokenizer: Any, generation_position_groups: Sequence[Sequence[int]],
        ...
        required = {"representation": "dynamic_v1", "max_atoms": 20, "coord_period": 100,
                    "duplicate_coordinate_mask": True, "lattice_volume_mask": True, "min_lattice_rad": 1e-4}
        if not isinstance(native_constraints, Mapping) or any(native_constraints.get(k) != v for k, v in required.items()):
            raise GeometryBridgeContractError("original R03 schema, duplicate and nondegenerate-lattice guards must stay enabled")
        ...
        for key in ("count_token_to_n", "coord_token_to_bin", "z_bin_to_token_id", "angle_token_to_bin", "gamma_bin_to_token_id"):
        ...
        self.constraints = {**self.constraints, "lattice_volume_mask": False, "duplicate_coordinate_mask": False}
        self.element_ids = {int(value) for token, value in tokenizer.get_vocab().items()
    ...
    def source_receipts(self) -> dict[str, Any]:
        return {
            "bridge": _file_identity(Path(__file__)),
            "existing_geometry_masks": _file_identity(_apply_lightweight_decoding_masks),
            "token_map_builder": _file_identity(build_repair_constraints),
            "original_paired_candidate": None if self.original_candidate is None else _file_identity(self.original_candidate),
            "native_constraints_sha256": getattr(self, "native_constraints_sha256", None),
        }
    ...
    def report(self) -> dict[str, Any]:
        return {
            ...
            "schedule_changed": False, "paired_noise_changed": False,
        }

    def _fail(self, reason, x, prompt_length, semantic_group, step, active, *, positions=None):
        self.failed = True
        details = {
            "reason": reason, "row_indices": [0], "semantic_group": int(semantic_group),
            "step_in_group": int(step), "group_positions": list(self.groups[semantic_group]),
    ...
    def apply_logits(self, logits: torch.Tensor, *, current_tokens: torch.Tensor, prompt_length: int,
                     gen_length: int, semantic_group: int, step_in_group: int) -> torch.Tensor:
        if not self.enabled:
            return torch.zeros(current_tokens[:, prompt_length:].shape, dtype=torch.bool, device=current_tokens.device)
        x = current_tokens
        if (x.ndim != 2 or x.shape[0] != 1 or logits.ndim != 3 or logits.shape[:2] != x.shape
                or prompt_length < 0 or x.shape[1] != prompt_length + gen_length
                or gen_length != sum(map(len, self.groups)) or not logits.is_floating_point()
                or not 0 <= semantic_group < len(self.groups) or step_in_group < 0):
            raise GeometryBridgeContractError("geometry ON requires one exact R03 request and the original candidate-logit ABI")
        ...
        if any(int(body[0, 7 + 4 * slot]) not in self.element_ids for slot in range(n)):
            raise GeometryBridgeContractError("R03 element prefill is missing or outside the unchanged vocabulary")
        ...
        if not bool(active.any()):
            raise GeometryBridgeContractError("frozen candidate hook was called without an active masked position")
        stage = _stage(positions[0])
        if stage == "prefilled_composition":
            raise GeometryBridgeContractError("composition must remain prefilled and cannot be sampled by this bridge")
        if stage in "XYZ":
            for offset, name in ((1, "LA"), (2, "LB"), (3, "LC"), (4, "AA"), (5, "AB"), (6, "AG")):
                mapping = self.constraints["length_token_to_bin" if offset < 4 else "angle_token_to_bin"][name]
                if int(body[0, offset]) not in mapping:
                    raise GeometryBridgeContractError("active coordinates require the completed, valid-token lattice")
            angles = [self.constraints["angle_token_to_bin"][name][int(body[0, offset])]
                      for offset, name in ((4, "AA"), (5, "AB"), (6, "AG"))]
            lattice = _lattice_matrix_from_token_ids(x[0], prompt_length=prompt_length, constraints=self.constraints)
            if lattice_angle_rad(*angles) <= PROTOCOL["min_lattice_rad"] or lattice is None or not bool(torch.isfinite(lattice).all()):
                self._fail("nondegenerate_lattice_has_no_legal_continuation", x, prompt_length, semantic_group, step_in_group, active)
            for slot in range(n):
                for component, axis in enumerate("XYZ"):
                    ...
                    if token != self.mask_id and token not in self.constraints["coord_token_to_bin"][axis]:
                        raise GeometryBridgeContractError("visible coordinate is outside the unchanged R03 coordinate vocabulary")
                    if stage == "Z" and axis in "XY" and token == self.mask_id:
                        raise GeometryBridgeContractError("active R03 Z requires every X and Y to be already visible")
        ...
        active_positions = active[0].nonzero(as_tuple=False).flatten().tolist()
        if empty:
            self._fail("empty_active_legal_support", x, prompt_length, semantic_group, step_in_group, active,
...
def construction_geometry_bridge(paired_module: Any, *, tokenizer: Any,
    ...
    if not enabled:
        yield monitor
        return
    original = paired_module._paired_suffix_candidates
    if getattr(original, "_r03_geometry_bridge_hook", False):
        raise GeometryBridgeContractError("construction geometry hook cannot be nested or applied twice")
    monitor.original_candidate = original
    def hooked(logits, **kwargs):
        active = monitor.apply_logits(logits, **{name: kwargs[name] for name in
            ("current_tokens", "prompt_length", "gen_length", "semantic_group", "step_in_group")})
        # The old categorical sampling and confidence calculation are unchanged,
        # including the exact original per-attempt noise namespace and shape.
        x0, confidence = original(logits, **kwargs)
        suffix = logits[:, kwargs["prompt_length"]:]
        choices = x0[:, kwargs["prompt_length"]:][active]
        selected = suffix[active].gather(1, choices[:, None]).flatten()
        if not bool((torch.isfinite(selected) & (selected > torch.finfo(logits.dtype).min)).all()):
            raise GeometryBridgeContractError("native candidate selected a masked action despite nonempty support")
        return x0, confidence
    hooked._r03_geometry_bridge_hook = True
    paired_module._paired_suffix_candidates = hooked
    try:
        yield monitor
    finally:
        paired_module._paired_suffix_candidates = original


__all__ = ["SCHEMA", "PROTOCOL", "GeometryBridgeContractError", "GeometryNoLegalSupport",
           "ConstructionGeometryMonitor", "construction_geometry_bridge"]
