"""Exact directed N/U predicates, evaluated only where SUN can still be true."""
...
def matcher_contract():
    ...
    return {'settings': matcher.as_dict(), 'pymatgen_version': importlib.metadata.version('pymatgen'),
...
def _match_worker(connection, contract):
    ...
    while True:
        ...
        if record is None:
            return
        try:
            matched = bool(matcher.fit(Structure.from_dict(record['left']), Structure.from_dict(record['right'])))
...
class PairCache:
    ...
    def descriptor(self, left, right):
        identity = {'schema': PAIR_SCHEMA, 'left_sha256': fingerprint(left),
                    'right_sha256': fingerprint(right), 'matcher_sha256': self.contract_sha}
    ...
    def get(self, key, identity):
        ...
        if (saved.get('identity') != identity or saved.get('key') != key
...
def evaluate_sun_predicates(structures, train_structures, train_formula_index, required_indices, *,
                            cache_dir, frozen_nu_sha256, training_identity, workers=4, pair_timeout=30.,
                            startup_timeout=120., pair_runner=None, contract=None):
    ...
    if len(required) != len(required_indices) or any(type(i) is not int or not 0 <= i < len(structures) for i in required):
    ...
    def process_groups(groups):
        ...
        def admitted(key, payload):
            return any(states[index]['value'] is not False and states[partner(index)]['value'] is not False
                       and key in states[index]['pending'] for index in consumers[key])
        ...
        if pair_runner is None:
            results = isolated_results(active_tasks, worker_target=_match_worker,
                        worker_arguments=[(runtime,)]*workers, task_timeout=pair_timeout,
                        startup_timeout=startup_timeout, admitted=admitted)
        else:
        ...
        for key, _, result in results:
            ...
            if matched is None:
                ...
                problems.append({'pair_key': key, 'error': result.get('error'), 'affected_inputs': sorted(consumers[key])})
            else:
    ...
    for index in required:
        ...
        groups[('U', index)] = [(payloads[index], payloads[earlier])
                               for earlier in formula_indices[formulas[index]] if earlier < index]
    ...
    return {'novel': novel, 'unique': unique, 'required_indices': required,
            'unresolved_sun_indices': unresolved, 'pair_attempt_problems': problems,
            'counters': dict(counters), 'complete_sun_predicates': not unresolved,
            'standalone_NU_complete': all(x is not None for x in novel+unique),
            'matcher_contract': cache_contract, 'training_identity': training_identity,
