"""Editor content and decision learning with an effective update per minibatch."""
...
from crystal_dlm.editor_t2t import dense_loss, dense_vectors
from crystal_dlm.expert_edit import ExpertEditObjective
...
def permute_atoms(row, permutation):
    ...
    for key in ('current_tokens', 'proposal_tokens', 'content_target_tokens', 'chosen_tokens',
                'rejected_tokens', 'healthy_anchor_tokens'):
        body = result.get(key)
        if body is not None:
            if len(body) != 7 + 4 * n:
    ...
    for key in ('action_positions', 'content_positions'):
        if key in result:
            result[key] = sorted(p if p < 7 else 7 + 4 * old_to_new[(p - 7) // 4] + (p - 7) % 4
                                 for p in result[key])
...
def train_editor_minibatches(model, tokenizer, examples, spec, selected, reference, optimizer,
                            support, output, write_json):
    ...
    timed_out = False
    for epoch, indices in epoch_indices(len(examples), batch_size=spec['batch_size'], world=world,
        ...
        for index in indices[rank * width:(rank + 1) * width]:
            ...
            if row.get('content_target_tokens'):
                ...
                row['training_cut'] = 0 if rng.random() < .5 else rng.randrange(len(order))
            rows.append(row)
        ...
        loss_heads, count = editor_head_loss(model, tokenizer, rows, device, detach_content=True,
                                             site_objective='categorical')
        head_value = None
        ...
        for name, parameters, present in zip(('content', 'heads'), (content, heads), active.tolist()):
            ...
            norms[name] = float(torch.nn.utils.clip_grad_norm_([p for _, p in parameters], 1., error_if_nonfinite=True))
        optimizer.step()
        if bool(active[0]):
            content_steps += 1
        ...
        event = dict(step=len(history) + 1, epoch=epoch, dense_CE=ce_value, reference_KL=kl_value,
                     decision_loss=head_value, gradient_norms=norms, source_examples_per_GPU=width,
                     content_optimizer_steps=content_steps, head_optimizer_steps=head_steps,
                     content_rows=len(targets), head_rows=count, update_kind='minibatch',
        ...
        if bool(stop):
            timed_out = True
            break
    report = dict(pair_visits=dict(inspected), pair_visits_semantics='actual_minibatches',
                  content_updated_pair_visits=dict(content_visits), head_updated_pair_visits=dict(head_visits),
                  content_updated_token_visits=dict(token_visits), local_site_target_visits=dict(site_visits),
                  content_optimizer_steps=content_steps, head_optimizer_steps=head_steps,
                  KL_stop=False, KL_trigger=None, head_continuation_after_KL=False,
                  ...
                  epochs_cap=spec['epochs'], timed_out=timed_out)
