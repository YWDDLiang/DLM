...
if [[ -e "$RUN/submission_record.json" ]]; then
  echo "H1 exact replay already has a submission record" >&2
  exit 2
fi
...
job_id="$(
  cd "$ROOT"
  sbatch --parsable \
    --export=ALL,H1_EXACT_REPLAY_SOURCE_SHA256="$source_sha" \
    "$SOURCE/replay_p0.sbatch"
)"
...
record = {
    "schema": "h1_exact_replay_submission_record_v1",
    "status": "complete",
