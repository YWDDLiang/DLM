...
class CompletionContractTests(unittest.TestCase):
    ...
    def test_no_gpu_slurm_rerun_or_downstream(self) -> None:
        ...
        self.assertFalse(self.config["execution"]["network_reads_allowed"])
        self.assertTrue(authorization["completed_snapshot_reuse_authorized"])
        self.assertFalse(
            authorization["completed_hull_zero_unknown_gate_enabled"]
        )
        for name in (
    ...
    def test_offline_snapshot_reuse_has_no_secret_or_query_path(self) -> None:
        self.assertEqual(self.config["materials_project"]["network_queries"], 0)
        self.assertEqual(self.config["materials_project"]["transport_retries"], 0)
        self.assertNotIn("H1_R03F_MP_KEY_FILE", self.runner)
        self.assertNotIn("--key-file", self.runner)
        self.assertNotIn("read_and_destroy_api_key(args", self.source)
        self.assertNotIn("query_missing_chemsys(", self.source.split("def execute", 1)[1])
    ...
    def test_completed_attempts_preserve_parity_and_resolve_unknown(self) -> None:
        ...
        completed, counts = complete_arm_attempts(
            ...
            completed_snapshot_chemsys={"A-B", "A-C"},
        )
    ...
    def test_residual_unknown_is_false_and_conserved_not_rejected(self) -> None:
        attempts = [
            {
        ...
        completed, counts = complete_arm_attempts(
            context=context,
            hulls={("r0_control", "x-0"): None},
            strict_threshold=0.0,
            meta_threshold=0.1,
            execution_manifest_sha256="a" * 64,
            completed_snapshot_chemsys={"A-B"},
        )
        self.assertEqual(counts["source_unknown_resolved"], 0)
        self.assertEqual(counts["relaxation_or_hull_unknown"], 1)
        self.assertFalse(completed[0]["metrics"]["strict_full_sun"])
        self.assertFalse(completed[0]["metrics"]["meta_full_sun"])
