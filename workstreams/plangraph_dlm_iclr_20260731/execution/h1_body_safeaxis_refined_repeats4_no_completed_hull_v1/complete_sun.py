#!/usr/bin/env python3
"""Recompute frozen R03E S.U.N. without the completed-hull zero-unknown gate.

The R03E and failed R03F sources are immutable. This program performs no
generation, refinement, CHGNet relaxation, direct metric, novelty, filtering,
ranking, training, sample retry/replacement, network query, or API-key work. It
reuses the one frozen R03F 227-system thermodynamic snapshot and maps it back to
all 4 repeats x 2 arms x 256 original attempts. Existing finite hull values
must reproduce exactly. Each source unknown must be conserved as resolved or
still unknown; residual unknowns score false for strict/meta S.U.N. and remain
...
def complete_arm_attempts(
    *,
    ...
    completed_snapshot_chemsys: set[str],
    ...
    for source in context["attempts"]:
        ...
        row["schema"] = "crysllmgen_r5c_a100_sun_no_completed_hull_attempt_v1"
        ...
        row["hull_recompute"] = {
            ...
            "reference_source": (
                "frozen_r03f_common_snapshot"
                if target["chemsys"] in completed_snapshot_chemsys
...
def _validate_authorization(config: Mapping[str, Any]) -> None:
    ...
    if (
        config.get("status") != "user_authorized_no_completed_hull_recompute"
        or config["execution"]["slurm_allowed"] is not False
        or config["execution"]["gpu_allowed"] is not False
        or config["execution"]["network_reads_allowed"] is not False
        or authorization["manual_mp_reference_completion_authorized"] is not False
        or authorization["completed_snapshot_reuse_authorized"] is not True
        or authorization["completed_hull_zero_unknown_gate_enabled"] is not False
        or config["sun"]["completed_hull_zero_unknown_gate_enabled"] is not False
        or config["sun"]["residual_hull_unknown_scored_false"] is not True
        or config["sun"]["residual_hull_unknown_reported"] is not True
        or any(
        ...
        raise CompletionError("R03G authorization firewall changed")
...
def prepare_frozen_state(args: argparse.Namespace) -> dict[str, Any]:
    if os.environ.get("SLURM_JOB_ID") or os.environ.get("SLURM_JOB_NAME"):
        raise CompletionError("R03G is login-node-only and forbids Slurm")
    if os.environ.get("CUDA_VISIBLE_DEVICES", "") not in {"", "-1"}:
        raise CompletionError("R03G requires CUDA_VISIBLE_DEVICES empty")
    execution_sha = require_hex_sha(
        args.execution_manifest_sha256, "R03G source manifest"
    )
    ...
    source_r03f_run = Path(str(config["source_r03f"]["run_root"])).resolve()
    if not source_r03f_run.is_dir():
        raise FileNotFoundError(source_r03f_run)
    r03f_failure_path = _resolve_frozen_file(
        source_r03f_run,
        config["source_r03f"]["failure_report"],
        "frozen R03F failure report",
    )
    r03f_query_progress_path = _resolve_frozen_file(
        source_r03f_run,
        config["source_r03f"]["query_progress"],
        "frozen R03F query progress",
    )
    r03f_snapshot_path = _resolve_frozen_file(
        source_r03f_run,
        config["source_r03f"]["common_snapshot"],
        "frozen R03F common snapshot",
    )
    r03f_failure = read_json(r03f_failure_path)
    r03f_query_progress = read_jsonl(r03f_query_progress_path)
    r03f_snapshot_rows = read_jsonl(r03f_snapshot_path)
    if (
        r03f_failure.get("schema") != "h1_r03f_failure_report_v1"
        or r03f_failure.get("status") != "failed_no_retry"
        or r03f_failure.get("reason")
        != "r0_control completed hull hard gate failed"
        or r03f_failure.get("sample_retry_or_replacement_used") is not False
        or r03f_failure.get("automatic_downstream") is not False
    ):
        raise CompletionError("R03F failure identity changed")
    ...
    if set(configured_arms) != set(ARM_KEYS):
        raise CompletionError("R03G eight-arm source mapping changed")
    contexts = {
    ...
    snapshot_chemsys = [str(row.get("chemsys")) for row in r03f_snapshot_rows]
    snapshot_cache = {
        str(row["chemsys"]): row.get("entries") for row in r03f_snapshot_rows
    }
    query_chemsys = [str(row.get("chemsys")) for row in r03f_query_progress]
    if (
        len(missing_union)
        ...
        or len(r03f_query_progress) != len(missing_union)
        or len(set(query_chemsys)) != len(query_chemsys)
        or set(query_chemsys) != missing_union
        or any(row.get("status") != "resolved" for row in r03f_query_progress)
        or any(int(row.get("transport_retries", -1)) != 0 for row in r03f_query_progress)
        or len(r03f_snapshot_rows) != len(wanted_chemsys)
        or len(set(snapshot_chemsys)) != len(snapshot_chemsys)
        or set(snapshot_chemsys) != wanted_chemsys
        or any(
            not isinstance(snapshot_cache.get(system), list)
            or not snapshot_cache[system]
            for system in wanted_chemsys
        )
    ):
        raise CompletionError("R03G frozen snapshot scope changed")
    ...
    if args.run_root.resolve() != configured_run_root:
        raise CompletionError("fixed R03G run root changed")
    if configured_run_root.exists() and args.preflight_only:
        raise CompletionError("fixed R03G run root already exists")
    if configured_run_root.exists() and not args.preflight_only:
        existing = {path.name for path in configured_run_root.iterdir()}
        if existing - {"logs"}:
            raise CompletionError("fixed R03G run root is not empty")
    input_contract = {
        "schema": "h1_r03g_frozen_input_contract_v1",
        "status": "pass",
        ...
        "source_r03f": {
            "source_manifest_sha256": config["source_r03f"][
                "source_manifest_sha256"
            ],
            "failure_report": identity(r03f_failure_path),
            "query_progress": identity(r03f_query_progress_path),
            "common_snapshot": identity(r03f_snapshot_path),
            "resolved_queries": len(r03f_query_progress),
            "snapshot_chemsys": len(r03f_snapshot_rows),
        },
        "repeat_count": 4,
        "arm_count": 8,
        ...
        "completed_hull_zero_unknown_gate_enabled": False,
        "residual_hull_unknown_scored_false": True,
        "network_used": False,
        "slurm_used": False,
        "gpu_used": False,
        "api_key_read": False,
    ...
    return {
        ...
        "source_r03f_run": source_r03f_run,
        "r03f_failure_path": r03f_failure_path,
        "r03f_query_progress_path": r03f_query_progress_path,
        "r03f_snapshot_path": r03f_snapshot_path,
        "r03f_snapshot_cache": snapshot_cache,
        "contexts": contexts,
...
def _verify_source_evidence_unchanged(state: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise CompletionError("frozen base MP hull cache changed during R03G")
    source_r03f_run = state["source_r03f_run"]
    for field, label in (
        ("failure_report", "frozen R03F failure report"),
        ("query_progress", "frozen R03F query progress"),
        ("common_snapshot", "frozen R03F common snapshot"),
    ):
        _resolve_frozen_file(
            source_r03f_run,
            config["source_r03f"][field],
            label,
        )
...
def execute(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.preflight_only:
        return {
            "schema": "h1_r03g_non_network_preflight_v1",
            "status": "pass",
    ...
    submission_record = {
        "schema": "h1_r03g_login_node_submission_record_v1",
        "status": "started",
        ...
        "api_key_read": False,
        "api_key_serialized": False,
        "frozen_r03f_snapshot_reused": True,
        "completed_hull_zero_unknown_gate_enabled": False,
        "generation_rerun": False,
        "refinement_rerun": False,
        "chgnet_rerun": False,
    ...
    claim = {
        "schema": "h1_r03g_no_completed_hull_claim_v1",
        "status": "claimed_before_offline_recomputation",
        "created_at_utc": created_at,
        "run_id": config["run_id"],
        "execution_manifest_sha256": state["execution_sha"],
        "logical_mp_queries": 0,
        "common_snapshot_for_all_eight_arms": True,
        "common_snapshot": identity(state["r03f_snapshot_path"]),
        "completed_hull_zero_unknown_gate_enabled": False,
        "residual_hull_unknown_scored_false": True,
        "network_used": False,
        "api_key_read": False,
    ...
    snapshot_path = state["r03f_snapshot_path"]
    snapshot_cache = state["r03f_snapshot_cache"]
    hulls = compute_hulls(state["all_targets"], snapshot_cache)
    ...
    for arm_key in ARM_KEYS:
        ...
        completed, counts = complete_arm_attempts(
            ...
            execution_manifest_sha256=state["execution_sha"],
            completed_snapshot_chemsys=state["wanted_chemsys"],
        )
        expected = config["source_run"]["arms"][arm_key]["expected_counts"]
        if (
            counts["source_evaluated_parity"] != int(expected["source_evaluated"])
            or counts["source_unknown_resolved"]
            + counts["relaxation_or_hull_unknown"]
            != int(expected["source_hull_unknown"])
        ):
            raise CompletionError(f"{arm_key} hull parity/conservation gate failed")
        arm_dir = (
            run_root
            / "repeats"
            / str(context["repeat"])
        ...
        vector_payload = {
            "schema": "h1_r03g_all_attempt_sun_vectors_v1",
        ...
        summary = {
            "schema": "h1_r03g_no_completed_hull_arm_v1",
            "status": "complete_with_residual_unknowns_disclosed",
            "ok": True,
            "repeat": context["repeat"],
            "arm": context["arm"],
            "arm_key": arm_key,
            ...
            "coverage_delta": {
                "e_hull_unknown": counts["relaxation_or_hull_unknown"]
                - source_counts["e_hull_unknown"],
            ...
            "completed_hull_zero_unknown_gate_enabled": False,
            "residual_hull_unknown_scored_false": True,
            "residual_hull_unknown_reported": True,
            "network_used": False,
            "api_key_read": False,
            "generation_rerun": False,
            "refinement_rerun": False,
    ...
    total_unknown_resolved = sum(
        values["source_unknown_resolved"] for values in counts_by_key.values()
    )
    ...
    if (
        total_existing_parity != total_existing_finite
        or total_unknown_resolved + total_unknown_after != total_unknown_before
    ):
        raise CompletionError("R03G global hull parity/conservation gate failed")

    terminal = {
        "schema": "h1_r03g_refined_repeats4_no_completed_hull_terminal_v1",
        "status": "complete_without_completed_hull_zero_unknown_gate",
        "ok": True,
        "created_at_utc": created_at,
        "run_id": config["run_id"],
        "execution_manifest_sha256": state["execution_sha"],
        ...
        "source_r03f": {
            "failure_report": identity(state["r03f_failure_path"]),
            "query_progress": identity(state["r03f_query_progress_path"]),
            "common_snapshot": identity(state["r03f_snapshot_path"]),
            "evidence_unchanged": True,
        },
        "repeat_count": 4,
        ...
        "coverage": {
            ...
            "e_hull_unknown_resolved": total_unknown_resolved,
            "unknown_conservation_pass": (
                total_unknown_resolved + total_unknown_after
                == total_unknown_before
            ),
            "existing_finite_e_hull": total_existing_finite,
            "existing_finite_e_hull_parity": total_existing_parity,
            "existing_finite_e_hull_parity_rate": (
                total_existing_parity / total_existing_finite
            ),
            "completed_hull_zero_unknown_gate_enabled": False,
            "residual_hull_unknown_scored_false": True,
            "interpretation": "strict/meta counts are lower bounds under residual-unknown-false semantics",
        },
        "snapshot_reuse": {
            ...
            "logical_queries_submitted": 0,
            "transport_attempts_total": 0,
            "transport_retries_total": 0,
            "source_r03f_query_progress": identity(
                state["r03f_query_progress_path"]
            ),
            "common_snapshot": identity(snapshot_path),
            "network_used": False,
            "api_key_read": False,
        ...
        "automatic_downstream": False,
        "decision": "no_completed_hull_report_only_no_automatic_promotion",
        "recommended_next_step": (
            "Interpret strict/meta as residual-unknown-false lower bounds, compare "
            "the paired effects to frozen R03E, then authorize at most one minimal "
            "H1-based scientific change."
        ),
    }
    ...
    decision_record = {
        "schema": "h1_r03g_no_completed_hull_decision_v1",
        ...
        if not args.preflight_only and run_root.is_dir():
            ...
            if not failure_path.exists():
                write_json_exclusive(
                    failure_path,
                    {
                        "schema": "h1_r03g_failure_report_v1",
