...
from crystal_dlm.plangraph_v1 import ensure_valid_plangraph
...
def plangraph_shuffled_dependency_groups(
    graph: Mapping[str, Any],
    *,
    corruption_key: int,
    seed: int,
) -> tuple[PositionGroup, ...]:
    """Return a deterministic D2 mechanism control with shuffled dependencies.

    Group membership is bit-for-bit identical to D2; only the sequential
    prerequisite order changes.  The ordering is a content-keyed SHA-256 rank,
    so it does not depend on sample IDs, row order, Python RNG state, or
    training step.  An accidental identity permutation is rotated to ensure
    that the negative control actually differs from D2.
    """

    original = plangraph_dependency_groups(graph)

    def rank(group: PositionGroup) -> bytes:
        payload = (
            f"plangraph_d2_shuffle_v1\0{int(seed)}\0"
            f"{int(corruption_key)}\0{group.name}"
        )
        return hashlib.sha256(payload.encode("utf-8")).digest()

    shuffled = sorted(original, key=lambda group: (rank(group), group.name))
    if len(shuffled) > 1 and [group.name for group in shuffled] == [
        group.name for group in original
    ]:
        rotation_digest = hashlib.sha256(
            (
                f"plangraph_d2_shuffle_rotation_v1\0{int(seed)}\0"
                f"{int(corruption_key)}"
            ).encode("utf-8")
        ).digest()
        rotation = 1 + int.from_bytes(rotation_digest[:8], "big") % (
            len(shuffled) - 1
        )
        shuffled = [*shuffled[rotation:], *shuffled[:rotation]]
    groups = _sequential_groups(
        [(group.name, group.positions) for group in shuffled]
    )
...
__all__ = [
    ...
    "plangraph_shuffled_dependency_groups",
    "position_group_ids",
