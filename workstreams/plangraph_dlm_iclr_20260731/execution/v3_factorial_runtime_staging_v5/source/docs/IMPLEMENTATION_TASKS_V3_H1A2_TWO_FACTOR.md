...
- [-] Freeze the DLM learning rate and resource envelope after the smoke.
  - Resource envelope is frozen at 2xA800/8 CPU/64 GiB/2h per arm,
    concurrency one, global batch 16.
  - LR is not frozen: 5e-5 raised fixed-panel validation loss by 7.90% (B1)
    and 6.52% (B2) after two updates.
- [ ] Train B1 and B2 for at most 400 updates with validation every 50.
- [ ] Select B2 only if it beats both B0 likelihood noninferiority and the B1
  registered dependency margin.
...
- [-] Prove that the body `plan_state`/PlanGraph is compiled from the persisted
  sampled P0/P* Plan and can never fall back to a structure-derived teacher
  Plan.
  - Contract result: body `plan_state` and prompt are compiled only from the
    persisted model-sampled record, and teacher provenance fails closed.
    PlanGraph/body-runner wiring remains pending.
- [ ] Enforce prompt/answer byte parity, additive tokenization, exact
...
- [x] Freeze per-ordinal body noise, refiner noise, and evaluation order.
  - Result: contract-level identities and strict four-arm merge pass 27/27
    targeted tests; runtime runner integration is still required.
...
- [ ] Freeze B1/B2 learning rate and update count after that smoke while
  retaining global effective batch `16`.
- [ ] Replace rank/batch-dependent inference RNG with stateless per-ordinal
  Planner/body/refiner seeds.
  - Contract layer is complete; the historical body/refiner runner is left
    untouched and a fresh factorial runner must consume the contract.
...
They contain no job IDs and are blocked on explicit scientific authorization;
the DLM manifest is additionally blocked on a separately authorized
single-candidate `5e-6` engineering LR recheck.
