...
The screen is capped at `400` updates with validation every `50` updates.
The exact learning rate and full-run resource envelope must be frozen in a
new execution manifest after the revised 32-row smoke; V1/V2 values are not
silently inherited.
...
Its canonical learning rate is `5e-5`; B1/B2 are continuations, so their
learning rate is still selected by the registered short smoke and then frozen.
