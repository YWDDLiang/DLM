#!/usr/bin/env python3
"""Assemble the paired H1 exact-plan versus PlanGraph schedule screen."""
...
def main() -> None:
    ...
    if (
        ...
        or generation_report.get("generation_policies") != ["d1", "d2"]
    ...
    for label, schedule_arm, generation_policy, attempts_path in (
        ...
        ("candidate", "D2", "d2", candidate_path),
    ):
    ...
    candidate_policy_exact = all(
        row.get("generation_policy") == "d2"
        and row.get("schedule_arm") == "D2"
        and row.get("body_checkpoint_arm") == "B0"
        and row.get("planner_arm") == "P0"
    ...
    report = {
        "schema": "h1_body_schedule32_terminal_report_v1",
        "status": "complete",
        "gate_passed": gate_passed,
        "decision": (
            "schedule32_safe_to_expand"
            if gate_passed
        ...
        "candidate": {
            "identity": "H1_P0_B0_plangraph_schedule",
