#!/usr/bin/env python3
"""Run the sole R03 treatment: B0 body generation with the D2 schedule."""
...
def load_tasks(config: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for cell in rows[:denominator]:
        ...
        d2 = h1a2_generation_schedule(plan_state, policy="d2")
        tasks.append(
            {
                "ordinal": ordinal,
                "eligible": True,
                ...
                "candidate_schedule": d2,
                "candidate_schedule_sha256": canonical_sha256(d2),
                "schedule_changed": d1 != d2,
            }
        )
    ...
    if sum(task.get("schedule_changed") is True for task in tasks) != eligible:
        raise ValueError("D2 is not an active schedule treatment on every eligible row")
    return tasks
...
def base_record(
    ...
    *,
    ...
    return {
        "schema": "h1_body_schedule32_attempt_v1",
...
def generate_paired_arms(
    *,
    ...
    write_json_exclusive(
        ...
        {
            "schema": "h1_body_schedule32_shared_batch_partition_v1",
            "partition": shared_partition,
            "sha256": canonical_sha256(shared_partition),
            "applied_identically_to_arms": ["D1", "D2"],
        },
    )
    ...
    for label, generation_policy, schedule_key in (
        ...
        ("candidate", "d2", "candidate_schedule"),
    ):
    ...
    report = {
        "schema": "h1_body_schedule32_generation_report_v1",
        "status": "complete",
...
def main() -> None:
    ...
    if (
        config.get("schema") != "h1_body_schedule32_config_v1"
        or int(config.get("denominator", -1)) != 32
        or config.get("automatic_downstream") is not False
        or config["treatment"].get("candidate_policy") != "d2"
    ):
