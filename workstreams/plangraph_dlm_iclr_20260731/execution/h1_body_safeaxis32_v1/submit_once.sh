...
SOURCE="$ROOT/workstreams/plangraph_dlm_iclr_20260731/execution/h1_body_safeaxis32_v1"
RUN="$ROOT/runs/20260802_h1_body_safeaxis32_v1"

if [[ -e "$RUN/submission_record.json" ]]; then
  echo "H1 body safeaxis32 already has a submission record" >&2
  exit 2
fi
...
job_id="$(
  cd "$ROOT"
  sbatch --parsable \
    --export=ALL,H1_BODY_SAFEAXIS32_SOURCE_SHA256="$source_sha" \
...
record = {
    "schema": "h1_body_safeaxis32_submission_record_v1",
    "status": "complete",
