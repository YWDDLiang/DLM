...
: "${H1_R03F_MP_KEY_FILE:?private MP key file path is required}"

if [[ -n "${SLURM_JOB_ID:-}" || -n "${SLURM_JOB_NAME:-}" ]]; then
  echo "ERROR: R03F is A800 login-node-only" >&2
  exit 2
fi
if [[ ! "$H1_R03F_SOURCE_SHA256" =~ ^[0-9a-f]{64}$ ]]; then
  echo "ERROR: invalid R03F source manifest SHA256" >&2
  exit 3
fi
if [[ ! -x "$PYTHON" || ! -s "$H1_R03F_MP_KEY_FILE" ]]; then
...
umask 077
mkdir -p "$RUN/logs"
trap 'status=$?; if [[ -n "${H1_R03F_MP_KEY_FILE:-}" && -f "${H1_R03F_MP_KEY_FILE}" ]]; then rm -f "${H1_R03F_MP_KEY_FILE}"; fi; exit "$status"' EXIT
