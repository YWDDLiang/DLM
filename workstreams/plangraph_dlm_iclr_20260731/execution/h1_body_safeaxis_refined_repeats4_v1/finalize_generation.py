#!/usr/bin/env python3
"""Convert one R03E refined payload into a 256-row all-attempt ledger."""
...
def main() -> None:
    ...
    if (
        ...
        or {str(row.get("arm")) for row in attempts} != {arm}
        ...
        or metrics.get("new_scientific_seed_per_repeat") is not False
        or any(metrics.get(key) is not False for key in (
            "retry",
            "replacement",
            "repair",
            "filter",
        ...
        raise ValueError("R03E refinement all-attempt contract changed")
        ...
        rows.append(
            {
                ...
                "attempt_id": attempt_id(repeat, arm, ordinal),
                "method": method,
                "ordinal": ordinal,
                "sample_idx": ordinal,
                "repeat": repeat,
                "pair_id": f"h1-r03e-repeat{repeat}:{ordinal:04d}",
                "arm": arm,
                "planner_arm": "P0",
                "body_arm": "B0",
                "schedule_arm": config["arms"][arm]["schedule"],
    ...
    report = {
        "schema": "h1_r03e_generation_report_v1",
        "status": "complete",
        "ok": True,
        "arm": arm,
        "repeat": repeat,
