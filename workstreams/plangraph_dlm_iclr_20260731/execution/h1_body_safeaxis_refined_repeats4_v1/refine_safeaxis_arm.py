#!/usr/bin/env python3
"""Refine one frozen R03D arm using the common H1 ordinal seed ledger."""
    ...
    if ordered:
        payload = {
            "frac_coords": torch.cat(
                [row["frac_coords"] for row in ordered], dim=1
            ),
            "num_atoms": torch.cat([row["num_atoms"] for row in ordered], dim=1),
            "atom_types": torch.cat(
                [row["atom_types"] for row in ordered], dim=1
            ),
...
def main() -> None:
    ...
    if (
        ...
        != Path(config["refiner"]["checkpoint"]).resolve()
        or args.checkpoint_sha256 != config["refiner"]["checkpoint_sha256"]
        or int(config["refiner"]["diffusion_steps"]) != FROZEN_DIFF_STEPS
        or int(config["refiner"]["effective_batch_size"]) != 1
    ):
        raise ValueError("frozen refiner identity or configuration changed")

    attempts = ordered_rows(
        read_jsonl(args.body_attempts.resolve()),
        ordinal_field="ordinal",
    )
    ledger = ordered_rows(
        read_jsonl(args.attempt_ledger.resolve()),
    ...
    if (
        len(success_ordinals) != int(specification["expected_body_success"])
        or any(
            int(seed_row.get("ordinal", -1)) != ordinal
            or not isinstance(seed_row.get("refiner_noise_seed"), int)
            or (
                body_row.get("body_eligible") is True
                and int(body_row.get("body_noise_seed", -1))
                != int(seed_row.get("body_noise_seed", -2))
            )
            for ordinal, (body_row, seed_row) in enumerate(zip(attempts, ledger))
        )
    ):
    ...
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("R03E expects exactly one CUDA device")
    ...
    with torch.no_grad():
        for ordinal in tqdm(range(DENOMINATOR), desc=f"R03E r{repeat} {arm}"):
            body = dict(attempts[ordinal])
            seed_record = ledger[ordinal]
            record = {
                **body,
                "schema": "h1_r03e_refinement_attempt_v1",
                "arm": arm,
                "repeat": repeat,
                "sample_idx": ordinal,
                "ordinal": ordinal,
                "refiner_sampling_seed": int(seed_record["refiner_noise_seed"]),
                ...
                record.update(
                    {
                        ...
                        "failure_reason": str(body.get("reason") or "upstream_failed"),
            ...
            seed = int(seed_record["refiner_noise_seed"])
            record.update(
                {
                    "attempt_status": "failed",
            ...
            try:
                ...
                sampled, _ = model.sample(batch, diff_steps=FROZEN_DIFF_STEPS)
    ...
    payload = _write_refined_payload(
        output,
        refined_samples,
        elapsed=elapsed,
    )
    ...
    report = {
        "schema": "h1_r03e_refiner_metrics_v1",
        "status": "complete",
        "arm": arm,
        "repeat": repeat,
