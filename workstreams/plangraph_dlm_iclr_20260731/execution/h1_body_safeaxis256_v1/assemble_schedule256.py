...
def paired_completion_outcomes(
    control: list[Mapping[str, Any]],
    candidate: list[Mapping[str, Any]],
) -> dict[str, Any]:
    both_succeeded = 0
    ...
    for left, right in zip(control, candidate, strict=True):
        left_ok = left.get("status") == "succeeded"
        right_ok = right.get("status") == "succeeded"
    ...
    denominator = len(control)
    return {
        "both_succeeded": both_succeeded,
        "both_failed": both_failed,
        "control_only_succeeded": control_only,
        "candidate_only_succeeded": candidate_only,
        "discordant": discordant,
        "candidate_minus_control_count": candidate_only - control_only,
        "candidate_minus_control_percentage_points": (
            100.0 * (candidate_only - control_only) / denominator
        ),
...
def main() -> None:
    ...
    if (
        config.get("schema") != "h1_body_safeaxis256_config_v1"
        or denominator != 256
    ...
    expected_eligible = int(frozen["expected_first256_body_eligible"])
    expected_ineligible = int(frozen["expected_first256_body_ineligible"])
    expected_ineligible_ordinals = list(frozen["expected_ineligible_ordinals"])
    expected_eligible_ordinals = [
        ordinal
        for ordinal in range(denominator)
        if ordinal not in set(expected_ineligible_ordinals)
    ]
    partition_ordinals = (
        [item for batch in partition for item in batch]
    ...
    if (
        ...
        or int(generation_report.get("body_eligible_attempts", -1))
        != expected_eligible
        or int(generation_report.get("planner_ineligible_attempts", -1))
        != expected_ineligible
        ...
        or int(invariant_report.get("attempts", -1)) != expected_eligible
        or int(invariant_report.get("raw_denominator", -1)) != denominator
        or int(invariant_report.get("eligible_attempts", -1))
        != expected_eligible
        or int(invariant_report.get("ineligible_attempts", -1))
        != expected_ineligible
        or invariant_report.get("eligible_ordinals_sha256")
        != frozen["eligible_ordinals_sha256"]
        ...
        or sorted(partition_ordinals) != expected_eligible_ordinals
        or int(partition_report.get("raw_denominator", -1)) != denominator
        or int(partition_report.get("eligible_attempts", -1))
        != expected_eligible
        or int(partition_report.get("ineligible_attempts", -1))
        != expected_ineligible
        or partition_report.get("ineligible_ordinals")
        != expected_ineligible_ordinals
        or partition_report.get("eligible_ordinals_sha256")
        != frozen["eligible_ordinals_sha256"]
    ...
    for label, schedule_arm, generation_policy, attempts_path in (
        ...
        if (
            ...
            or int(arm_report.get("body_eligible_attempts", -1))
            != expected_eligible
            or int(arm_report.get("planner_ineligible_attempts", -1))
            != expected_ineligible
            or int(arm_report.get("exact_length_matches", -1))
            != expected_eligible
    ...
    historical_identity_exact = (
        int(historical_summary["succeeded"])
        == int(frozen["expected_historical_succeeded"])
        and historical_summary["failure_classes"]
        == frozen["expected_historical_failure_classes"]
    )
    ...
    schedule_invariant_exact = all(
        (
            ...
            and row.get("schedule_z_before_xy_count") == 0
        ...
        or (
            row.get("body_eligible") is False
            and row.get("earliest_failure_stage") == "planner"
            and row.get("schedule_z_before_xy_count") is None
            and row.get("schedule_all_xy_precede_all_z") is None
            and row.get("schedule_invariant_sha256") is None
        )
        for row in [*control, *candidate]
    )
    exact_length_exact = all(
        (
            row.get("body_eligible") is True
            and row.get("body_generation_complete") is True
            and isinstance(row.get("expected_body_token_count"), int)
            and row.get("actual_body_token_count")
            == row.get("expected_body_token_count")
            and row.get("exact_length_match") is True
            and len(row.get("raw_body_token_ids") or [])
            == row.get("expected_body_token_count")
        )
        or (
            row.get("body_eligible") is False
            and row.get("body_generation_complete") is False
            and row.get("expected_body_token_count") is None
            and row.get("actual_body_token_count") is None
            and row.get("exact_length_match") is None
        )
        for row in [*control, *candidate]
    )
    ...
    gate_passed = (
        ...
        and (
            treatment_applied == expected_eligible
        ...
        and exact_length_exact
        and no_forbidden_attempt_actions
    )
    report = {
        "schema": "h1_body_safeaxis256_terminal_report_v1",
        "status": "complete",
        "gate_passed": gate_passed,
        "decision": (
            "safe_axis256_confirmed"
            if gate_passed
            else "scientific_stop_retain_h1_exact_plan"
        ),
        "denominator_per_arm": denominator,
        "body_eligible_per_arm": expected_eligible,
        "planner_ineligible_per_arm": expected_ineligible,
