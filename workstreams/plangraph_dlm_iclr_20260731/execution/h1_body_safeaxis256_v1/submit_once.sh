...
if [[ -e "$RUN/submission_record.json" ]]; then
  echo "H1 body safeaxis256 already has a submission record" >&2
  exit 2
fi
...
job_id="$(
  cd "$ROOT"
  sbatch --parsable \
    --export=ALL,H1_BODY_SAFEAXIS256_SOURCE_SHA256="$source_sha" \
    "$SOURCE/schedule256.sbatch"
)"
...
record = {
    "schema": "h1_body_safeaxis256_submission_record_v1",
    "status": "complete",
