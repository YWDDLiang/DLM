...
from assemble_schedule256 import (  # noqa: E402
    output_agreement,
    paired_completion_outcomes,
...
class H1BodySafeAxis256ProtocolTests(unittest.TestCase):
    ...
    def test_authorization_and_config_change_only_schedule(self) -> None:
        ...
        self.assertEqual(config["denominator"], 256)
        self.assertTrue(config["parent_r03c"]["gate_passed"])
        self.assertEqual(
            config["parent_r03c"]["source_manifest_sha256"],
            "2c3485f8cf539ed5e33f3303a695947a2bd5fe72e83456d6f6f20cd5dca156bd",
        )
        self.assertEqual(
            config["parent_r03c"]["terminal_report_sha256"],
        ...
        self.assertEqual(
            config["frozen_h1"]["expected_ineligible_ordinals"],
            [86, 211],
        )
        self.assertEqual(
            config["frozen_h1"]["expected_historical_succeeded"],
            246,
        )
    ...
    def test_paired_identity_and_failure_summary_are_fail_closed(self) -> None:
        ...
        paired = paired_completion_outcomes(
            control,
            [
                {**control[0], "status": "succeeded"},
                {**control[1], "status": "succeeded"},
            ],
        )
        self.assertEqual(paired["candidate_minus_control_count"], 1)
        self.assertEqual(paired["candidate_only_succeeded"], 1)
    ...
    def test_full_denominator_keeps_planner_failures_out_of_gpu_batches(self) -> None:
        runner = (HERE / "run_schedule256.py").read_text(encoding="utf-8")
        assembler = (HERE / "assemble_schedule256.py").read_text(
            encoding="utf-8"
        )
        self.assertIn("expected_first256_body_eligible", runner)
        self.assertIn("expected_first256_body_ineligible", runner)
        self.assertIn("expected_ineligible_ordinals", runner)
        self.assertIn(
            'if not task["eligible"]:\n            continue',
            runner,
        )
        self.assertIn('"reason": str(task["reason"])', runner)
        self.assertIn(
            "sorted([*eligible_ordinals, *ineligible_ordinals])",
            runner,
        )
        self.assertIn("treatment_applied == expected_eligible", assembler)
        self.assertIn("exact_length_7_plus_4n", assembler)

    def test_single_slurm_job_has_no_forbidden_downstream(self) -> None:
        script = (HERE / "schedule256.sbatch").read_text(encoding="utf-8")
        self.assertIn("#SBATCH --gres=gpu:NVIDIAA800-SXM4-80GB:1", script)
        self.assertNotIn("#SBATCH --array", script)
        self.assertIn("run_schedule256.py", script)
        self.assertIn("assemble_schedule256.py", script)
        runner = (HERE / "run_schedule256.py").read_text(encoding="utf-8")
