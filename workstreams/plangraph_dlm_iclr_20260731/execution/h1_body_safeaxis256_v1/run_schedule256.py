#!/usr/bin/env python3
"""Run R03D: frozen H1 D1 versus safe-axis on all 256 ordinals."""
...
def load_tasks(config: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    eligible_ordinals = [
        int(task["ordinal"]) for task in tasks if task["eligible"]
    ]
    ineligible_ordinals = [
        int(task["ordinal"]) for task in tasks if not task["eligible"]
    ]
    if eligible != int(frozen["expected_first256_body_eligible"]):
        raise ValueError("first-256 H1 body eligibility changed")
    if len(ineligible_ordinals) != int(
        frozen["expected_first256_body_ineligible"]
    ):
        raise ValueError("first-256 H1 body ineligibility count changed")
    if ineligible_ordinals != list(frozen["expected_ineligible_ordinals"]):
        raise ValueError("first-256 H1 ineligible ordinals changed")
    if canonical_sha256(eligible_ordinals) != frozen["eligible_ordinals_sha256"]:
        raise ValueError("first-256 H1 eligible ordinal identity changed")
...
def base_record(
    ...
    *,
    ...
    return {
        "schema": "h1_body_safeaxis256_attempt_v1",
        ...
        "body_eligible": bool(task["eligible"]),
        "num_atoms": (
            int(task["plan_state"]["N"]) if task["eligible"] else None
        ),
        "expected_body_token_count": (
            exact_body_token_count(task["plan_state"])
            if task["eligible"]
            else None
        ),
...
def generate_paired_arms(
    *,
    ...
    expected_eligible = int(
        config["frozen_h1"]["expected_first256_body_eligible"]
    )
    expected_ineligible = int(
        config["frozen_h1"]["expected_first256_body_ineligible"]
    )
    if (
        len(invariant_rows) != expected_eligible
        or any(
    ...
    invariant_report = {
        "schema": "h1_body_safeaxis256_invariant_report_v1",
        "status": "complete",
        "attempts": len(invariant_rows),
        "raw_denominator": int(config["denominator"]),
        "eligible_attempts": expected_eligible,
        "ineligible_attempts": expected_ineligible,
        "eligible_ordinals_sha256": canonical_sha256(
            [int(row["ordinal"]) for row in invariant_rows]
        ),
    ...
    eligible_ordinals = [
        int(item) for batch in shared_partition for item in batch
    ]
    ineligible_ordinals = [
        int(task["ordinal"]) for task in tasks if not task["eligible"]
    ]
    if (
        sorted(eligible_ordinals)
        != [int(task["ordinal"]) for task in tasks if task["eligible"]]
        or ineligible_ordinals
        != list(config["frozen_h1"]["expected_ineligible_ordinals"])
        or sorted([*eligible_ordinals, *ineligible_ordinals])
        != list(range(int(config["denominator"])))
    ):
        raise ValueError("eligible/ineligible shared partition changed")
    write_json_exclusive(
        output_dir / "shared_batch_partition.json",
        {
            "schema": "h1_body_safeaxis256_shared_batch_partition_v1",
    ...
    for label, generation_policy, schedule_key in (
        ...
        progress = tqdm(
            ...
            desc=f"H1 P0+B0 {schedule_arm} schedule256",
        )
        for batch in shared_batches:
            ...
            for task, token_ids, text in zip(batch, generated, texts, strict=True):
                ...
                record["actual_body_token_count"] = len(
                    record["raw_body_token_ids"]
                )
                ...
                if record["exact_length_match"] is not True:
                    raise ValueError(
                        f"{schedule_arm} exact-length mismatch at {ordinal}"
                    )
                try:
        ...
        arm_reports[label] = {
            ...
            "body_eligible_attempts": expected_eligible,
            "planner_ineligible_attempts": expected_ineligible,
    ...
    report = {
        "schema": "h1_body_safeaxis256_generation_report_v1",
        "status": "complete",
...
def main() -> None:
    ...
    if (
        ...
        or config["parent_r03c"].get("gate_passed") is not True
        or config["parent_r03c"].get("source_manifest_sha256")
