...
def schedule_for_plan(
    ...
    *,
    generation_schedule: str,
) -> List[List[int]] | None:
    """Resolve a frozen H1 or matched PlanGraph inference schedule."""

    if generation_schedule == "default":
        return None
    if generation_schedule == "exact-plan":
        return exact_dynamic_generation_schedule(int(plan_state["N"]))
    if generation_schedule == "plangraph":
        return h1a2_generation_schedule(
            plan_state,
            policy="d2",
        )
    raise ValueError(f"unsupported generation schedule: {generation_schedule}")


def schedule_signature(schedule: List[List[int]] | None) -> Tuple[Tuple[int, ...], ...]:
    if schedule is None:
        return ()
    return tuple(tuple(int(position) for position in group) for group in schedule)
...
def main() -> None:
    ...
    for task in tasks:
        task["_generation_schedule"] = schedule_for_plan(
            task["plan_state"],
            generation_schedule=args.generation_schedule,
        )
        task["_generation_schedule_signature"] = schedule_signature(
            task["_generation_schedule"]
        )
    tasks.sort(
        key=lambda item: (
            int(item["plan_state"]["N"]),
            item["_generation_schedule_signature"],
            int(item["sample_idx"]),
        )
    )
    ...
    with raw_path.open("w", encoding="utf-8") as raw_handle, failure_path.open("w", encoding="utf-8") as failure_handle:
        ...
        while offset < len(tasks):
            ...
            schedule_key = tasks[offset]["_generation_schedule_signature"]
            while (
                offset < len(tasks)
                and len(batch) < args.batch_size
                and int(tasks[offset]["plan_state"]["N"]) == num_atoms
                and tasks[offset]["_generation_schedule_signature"] == schedule_key
            ):
            ...
            schedule = batch[0]["_generation_schedule"]
