...
def _metric_p_map(func, iterable):
    """Bound p_tqdm workers without changing the metric definition."""
    num_cpus = os.environ.get("CRYSLLMGEN_METRICS_NUM_CPUS")
    if not num_cpus:
        return p_map(func, iterable)
    num_cpus = int(num_cpus)
    if num_cpus <= 1:
        return list(map(func, iterable))
    return p_map(func, iterable, num_cpus=num_cpus)
...
def main(args):
    ...
    if 'recon' in args.tasks:
        ...
        if not args.multi_eval:
            pred_crys = _metric_p_map(lambda x: Crystal(x), crys_array_list)
        else:
            pred_crys = []
            for i in range(len(crys_array_list)):
                print(f"Processing batch {i}/ {len(crys_array_list)}")
                pred_crys.append(_metric_p_map(lambda x: Crystal(x), crys_array_list[i]))

        gt_crys = _metric_p_map(lambda x: Crystal(x), true_crystal_array_list)
    ...
    if 'gen' in args.tasks:
        ...
        gen_crys = _metric_p_map(lambda x: Crystal(x), crys_array_list)

        csv = pd.read_csv(args.gt_file)
        gt_crys = _metric_p_map(get_gt_crys_ori, csv['cif'])
