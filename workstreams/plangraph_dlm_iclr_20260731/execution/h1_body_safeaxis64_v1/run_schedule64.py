#!/usr/bin/env python3
"""Run R03C: frozen H1 D1 versus safe-axis on the first 64 ordinals."""
...
def load_tasks(config: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if eligible != int(frozen["expected_first64_body_eligible"]):
        raise ValueError("first-64 H1 body eligibility changed")
...
def base_record(
    ...
    *,
    ...
    return {
        "schema": "h1_body_safeaxis64_attempt_v1",
...
def generate_paired_arms(
    *,
    ...
    invariant_report = {
        "schema": "h1_body_safeaxis64_invariant_report_v1",
        "status": "complete",
    ...
    write_json_exclusive(
        ...
        {
            "schema": "h1_body_safeaxis64_shared_batch_partition_v1",
    ...
    for label, generation_policy, schedule_key in (
        ...
        progress = tqdm(
            ...
            desc=f"H1 P0+B0 {schedule_arm} schedule64",
        )
    ...
    report = {
        "schema": "h1_body_safeaxis64_generation_report_v1",
        "status": "complete",
...
def main() -> None:
    ...
    if (
        config.get("schema") != "h1_body_safeaxis64_config_v1"
        or int(config.get("denominator", -1)) != 64
        ...
        or config["parent_r03b"].get("gate_passed") is not True
        or config["parent_r03b"].get("source_manifest_sha256")
        != "cd21ee664f5c227698e96187906944a7d29016bd270da645c56ed6501cb3c866"
    ):
