...
SOURCE="$ROOT/workstreams/plangraph_dlm_iclr_20260731/execution/h1_body_safeaxis64_v1"
RUN="$ROOT/runs/20260802_h1_body_safeaxis64_v1"

if [[ -e "$RUN/submission_record.json" ]]; then
  echo "H1 body safeaxis64 already has a submission record" >&2
  exit 2
fi
...
job_id="$(
  cd "$ROOT"
  sbatch --parsable \
    --export=ALL,H1_BODY_SAFEAXIS64_SOURCE_SHA256="$source_sha" \
    "$SOURCE/schedule64.sbatch"
)"
...
record = {
    "schema": "h1_body_safeaxis64_submission_record_v1",
    "status": "complete",
