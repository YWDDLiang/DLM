...
from assemble_schedule64 import (  # noqa: E402
    output_agreement,
...
class H1BodySafeAxis64ProtocolTests(unittest.TestCase):
    ...
    def test_authorization_and_config_change_only_schedule(self) -> None:
        ...
        self.assertEqual(scope["attempts"], 64)
        ...
        self.assertEqual(config["denominator"], 64)
        self.assertTrue(config["parent_r03b"]["gate_passed"])
        self.assertEqual(
            config["parent_r03b"]["source_manifest_sha256"],
    ...
    def test_single_slurm_job_has_no_forbidden_downstream(self) -> None:
        script = (HERE / "schedule64.sbatch").read_text(encoding="utf-8")
        self.assertIn("#SBATCH --gres=gpu:NVIDIAA800-SXM4-80GB:1", script)
        self.assertNotIn("#SBATCH --array", script)
        self.assertIn("run_schedule64.py", script)
        self.assertIn("assemble_schedule64.py", script)
        runner = (HERE / "run_schedule64.py").read_text(encoding="utf-8")
