...
class ProtocolTests(unittest.TestCase):
    ...
    def test_config_is_diagnostic_and_refines_every_arm(self) -> None:
        ...
        self.assertEqual(
            config["body_models"]["expected_tokenizer_vocab_sha256"],
            "3acc073da85047265769f2dccd93543fa9d7cbfa95021aef54ef282b13ce2f37",
        )
        self.assertEqual(
            config["body_models"]["expected_tokenizer_json_sha256"],
            "3a21588abca8e56155cc7b6cabb81df51992ccd2e89704aec770912f24e75509",
        )
        self.assertEqual(config["body_models"]["expected_tokenizer_size"], 128830)

    def test_packed_arm_checks_frozen_checkpoint_tokenizer_files(self) -> None:
        script = (HERE / "arm_pipeline.sbatch").read_text(encoding="utf-8")
        self.assertIn(
            'sha256sum "$BODY_CHECKPOINT/tokenizer.json"',
            script,
        )
        self.assertIn(
            'sha256sum "$BODY_CHECKPOINT/tokenizer_config.json"',
            script,
        )
    ...
    def test_only_python_bytecode_is_ignored_by_source_inventory(self) -> None:
        self.assertTrue(
            _is_python_bytecode_cache(
                HERE / "__pycache__/protocol.cpython-310.pyc",
                HERE,
            )
        )
        self.assertTrue(
            _is_python_bytecode_cache(HERE / "runtime/module.pyo", HERE)
        )
        self.assertFalse(_is_python_bytecode_cache(HERE / "protocol.py", HERE))
