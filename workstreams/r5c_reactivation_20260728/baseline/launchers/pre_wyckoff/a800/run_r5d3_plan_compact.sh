...
PLAN_DATA_DIR="${PLAN_DATA_DIR:-data/dlm_sft/mp_20_r5_plan_compact}"
...
MAX_ATTEMPT_FACTOR="${MAX_ATTEMPT_FACTOR:-12}"
FORMULA_CAP="${FORMULA_CAP:-2}"
N_CAP_FRACTION="${N_CAP_FRACTION:-0.34}"
...
payload = {
  ...
  "formula_cap": int("${FORMULA_CAP}"),
  "n_cap_fraction": float("${N_CAP_FRACTION}"),
  "constraints": {
    "max_a800_gpus": 2,
...
Path("${NOTES_DIR}/r5d3_plan_compact_config.json").write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n")
PY
...
if [ "${REBUILD_PLAN_DATA}" = "1" ] || [ ! -f "${PLAN_DATA_DIR}/_SUCCESS" ]; then
  ...
  run_logged "${LOG_DIR}/build_r5_plan_compact_data.log" \
...
next_port
run_logged "${LOG_DIR}/r5d3_plan_train.log" \
  torchrun --nproc_per_node="${NPROC_PER_NODE}" --master_port="${NEXT_PORT}" scripts/llada_sft.py \
    ...
    --output-dir "${OUT_DIR}/r5d3_plan_compact_sft" \
...
next_port
run_logged "${LOG_DIR}/r5d3_plan_sample256.log" \
  torchrun --nproc_per_node="${NPROC_PER_NODE}" --master_port="${NEXT_PORT}" scripts/sample_llada_r5_plan_compact.py \
    --model-path "${MODEL_PATH}" \
    --checkpoint-path "${OUT_DIR}/r5d3_plan_compact_sft/final" \
    --stats-json "${PLAN_DATA_DIR}/stats.json" \
    --output-dir "${OUT_DIR}/r5d3_plan_sample256" \
    --num-samples "${SMOKE_SAMPLES}" \
    --batch-size "${SAMPLE_BATCH_SIZE}" \
    --temperature "${TEMPERATURE}" \
    --max-attempt-factor "${MAX_ATTEMPT_FACTOR}" \
    --formula-cap "${FORMULA_CAP}" \
    --n-cap-fraction "${N_CAP_FRACTION}" \
    --require-strict-smact

run_logged "${LOG_DIR}/r5d3_plan_gate.log" \
  python scripts/evaluate_r5d_plan_gate.py \
    --sample-metrics "${OUT_DIR}/r5d3_plan_sample256/sample_metrics.json" \
    --raw-generations-jsonl "${OUT_DIR}/r5d3_plan_sample256/raw_generations.jsonl" \
    --output-json "${NOTES_DIR}/r5d3_plan_sample256_gate.json" \
    --output-md "${NOTES_DIR}/r5d3_plan_sample256_gate_report.md"
