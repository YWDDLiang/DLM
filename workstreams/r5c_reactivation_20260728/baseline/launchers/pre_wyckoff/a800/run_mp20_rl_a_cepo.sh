...
if [ "$#" -ge 1 ] && [ -z "${BEST_CHECKPOINT:-}" ]; then
  BEST_CHECKPOINT="$1"
fi
...
CEPO_LAMBDA="${CEPO_LAMBDA:-0.2}"
CEPO_CLIP_EPS="${CEPO_CLIP_EPS:-0.2}"
SAMPLE_COUNT="${SAMPLE_COUNT:-256}"

RUN_DIR="runs/${RUN_ID}"
mkdir -p \
  "${RUN_DIR}/outputs/rollout" \
  "${RUN_DIR}/outputs/tracerl" \
  "${RUN_DIR}/outputs/sample256" \
  "${RUN_DIR}/notes"
...
(run / "notes" / "run_config.json").write_text(json.dumps({
    ...
    "rl_mode": "reason_aware_cepo_lite",
...
torchrun --nproc_per_node=2 scripts/rl_rollout_crystals.py \
  --checkpoint-path "${BEST_CHECKPOINT}" \
  --output-dir "${RUN_DIR}/outputs/rollout" \
