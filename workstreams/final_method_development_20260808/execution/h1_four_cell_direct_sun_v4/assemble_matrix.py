...
def _assemble(args: argparse.Namespace) -> dict[str, Any]:
    ...
    evaluation_contract = evidence["M00"]["summary"]["evaluation_contract"]
    if any(
        evidence[cell]["summary"]["evaluation_contract"]
        != evaluation_contract
        for cell in CELLS
    ):
        raise ValueError("four cells did not use one GCD/cache evaluation contract")
