#!/usr/bin/env python3
"""Convert one refined cell into the 256-row all-attempt generation ledger."""
...
def main() -> None:
    ...
    if (
        {str(row.get("cell")) for row in attempts} != {cell}
        ...
        rows.append(
            {
                ...
                "attempt_id": attempt_id(cell, ordinal),
                "method": method,
                "ordinal": ordinal,
                "sample_idx": ordinal,
    ...
    report = {
        "schema": "h1_ef_fourcell_generation_report_v1",
        "status": "complete",
        "ok": True,
        "cell": cell,
