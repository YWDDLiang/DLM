...
def complete(args: argparse.Namespace) -> dict[str, Any]:
    ...
    base_cache_rows = _nonempty_line_count(base_cache)
    if base_cache_rows != int(sun["base_mp_hull_cache_rows"]):
        raise ValueError("frozen base MP hull cache physical row count changed")
    ...
    base_cache_distinct_chemsys = len(all_cached_chemsys)
    if base_cache_distinct_chemsys != int(
        sun["base_mp_hull_cache_distinct_chemsys"]
    ):
        raise ValueError("frozen base MP hull cache distinct identity changed")
    ...
    claim = {
        ...
        "base_mp_hull_cache_rows": base_cache_rows,
        "base_mp_hull_cache_distinct_chemsys": base_cache_distinct_chemsys,
