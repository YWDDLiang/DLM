...
FROZEN_AUDIT_ROOT="${PROJECT_ROOT}/runs/20260808_h1_chemistry_first_sft_v2_raw64_smact4_audit_input_v1"
...
LOCAL_SMACT4_AUDIT_ROOT="${3:?local raw64 SMACT4 audit root}"
EXPECTED_LOCAL_SMACT4_AUDIT_MANIFEST_SHA256="${4:?local raw64 audit manifest digest}"

test -f "${RUN_ROOT}/submission_record.json"
test -f "${RUN_ROOT}/submission_record.sha256"
sha256sum -c "${RUN_ROOT}/submission_record.sha256"
...
test ! -e "${RUN_ROOT}/assembly64_submission_record.json"
test ! -e "${RUN_ROOT}/planner64/terminal"
test ! -e "${RUN_ROOT}/planner64/_SUCCESS"
test ! -e "${RUN_ROOT}/planner64/_SCIENTIFIC_STOP"
test ! -e "${RUN_ROOT}/planner64/_FAILED"
mkdir "${RUN_ROOT}/.submit_science64_assembly_lock"
...
PRIOR_GENERATION_SUBMISSION_SHA="$(sha256sum "${RUN_ROOT}/submission_record.json" | cut -d' ' -f1)"
PLANNER_JOB_ID="$(tr -d '[:space:]' < "${RUN_ROOT}/status/submitted_planner64_job_id.txt")"
case "${PLANNER_JOB_ID}" in ''|*[!0-9]*) echo "invalid planner64 job id: ${PLANNER_JOB_ID}" >&2; exit 3 ;; esac
sacct -n -X -j "${PLANNER_JOB_ID}" -o JobIDRaw,State,ExitCode -P \
  > "${RUN_ROOT}/status/sacct_planner64_before_assembly.txt"
for task in 0 1 2; do
  expected_id="${PLANNER_JOB_ID}_${task}"
  state="$(awk -F'|' -v wanted="${expected_id}" '$1 == wanted {print $2}' "${RUN_ROOT}/status/sacct_planner64_before_assembly.txt")"
  test "$(printf '%s\n' "${state}" | sed '/^$/d' | wc -l)" -eq 1
  case "${state%%+*}" in
    COMPLETED|FAILED|OUT_OF_MEMORY|TIMEOUT|NODE_FAIL|CANCELLED|PREEMPTED|BOOT_FAIL|DEADLINE) ;;
    *) echo "planner64 task not terminal: ${expected_id} state=${state}" >&2; exit 3 ;;
  esac
done
GENERATION_SACCT_SHA="$(sha256sum "${RUN_ROOT}/status/sacct_planner64_before_assembly.txt" | cut -d' ' -f1)"
AUDITED_ARMS="$("${LEGACY_PYTHON}" -c 'import json,sys; d=json.load(open(sys.argv[1])); assert d["status"]=="pass" and d["stage"]==64 and d["denominator"]==64; print(",".join(d["arms"]))' "${LOCAL_SMACT4_AUDIT_ROOT}/MANIFEST.json")"
case "${AUDITED_ARMS}" in
  p0|p0,sft_v2|p0,sft_v2_c|p0,sft_v2,sft_v2_c) ;;
  *) echo "invalid raw64 local audit arm list: ${AUDITED_ARMS}" >&2; exit 3 ;;
esac

test ! -e "${RUN_ROOT}/preflight/preflight_science64_assembly_report.json"
...
"${LEGACY_PYTHON}" "${EXECUTION_DIR}/preflight.py" \
  ...
  --output "${RUN_ROOT}/preflight/preflight_science64_assembly_report.json"
PREFLIGHT_SHA="$(sha256sum "${RUN_ROOT}/preflight/preflight_science64_assembly_report.json" | cut -d' ' -f1)"

partition_snapshot="$(sinfo -h -o '%P|%a|%l|%G' | sed 's/[*]//g')"
printf '%s\n' "${partition_snapshot}" | awk -F'|' '$1 == "normal" && $2 == "up" {found=1} END {exit found ? 0 : 1}'
printf '%s\n' "${partition_snapshot}" > "${RUN_ROOT}/status/sinfo_before_science64_assembly.txt"
squeue -h -u "${USER}" -o '%i|%j|%T|%M|%l|%P|%b|%R' > "${RUN_ROOT}/status/squeue_before_science64_assembly.txt"
SINFO_SHA="$(sha256sum "${RUN_ROOT}/status/sinfo_before_science64_assembly.txt" | cut -d' ' -f1)"
SQUEUE_SHA="$(sha256sum "${RUN_ROOT}/status/squeue_before_science64_assembly.txt" | cut -d' ' -f1)"

common_export="ALL,EXPECTED_SOURCE_INVENTORY_SHA256=${EXPECTED_SOURCE_INVENTORY_SHA256},LEGACY_PYTHON=${LEGACY_PYTHON},EXPECTED_LEDGER_SHA256=${LEDGER64_SHA},LOCAL_SMACT4_AUDIT_ROOT=${LOCAL_SMACT4_AUDIT_ROOT},EXPECTED_LOCAL_SMACT4_AUDIT_MANIFEST_SHA256=${EXPECTED_LOCAL_SMACT4_AUDIT_MANIFEST_SHA256},AUDITED_ARMS=${AUDITED_ARMS}"
ASSEMBLY_JOB_ID="$(sbatch --parsable --export="${common_export}" "${EXECUTION_DIR}/assemble64.sbatch")"
...
export ASSEMBLY_JOB_ID PRIOR_GENERATION_SUBMISSION_SHA LOCAL_SMACT4_AUDIT_ROOT AUDITED_ARMS
...
"${LEGACY_PYTHON}" "${EXECUTION_DIR}/write_submission_record.py" \
  --stage planner64_assembly --output "${RUN_ROOT}/assembly64_submission_record.json"
sha256sum "${RUN_ROOT}/assembly64_submission_record.json" > "${RUN_ROOT}/assembly64_submission_record.sha256"
