...
test -f "${RUN_ROOT}/V7_TERMINAL_RECORD.json"
sha256sum -c "${RUN_ROOT}/V7_TERMINAL_RECORD.sha256"
test ! -e "${RUN_ROOT}/optimizer_smoke_submission_record.json"
test ! -e "${RUN_ROOT}/optimizer_smoke"
test ! -e "${RUN_ROOT}/training"
test ! -e "${RUN_ROOT}/planner64"
mkdir "${RUN_ROOT}/.submit_optimizer_smoke_v8_lock"
...
test ! -e "${RUN_ROOT}/preflight/preflight_optimizer_smoke_v8.json"
...
"${LEGACY_PYTHON}" "${EXECUTION_DIR}/preflight.py" \
  ...
  --output "${RUN_ROOT}/preflight/preflight_optimizer_smoke_v8.json"
PREFLIGHT_SHA="$(sha256sum "${RUN_ROOT}/preflight/preflight_optimizer_smoke_v8.json" | cut -d' ' -f1)"

partition_snapshot="$(sinfo -h -o '%P|%a|%l|%G' | sed 's/[*]//g')"
printf '%s\n' "${partition_snapshot}" | awk -F'|' '$1 == "gpu" && $2 == "up" {found=1} END {exit found ? 0 : 1}'
printf '%s\n' "${partition_snapshot}" > "${RUN_ROOT}/status/sinfo_before_optimizer_smoke_v8.txt"
squeue -h -u "${USER}" -o '%i|%j|%T|%M|%l|%P|%b|%R' > "${RUN_ROOT}/status/squeue_before_optimizer_smoke_v8.txt"
SINFO_SHA="$(sha256sum "${RUN_ROOT}/status/sinfo_before_optimizer_smoke_v8.txt" | cut -d' ' -f1)"
SQUEUE_SHA="$(sha256sum "${RUN_ROOT}/status/squeue_before_optimizer_smoke_v8.txt" | cut -d' ' -f1)"

common_export="ALL,EXPECTED_SOURCE_INVENTORY_SHA256=${EXPECTED_SOURCE_INVENTORY_SHA256},LEGACY_PYTHON=${LEGACY_PYTHON}"
OPTIMIZER_SMOKE_JOB_ID="$(sbatch --parsable --array=0-1%2 --export="${common_export}" "${EXECUTION_DIR}/optimizer_smoke_v8.sbatch")"
printf '%s\n' "${OPTIMIZER_SMOKE_JOB_ID}" > "${RUN_ROOT}/status/submitted_optimizer_smoke_job_id.txt"
...
export OPTIMIZER_SMOKE_JOB_ID
export PARENT_V7_TERMINAL_SHA="$(sha256sum "${RUN_ROOT}/V7_TERMINAL_RECORD.json" | cut -d' ' -f1)"
"${LEGACY_PYTHON}" "${EXECUTION_DIR}/write_submission_record.py" \
  --stage optimizer_smoke \
  --output "${RUN_ROOT}/optimizer_smoke_submission_record.json"
sha256sum "${RUN_ROOT}/optimizer_smoke_submission_record.json" \
  > "${RUN_ROOT}/optimizer_smoke_submission_record.sha256"
printf 'optimizer_smoke=%s\n' "${OPTIMIZER_SMOKE_JOB_ID}"
