...
EXPECTED_SELF_SHA256="${1:?expected V16 generator SHA256}"
...
STAGE_ROOT="${PROJECT_ROOT}/runs/transfer_v16_sacct_array_task_repair_${EXPECTED_SELF_SHA256}"
GENERATED="${STAGE_ROOT}/prepare_and_submit_v16_raw256_assembly_on_a800.sh"
...
export V15_SOURCE GENERATED V16_RUN_ROOT EXPECTED_V15_SACCT_SHA256
"${PYTHON}" - <<'PY'
import os
...
if text.count("sacct -n -X -j") != 1:
    raise SystemExit("V15 sacct query identity mismatch")
if text.count("v15") < 8 or text.count("V15") < 5:
...
new_run = Path(os.environ["V16_RUN_ROOT"]).name
if text.count(old_run) != 2:
    raise SystemExit("V15 run-name identity mismatch")
text = text.replace(old_run, new_run)
text = text.replace("sacct -n -X -j", "sacct -n -j", 1)
text = text.replace("v15", "v16").replace("V15", "V16")
if "sacct -n -X -j" in text or "v15" in text or "V15" in text:
    raise SystemExit("stale V15 marker remains before V16 evidence insertion")
old_scope = (
    '    "repair_scope": '
...
new_scope = (
    ...
    '    "repair_scope": '
    '"assembly_path_schema_adapter_plus_sacct_array_task_visibility_only",\n'
)
if text.count(old_scope) != 1:
    raise SystemExit("V16 repair-record insertion identity mismatch")
text = text.replace(old_scope, new_scope, 1)

expected_root = f'RUN_ROOT="{os.environ["V16_RUN_ROOT"]}"'
if text.count(expected_root) != 1:
    raise SystemExit("V16 run-root identity mismatch")
if text.count("sacct -n -j") != 1:
    raise SystemExit("V16 sacct query replacement mismatch")
if text.count('"failed_v15_reason"') != 1:
    raise SystemExit("V15 failure evidence insertion mismatch")
generated.write_text(text, encoding="utf-8", newline="\n")
PY

bash -n "${GENERATED}"
grep -Fq 'sacct -n -j "${EXPECTED_JOB_ID}"' "${GENERATED}"
if grep -Fq 'sacct -n -X -j' "${GENERATED}"; then
  echo 'stale allocation-only sacct query remains' >&2
  exit 3
fi

GENERATED_SHA256="$(sha256sum "${GENERATED}" | cut -d' ' -f1)"
cat > "${STAGE_ROOT}/V16_GENERATION_RECORD.json" <<EOF
{
  "schema": "h1_chemistry_first_v16_sacct_array_task_repair_generator",
  "status": "pass",
  "v15_source_sha256": "${EXPECTED_V15_SOURCE_SHA256}",
  "failed_v15_sacct_sha256": "${EXPECTED_V15_SACCT_SHA256}",
  "failed_v15_reason": "sacct_-X_omitted_array_task_rows",
  "generated_v16_sha256": "${GENERATED_SHA256}",
  "repair_scope": "remove_sacct_-X_and_use_new_immutable_run_root_only",
...
(
  ...
  sha256sum \
    "$(basename "${SELF}")" \
    "$(basename "${GENERATED}")" \
    V16_GENERATION_RECORD.json
) > "${STAGE_ROOT}/V16_STAGE_SHA256.txt"
(cd "${STAGE_ROOT}" && sha256sum -c V16_STAGE_SHA256.txt)
