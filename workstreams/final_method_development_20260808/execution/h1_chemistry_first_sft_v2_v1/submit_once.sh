...
PREFLIGHT_SHA="$(sha256sum "${RUN_ROOT}/preflight/preflight_report.json" | cut -d' ' -f1)"
...
squeue -h -u "${USER}" -o '%i|%j|%T|%M|%l|%P|%b|%R' > "${RUN_ROOT}/status/squeue_before_initial64.txt"
SINFO_SHA="$(sha256sum "${RUN_ROOT}/status/sinfo_before_initial64.txt" | cut -d' ' -f1)"
SQUEUE_SHA="$(sha256sum "${RUN_ROOT}/status/squeue_before_initial64.txt" | cut -d' ' -f1)"
