#!/usr/bin/env python3
"""Offline regression tests for the immutable refine-import V8 continuation."""
...
def main() -> None:
    ...
    assert continuation["generation_outputs_reused"] is True
    assert continuation["slurm_job_ordinal_since_v5"] == 3
    assert continuation["remaining_official_slurm_jobs"] == 0
    body_continuation = config["body_continuation"]
    assert body_continuation["upstream_slurm_job_id"] == "31965"
    assert body_continuation["body_generation_rerun"] is False
    assert body_continuation["refinement_started"] is False
    assert body_continuation["required_body_succeeded"] == [246, 242, 250, 245]
    assert body_continuation["corrected_refine_pythonpath"] == (
        "SOURCE:R03E_RUNTIME:R03E:R03D:PROJECT"
    )
    ...
    assert scheduler["variant"] == "refine_import_continuation_single_job_max4_a800_32cpu_v8"
    ...
    assert "run_fresh_r03_refine_continuation" in combined
    ...
    body_refine = (source / "body_refine.sbatch").read_text(encoding="utf-8")
    assert "20260812_h1a2_retrained_world2_r03_postplanner_contract_repair_v7" in body_refine
    assert 'cp -a "$UPSTREAM_CELL/body" "$CELL/body"' in body_refine
    assert "run_body_replay.py" not in body_refine
    assert "r03_safe_axis_body256" not in body_refine
    corrected_path = 'export PYTHONPATH="$SOURCE:$R03E_RUNTIME:$R03E:$R03D:$PROJECT"'
    assert corrected_path in body_refine
    assert body_refine.index(corrected_path) < body_refine.index(
