...
def main() -> None:
    ...
    planner_terminal = read_json(run_root / "planner_terminal_report.json")
    if (
        planner_terminal.get("status") != "complete"
    ...
    deep_distribution_path = run_root / "planner_distribution_deep_audit.json"
    topology_planner = planner_terminal.get("topology_match_audit") or {}
    topology_planner_path = run_root / "planner_topology_match_audit.json"
    if (
        deep_distribution.get("path") != str(deep_distribution_path)
    ...
    if (
        ...
        != 800
        or refiner_contract.get("checkpoint_rehashed") is not True
    ):
    ...
    for index, cohort_id in enumerate(config["downstream_cohorts"]):
        ...
        cohort_report = read_json(
            run_root / "planner" / cohort_id / "frozen" / "cohort_report.json"
        )
        if (
