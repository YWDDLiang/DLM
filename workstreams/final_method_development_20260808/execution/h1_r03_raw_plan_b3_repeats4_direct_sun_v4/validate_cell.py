#!/usr/bin/env python3
"""Validate one four-cell Direct and completed-cache S.U.N. result."""
...
def main() -> None:
    ...
    if args.output_dir.resolve() != run_root / "cells" / cell / "evaluation":
        raise ValueError("cell evaluation output escaped the immutable run root")

    sun_contract = config["sun"]
    completion_manifest_path = run_root / sun_contract["completion_manifest"]
    completion_manifest = read_json(completion_manifest_path)
    completed_cache_spec = completion_manifest.get("completed_mp_hull_cache") or {}
    expected_cache_sha = str(completed_cache_spec.get("sha256", ""))
    completed_cache = run_root / sun_contract["completed_mp_hull_cache"]
    preflight_path = run_root / "status/preflight_report.json"
    preflight = read_json(preflight_path)
    gcd_evidence = preflight.get("gcd_before_comp_valid") or {}
    if (
        ...
        or completed_cache_spec.get("path")
        != sun_contract["completed_mp_hull_cache"]
        or int(completed_cache_spec.get("rows", -1))
        != int(sun_contract["wanted_chemsys_count"])
        or completed_cache_spec.get("all_rows_populated") is not True
        or sha256_file(completed_cache) != expected_cache_sha
        or preflight.get("status") != "pass"
        or preflight.get("source_manifest_sha256")
        != args.source_manifest_sha256
        or gcd_evidence.get("status") != "pass"
        or gcd_evidence.get("required_order") != "gcd_then_smact_validity"
    ):
        raise ValueError("GCD or completed-cache evaluation contract changed")
    ...
    source_cache_identity = (sun_summary.get("assets") or {}).get(
        "mp_hull_cache"
    ) or {}
    working_cache_identity = sun_summary.get("working_mp_hull_cache") or {}
    if (
    ...
    if (
        ...
        or source_cache_identity.get("sha256") != expected_cache_sha
        or working_cache_identity.get("sha256") != expected_cache_sha
    ...
    report = {
        ...
        "evaluation_contract": {
            ...
            "direct_runner_sha256": config["direct"]["gcd_before_comp_valid"][
                "runner_sha256"
            ],
            "direct_upstream_compute_metrics_sha256": config["direct"][
                "gcd_before_comp_valid"
            ]["upstream_compute_metrics_sha256"],
            "completed_mp_hull_cache_sha256": expected_cache_sha,
            "completed_mp_hull_cache_rows": int(
                completed_cache_spec["rows"]
            ),
            "wanted_chemsys_count": int(sun_contract["wanted_chemsys_count"]),
            "missing_chemsys_resolved": int(
                sun_contract["missing_chemsys_count"]
            ),
            "mp_api_used_during_slurm": False,
        },
        "artifacts": {
            ...
            "mp_cache_completion_manifest": _identity(
                completion_manifest_path
            ),
            "completed_mp_hull_cache": _identity(completed_cache),
