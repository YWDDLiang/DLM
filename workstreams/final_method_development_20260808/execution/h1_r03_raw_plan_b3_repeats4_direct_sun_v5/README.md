# R03 raw Plan × B3 repeated Direct/S.U.N. evaluation — V5

V4 failed closed before submitting Slurm work because its preflight required
the frozen historical R03G baseline rows to use the current candidate S.U.N.
schema. V5 validates the historical `no_completed_hull` schema and the current
candidate schema separately. Their paired metric fields and ordinal coverage
remain unchanged. Scientific inputs, model bytes, cache, repeats, seeds,
scorers, and statistics are unchanged.
