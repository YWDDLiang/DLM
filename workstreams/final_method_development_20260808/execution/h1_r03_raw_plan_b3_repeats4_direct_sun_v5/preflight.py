...
def main() -> None:
    ...
    runtime_copy_shas = {
        "run_body_safeaxis.py": "b7158eb4b24abb8abd9cbe8fc038a2c1dfdc0321fd4b70d2c15c681d857c3c59",
        "refine_cell.py": "0e18573219d198759f590aebce25377376ddc9d9a35b9306e7a3634ed8f16e6b",
        "finalize_generation.py": "9e80b5d4cadb0e3e21028dfde2316e9a9fcda16b16134a5b4e55b228706b76c3",
        "run_crysllmgen_a100_sun_completed_cache.py": "6a411205dd95e589a987fdfc9dcaa041f0ee714d46138c3218c1b5de31de1303",
        "validate_cell.py": "14983a14b6981ab2a7259c3735955314fcf74f3d885d3f3c8ba06dcd963d4b85",
    }
    runtime_copies: dict[str, Any] = {}
    for name, expected_sha in runtime_copy_shas.items():
        upstream = require_file(
            Path(runtime_spec["source_dir"]) / name,
            expected_sha,
            f"successful V4 runtime {name}",
        )
        copied = require_file(
            source / name, expected_sha, f"repeat-panel runtime copy {name}"
        )
        runtime_copies[name] = {
            "upstream": verified(upstream, expected_sha),
            "copy": verified(copied, expected_sha),
            "byte_identical": True,
        }
    ...
    for repeat, spec in enumerate(historical["attempt_results"]):
        ...
        if any(
            row.get("schema")
            != "crysllmgen_r5c_a100_sun_no_completed_hull_attempt_v1"
    ...
    report = {
        "schema": "h1_r03_raw_plan_b3_repeats4_preflight_v5",
        "status": "pass",
        "run_id": config["run_id"],
        "source_manifest_sha256": args.source_manifest_sha256,
        "reused_v4_runtime_manifest": identity(runtime_manifest),
        "byte_identical_v4_runtime_copies": runtime_copies,
