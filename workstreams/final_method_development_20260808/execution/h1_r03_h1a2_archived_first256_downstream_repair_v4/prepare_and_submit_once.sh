...
SOURCE="$ROOT/workstreams/final_method_development_20260808/execution/h1_r03_h1a2_archived_first256_downstream_repair_v4"
RUN="$ROOT/runs/20260812_h1_r03_h1a2_archived_first256_downstream_repair_v4"
V3_SOURCE="$ROOT/workstreams/final_method_development_20260808/execution/h1_r03_h1a2_archived_first256_downstream_repair_v3"
...
if [[ -e "$RUN" ]]; then
  echo "immutable downstream repair v4 run root already exists" >&2
  exit 2
fi
test "$(sha256sum "$V3_SOURCE/SOURCE_SHA256.txt" | awk '{print $1}')" = c84e6044978db5a8df96a71268246e15489605bb275c492bb8e9b7eaa9dc5f35
(cd "$V3_SOURCE" && sha256sum -c SOURCE_SHA256.txt >/dev/null)
test -f "$ROOT/runs/20260812_h1_r03_h1a2_archived_first256_downstream_repair_v3/status/PREPARATION_FAILURE"
test ! -e "$ROOT/runs/20260812_h1_r03_h1a2_archived_first256_downstream_repair_v3/submission_record.json"
...
python "$V3_SOURCE/preflight_downstream.py" \
...
job_id="$(cd "$ROOT" && sbatch --parsable \
  --export=ALL,ARCHIVED_DOWNSTREAM_V4_SOURCE_SHA256="$source_sha" \
...
record = {
    "schema": "h1_r03_h1a2_archived_first256_downstream_submission_v4",
    "status": "complete",
