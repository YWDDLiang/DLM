#!/usr/bin/env python3
"""Recompute only E_hull and S labels for one frozen evaluation cell."""
...
def main() -> None:
    ...
    if cache_manifest.get("query_status") != "complete_all_resolved":
        raise ContractError("official MP cache has unresolved queries")
    ...
    try:
        ...
        missing = sorted(required_chemsys - set(cache))
        if missing:
            raise ContractError(f"clean cache misses {len(missing)} required chemsys")
        diagrams = {
            chemsys: phase_diagram(cache[chemsys], chemsys)
            for chemsys in sorted(required_chemsys)
        }

        new_attempts: list[dict[str, Any]] = []
        hull_deltas: list[float] = []
        strict_01 = strict_10 = meta_01 = meta_10 = 0
        old_unknown = new_relax_unknown = 0
        newly_hull_resolved = 0
        evaluated = 0
        for old in old_attempts:
            attempt_id = str(old["attempt_id"])
            new = copy.deepcopy(old)
            new["schema"] = "crysllmgen_r5c_a100_sun_attempt_official_mp_clean_v1"
            metrics = new["metrics"]
        ...
        if evaluated + new_relax_unknown != novel_unique:
        ...
        report = {
            "schema": "h1_sun_official_gga_u_clean_cell_report_v1",
            "ok": True,
