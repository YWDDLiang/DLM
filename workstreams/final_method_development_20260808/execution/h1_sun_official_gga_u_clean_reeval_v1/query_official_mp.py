...
def main() -> None:
    ...
    try:
        ...
        if failures:
            write_json_exclusive(
                preparing / "query_failures.json",
                {
                    "schema": "h1_official_mp_gga_u_failures_v1",
                    "count": len(failures),
                    "failures": failures,
                },
            )
            raise ContractError(f"{len(failures)} official MP queries failed")
        ...
        manifest = {
            ...
            "query_status": "complete_all_resolved",
            ...
            "all_phase_diagrams_constructed": True,
            "all_elemental_references_present": True,
