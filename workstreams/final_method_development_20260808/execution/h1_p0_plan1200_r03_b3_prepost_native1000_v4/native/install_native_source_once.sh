...
if [[ "$#" -ne 1 ]] || [[ ! "$1" =~ ^[0-9a-f]{40}$ ]]; then
  echo "usage: install_native_source_once.sh SOURCE_GIT_COMMIT" >&2
  exit 2
fi
...
TMP="$RUN_ROOT/.native1000_source.preparing.$$"
PYTHON=/public/home/jiaosz/miniconda3/envs/diff_meets_diff/bin/python

cleanup() {
  rc=$?
  if [[ "$rc" -ne 0 ]] && [[ -d "$TMP" ]]; then
    mv "$TMP" "$RUN_ROOT/.native1000_source.FAILED.$$"
  fi
}
trap cleanup EXIT
...
PY
PYTHONPATH="$TMP:$BODY_SOURCE" "$PYTHON" "$TMP/self_test.py"
...
printf '%s\n' "$SOURCE_GIT_COMMIT" > "$RUN_ROOT/status/native1000_source_git_commit.txt"
mv "$TMP" "$SOURCE"
trap - EXIT
SOURCE_SHA="$(sha256sum "$SOURCE/SOURCE_SHA256.txt" | cut -d' ' -f1)"
printf '%s  native1000_source/SOURCE_SHA256.txt\n' "$SOURCE_SHA" \
  > "$RUN_ROOT/status/native1000_source_manifest.sha256"
echo "native1000_source_manifest_sha256=$SOURCE_SHA"
