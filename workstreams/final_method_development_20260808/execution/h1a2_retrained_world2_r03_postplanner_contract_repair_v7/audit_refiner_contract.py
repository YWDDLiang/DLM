...
def main() -> None:
    ...
    if (
        checkpoint != Path(refiner["checkpoint"]).resolve()
        or not checkpoint.is_file()
        or checkpoint.stat().st_size != int(refiner["checkpoint_expected_bytes"])
    ):
        raise ValueError("model_494 path or byte size changed")
    ...
    report = {
        ...
        "checkpoint_identity": {
            ...
            "sha256_registered": refiner["checkpoint_sha256"],
            "basis": "registered_sha256_plus_current_path_and_byte_size",
        },
