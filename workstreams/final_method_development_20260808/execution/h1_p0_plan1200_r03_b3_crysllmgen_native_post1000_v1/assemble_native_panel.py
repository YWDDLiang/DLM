...
def assemble(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if (
        config.get("schema")
        != "h1_p0_plan1200_r03_b3_crysllmgen_native_post1000_config_v1"
    ...
    report = {
        "schema": "h1_plan1200_crysllmgen_native_post1000_terminal_v1",
        "status": "complete",
        ...
        "statistical_contract": {
            "primary_all_attempt_inference": "retained_in_V3_terminal_report",
        ...
        "artifacts": {
            "main_V3_terminal": identity(run_root / "terminal_report.json"),
            "main_V3_markdown": identity(main_markdown),
...
def markdown(report: Mapping[str, Any]) -> str:
    lines = [
        ...
        "",
        "它不替代 V3：V3 固定 1,000 次 body attempt，并保留失败，适合无偏配对推断；native1000 是论文/上游基数对齐视图。由于 R03/B3 的 body failure 位置可能不同，native1000 的入选 plan 集合可能不同，因此跨臂差值只作描述；逐批精确 McNemar 与 50k hierarchical paired bootstrap 仍以 V3 为准。",
        "",
        "## 生成与完整 refine 审计",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "- 原 V3 的固定 all-attempt 结果和统计推断保留为主证据；本表只回答 CrysLLMGen-native 基数下的表现。",
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> None:
    ...
    try:
        ...
        marker = args.run_root.resolve() / "status/native1000_assembly_SUCCESS"
        ...
        failure = {
            "schema": "h1_plan1200_crysllmgen_native_post1000_failure_v1",
