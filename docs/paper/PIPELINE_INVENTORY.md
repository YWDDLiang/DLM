...
| Shared Plan contract | Serializes global chemical intent and Compact-V2 structural fields identically at train and inference | `src/crystal_dlm/c3fd_native_plan.py`, `src/crystal_dlm/composition_identity.py` | Plan output from Slurm111; paper wrapper pending | `tests/test_c3fd_native_plan.py`, `tests/test_materialize_c3fd_llama_prospective_conditions.py` | Plan SHA `5f1ae510fb35d7bbe0b5da4b32b0302f49d78dae653c5c31493db8a2219a54cb` |
...
| Fixed terminal diffusion | Refines the raw structure with model494 while preserving attempt identity and atom multiset | `src/scripts/refine_dlm_with_crysllmgen.py`, `scripts/assemble_grounding_repeat.py` | headline Slurm126; full-epoch Slurm131; model494 tau800 | `tests/test_slurm121_g2_model494_refine.py`, `tests/test_ctv_refiner_seed.py` | prospective BASE/G2 refined Direct `251/251`; job39172 A/B `254/254` |
| Physical evaluation | Joins Direct validity, N/U/NU, CHGNet and official hull on the fixed requested denominator | `eval_runtime/run_full_reconstructed_eval.py`, `eval_runtime/finalize_official.py`, paper finalizers under `scripts/finalize_*` | headline Slurm127; full-epoch Slurm131 plus CPU finalizers | `tests/test_slurm127_g2_final_offline_eval.py`, `tests/test_full_reconstructed_eval_helpers.py`, finalizer/protocol tests | fresh prospective `docs/36H_FINAL_REPORT_C3FD_G2_20260901.*`; full-epoch `docs/G2_FULL_EPOCH_AB_FINAL_20260901.*` |
...
- Metrics: fixed requested denominator256; no survivor filtering or result selection.

The earlier fresh prospective G2 endpoint remains the headline system result:
refined Strict/Meta S.U.N. `24/117` and paired official hull `-16.43
meV/atom`. Full-epoch A is the promoted implementation and mechanism evidence:
raw Direct `128/256` versus BASE `118/256`.

## Evidence boundaries retained by the paper branch

- The headline `24/117` endpoint uses the registered step348 G2 from Slurm116;
...
- model494 and the Direct implementation are frozen inherited transitions with
  pinned hashes. Their role is integrated into the coarse-to-fine method, while
  learned raw-DLM and complete-system evidence remain separately measurable.

## Evidence hashes

- C3FD checkpoint: `87c1673f709c14488848196e3f466b0f4797cc3511108a07b6d498ca96d17d53`.
- Llama typed config/state/adapter: `ca786e3d...ff89a`, `9912e259...871bc`,
  `7638b05d...84c75`.
- DLM teacher-data manifest/train/val: `b77a1d01...ae06`,
  `f61cd177...6068`, `bea7d8e6...09e1`.
- model494 checkpoint: `573e9b10...11e7e`; Direct evaluator:
  `556d4e14...e7a4`.
- Prospective official source/final JSON: `138e547f...b6a6`,
  `1b99aa33...b3070`.
- Full-epoch evaluation/final JSON: `6ca71897...aa00`, `b50dd8d2...19881`.
