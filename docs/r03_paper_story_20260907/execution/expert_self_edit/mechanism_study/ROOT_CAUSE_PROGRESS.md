...
当前编辑器尚未超过项目已有的 K8 强参照。已经确认一个内容监督位置偏差并实现修正；小样本结果显示 CE 改善，但物理收益证据不足。四轮自主编辑暴露了稳定化瓶颈：S 提案没有被提交，而被拒提案大多确实不够好。F800 和 R 的数值重复性问题已经在单列的小型确定性对照中修复。等价目标表示的训练对照仍在运行。
...
两臂实际训练时间2494.535和2474.060秒；累计保守训练时长记账8647.387秒，低于16小时上限。

## 在运行的受控工作

- 64来源的等价表示 A/B：相同 fresh B0、400更新、next-token/M2T1，控制臂原始目标，试验臂仅S内容目标做整体等价对齐；共同评价仍用原目标，完整提案评价两臂都用确定性 R。CE 是依赖代表选择的次要量，最终 checkpoint 固定为400，不据此早停或选更好 checkpoint。
