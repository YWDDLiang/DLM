...
记录日期：2026-09-08。本轮预定作业已完成，GPU资源已释放。原 MAIN 结果保持冻结；本目录实验均为后续探索和开发集诊断，不能冒充新的独立 MAIN 检验。

## 当前结论

当前编辑器尚未超过项目已有的 K8 强参照。已经确认并修正内容监督位置偏差，修复了固定对照中的F800/R数值重复性。8来源训练能够降至很低CE，并在训练来源自由生成中产生大部分正确物理修改；同一模型在未见开发来源上仍大量丢失原有可靠性，接受头也未筛掉恶化提案。因此尚未达到“小数据可靠泛化后扩量”的条件。本轮没有继续扩大数据或重跑MAIN。

![极小集合拟合与开发物理结果](ROOT_CAUSE_RESULTS.png)

上图由 `FIT8_ANALYSIS.json`、`GAUGE64_REPEATS_ANALYSIS.json`、`T2T64_ANALYSIS.json` 和 `TEACHER64_ANALYSIS.json` 生成，提供SVG/PDF版本及 `ROOT_CAUSE_PLOT_DATA.json`。不同checkpoint的柱图为同一开发来源上的描述性比较；不能把其差异全部归因于来源数。R相关量不是正式SUN/MSUN。
...
冻结原stage2 step800的原训练源码明确包含70% M2T和30%可见OLD的T2T视图，部署原来始终遮掉所有active位置。新增opt-in `proposal_input=old_values`，保留尚未改写位置的OLD值，后续位置仍读取之前实际采样的值；门槛、范围、采样顺序和预算不变。9个实际采样/调度测试通过。对照使用原checkpoint，因此两种输入均属于其训练过的视图族，不能把M2T本身称为实现错误。

四个固定配对生成种子复用相同32G+32S开发来源，全部采用共同确定性R；OLD与先前共同输入逐条确认body和结构相同。masked与old_values的几何支持为202→194/256，R verified 66→61/256，S双方verified且降能≥0.01为30→29/128。对应来源聚类95%差值区间为[-7.81,1.56]、[-7.42,3.13]、[-8.59,7.03]个百分点，均跨零。这个改动没有证实物理收益，保留为显式实验选项，默认仍为masked。

自主G/S从同一64个原始状态、同一seed开始。原始几何支持25、R verified11；masked为42/14，old_values为40/14。两臂提交G分别33/35次，S仍均为0次，forward总数均为2550。原始OLD到最终提交结构的可验证降能和恶化均为0；未改变结构的R标签没有不一致。数据见 `T2T64_ANALYSIS.json`、`T2T64_COMPACT.json`。

## 确定性重新认证与极小集合拟合

先对前述固定train/dev各64个来源的全部OLD和teacher共256个输入，启动新标签作业，禁用跨作业标签复用，再单独编译新版本。原prepared输入、原标签和原训练数据均保持冻结。256/256的状态、verified、实际步数和终态能量与此前分别计算的OLD128+teacher128完全一致；203个有有限能量输出，另53个两次均原始几何无效。它把R重复性证据扩展到这128个固定来源，仍不等于验证了物理模型的全局准确性。见 `RECERTIFY128_REPEAT_CHECK.json`、`TEACHER64_COMMON_OLD_INPUT_MATCH.json`。

新编译数据包含128个独立来源、317条记录，G原始几何恢复53、G终态可靠性恢复10、G松弛几何恢复2、S正例61、S反向负例61、S负例1、中性1、状态128；无工程错误排除。S正例为train30、dev31，训练正例不再包含已证实为负收益的原source2101。新标签SHA与compiled文件的可恢复原字节见 `RECERTIFY128_COMPACT.json`。

极小集合诊断按现有固定字典序选择器取新数据前4个G和前4个S训练来源。相同fresh B0、next-token/M2T1、batch24、1600更新，使用6GPU，固定最终checkpoint。训练集做两个生成种子，原先共同开发面板做四个种子；训练记忆能力和开发泛化分别报告。它用于检查训练覆盖与自由生成，不能把8与旧64之间的差值全部归因于来源数，因为标签已重新认证，GPU分组也改变。配置为 `RECERTIFY128_MANIFEST.json`、`FIT8_MANIFEST.json`。

训练已固定完成1600次更新，实际训练1300.985秒，累计保守梯度训练时长11264.306秒（3.13小时），低于16小时上限。最终训练CE为G0.057740/S0.048060，开发为G4.722849/S5.107486。当前设置能充分拟合这组训练目标；没有出现开发泛化的相应改善。4个G/4个S分别包含189/87个可编辑目标位置，每个位置平均直接监督92.15/85.48次，全部覆盖；完整回放见 `FIT8_CONTENT_COVERAGE.json`。

两个训练生成种子复用同一8个来源。G共8次提案有7次恢复几何支持；S共8次提案有7次双方verified且降能至少0.01 eV/atom，分种子为4/4和3/4。所有16次提案均被接受头提交，其中包括一个原始几何失败的G和一个R未verified的S。G/S可编辑token命中原目标的比例分别为90.21%/96.55%，完整body精确匹配为0/8和3/8；物理成功并不要求精确复现原目标token。三个精确匹配的教师终态标签均与独立教师重评一致。

四个开发生成种子全部完成，复用原共同32G+32S来源；输入body、结构、来源、划分逐条匹配共同OLD，所有272个train/dev提案均有完整确定性标签，无worker错误或超时。完整记录及每个物理输入的可恢复UTF8字节和哈希见 `FIT8_COMPACT.json`。

| 最终checkpoint的开发完整提案 | G：32来源×4种子 | S：32来源×4种子 |
|---|---:|---:|
| 输入几何支持 | 87/128 | 113/128 |
| R verified | 15/128 | 50/128 |
| 达到原G恢复合同 | 71/128 | 不适用 |
| 双方verified且降能≥0.01 | 不适用 | 28/128 |
| 双方verified且恶化≥0.01 | 不适用 | 17/128 |
| 被最终接受头提交 | 123/128 | 127/128 |

S降能分种子为7、7、9、5；同一32个OLD有31个R verified，重复到四种子为124/128。提案只有50/128 verified，原先verified但修改后失去verified为75次，新获verified为1次。相对OLD的verified率变化为-57.81个百分点，按来源聚类95%区间[-67.19,-47.66]。这个损失不能通过只统计28个成功降能的提案来掩盖。全部28个改善和17个恶化提案都被接受头提交，未显示可靠的收益筛选能力。

这些HOW检查强制生成full-cell内容，绕过任务admission与范围选择；表中的“提交”指最终接受头对这些完整提案的判断，不能冒充一轮自主G/S的端到端结果。本轮不再为该模型增加自主或MAIN评估。充分监督后的训练拟合成功与开发泛化失败已得到分别验证。

## 本轮保留的因素与后续方向

可以保留的实证因素是：匹配实际执行位置的next-token监督、按实际目标位置核算训练覆盖、确定性物理运行与重新认证、以真实OLD→提案收益检查自由生成。等价代表和可见OLD输入仍只是显式实验选项，现有四种子结果不支持把它们宣传为稳定性修复。

剩余问题已具体到内容和接受判断：未见结构上生成的整段修改经常丢失原有可靠性，接受头又缺少有效筛选；G的几何恢复目标还不保证能提供S所需的可靠入口。现有CE不区分几何错误严重程度，布尔收益头也不表达收益大小，这些是目标定义的限制，但尚未完成对应新目标的因果实验。下一轮应先围绕真实学生提案的几何严重度、相对收益和G→S合同设计小型受控修正；不能把这些待测方向写成已经有效，更不应直接扩量掩盖当前缺口。

## 解释边界

- 不能将256/1600直接与旧64/400的差异全部归于数据量；400与1600的学习率计划也不同。已完成的数据因素对照以两臂共同1600更新为准。
- G的几何恢复合同不保证R verified；实际32个G教师虽全部通过输入几何支持，train/dev只有14/11个R verified。S的入口却要求可靠的OLD。这说明几何恢复与稳定化之间存在合同落差；它是已有目标定义的直接后果，尚不能据此声称某个新训练合同已经改善结果。
