...
修复后的G1已完成8步、4次遍历、752次全局样本访问，训练127.46秒，峰值显存42.40GB，末步记录KL为0.00339。同Plan/种子S1生成223成功、33失败，相比S0净少6个成功样本（14条成功转失败、8条失败转成功），原27个F后SUN请求在新G阶段全部成功。S1 raw SUN5、严格稳定5、独占亚稳态26；S0对应4／4／32。完整G→F验收尚未完成，不能把退化幅度收敛写成质量提升。S1首次计分因新轮次预期缓存目录未建立而失败，已复制并逐文件校验原完整缓存后只重试计分，未重复训练或生成。
