...
创建日期：2026-09-07。更新日期：2026-09-10（北京时间）。状态：按§0.15已完成旧S1及新三级恢复S0、S1、S2的完整流程，正在执行第三次更新。本次仅TRAIN，12小时、4—6张A800，固定基座初始化，训练侧允许CHGNet指导；最新质量顺序为SUN＞严格Stable＞Meta Stable＞普通更稳定＞不稳定。轨迹按可复用数据集保留；GPU开始时间与绝对截止时间以正式运行回执为准。§0.11／§0.12为此前讨论与诊断，与最新计划冲突时以§0.15及未被覆盖的§0.14／§0.13为准。
...
用户明确纠正了上一轮在G→F处提前终止的执行方式：必须走完 **draft→refine→token→Edit/KEEP** 才判断完整输出。旧修复轮S1正在补做E1训练与最终Edit/KEEP；§0.14中“E1未执行、当前作业为空”是此前停止时的历史状态，不再是当前状态。新的三级恢复协议将从同Plan/种子的S0重新建立对照，完整执行更新；G+F指标只作诊断，不再单独阻止后续E。仍沿用原12小时绝对截止时间、最多6张A800及3个并发作业，不重置预算。
