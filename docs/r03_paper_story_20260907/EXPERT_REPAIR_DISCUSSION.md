...
创建日期：2026-09-07。更新日期：2026-09-10（北京时间）。最新状态：1000运行已于12:57:35完成S1并按用户要求暂停，G1/E1真实更新和S0/S1结果均已归档；G2/E2/G3/E3未运行。E1最终116 Stable/102 SUN/477 MSUN，与同输入KEEP相同，未取得增量。后续集中KEEP/EDIT训练与分析，使用已有1000及旧256的draft/proposal/反馈，以未参与E训练的来源上SUN和MSUN提高为采用目标。旧256同源前缀与起始E权重的既有训练来源全部留在训练侧，开发及最终评估按祖先/约化组成隔离。256请求的S0—S3及后续固定G/F试验均已结束，后者未通过条件，详见§0.17。
...
## 0.18 专注KEEP/EDIT：连续主状态和有符号收益头（2026-09-10）

用户在S1结束后明确停止1000后三轮，要求集中分析和训练KEEP/EDIT，旧256及现有draft/proposal均可使用，目标是在E更新没有见过的来源上提高SUN、MSUN。随后提出KEEP保留连续状态、EDIT才转token。已先登记[专注计划](KEEP_EDIT_FOCUS_PLAN_20260910.md)，在独立目录执行；原截止北京时间18:01:15不重置，最多5张可用A800、3个排队或运行作业。

连续状态来自已经保存的F结果，无需再次运行F。代码把连续结构作为返回值，token作为操作视图；KEEP及未改数值逐项保留，EDIT只写回改变的数值字段，并对实际混合结构执行几何检查，失败回退KEEP。7项测试覆盖原值保留、局部/晶格修改、周期映射、无匹配视图和几何回退。1000条中974条采用连续F，24条保留原直通或上游失败状态，2条因连续几何检查失败沿用原token回退；分母不变。

E来源划分已冻结为506 TRAIN、247 DEV、247 FINAL。旧256祖先、同组成及所有跨轮变体都在TRAIN，旧E3的三个训练文件和检查点回执重新核验。第一阶段冻结内容和范围，仅训练质量头；监督为新颖Stable和新颖MS的有符号指标差，中性为0，物理未知不充当负例，历史U不搬入新目标。按来源归一加权，防止旧256轨迹多而支配训练。实际数据5664条、492来源，目标−2/−1/0/+1/+2分别为87/802/4549/215/11；其中新颖Stable晋升仅34条、22个来源，7个是新增TRAIN来源。

新头已完成32个完整数据遍历、736步，每条覆盖32次，保留4/8/16/32遍快照。缓存特征可精确复现原头，所有内容/范围参数保持原哈希。最后一遍在TRAIN上把226个正收益中的148个判为正、889个负收益中的8个误判为正；这是训练拟合，不能代替物理泛化。源码、来源和训练覆盖见[回执](receipts/KEEP_EDIT_FOCUS_REGISTRATION_AND_TRAINING_20260910.json)。

连续KEEP与token KEEP的已开放比较为：TRAIN的Stable/SUN/MSUN从57/51/241到57/51/245，DEV从28/24/115到29/24/115。当前证据支持减少表示损失的动机，尚不能声称提高SUN。实际E提案换成混合结构后必须重新标注，897条已进入该评价，不能移用旧token提案标签。

另发现原解码中的判断和固定完整提案批次之间，旧头概率最高相差约0.0088。按原shard、64条组、各来源停止步数重建混合批次，5例中4例完全复现、1例差约6e-8；固定16条批次5例完全复现。后续学习头与旧头使用同一判断输入/批处理，同时保留旧记录决策对照，避免把这项执行差异混作训练贡献。详见[批次审计](receipts/KEEP_EDIT_JUDGEMENT_BATCH_AUDIT_20260910.json)。

第一阶段16个纯收益策略全部未通过DEV：连续KEEP为29 Stable/24 SUN/115 MSUN，原E3相同批处理为28/25/103；按预登记规则选出的失败候选（第32遍、阈值0.05）为28/25/115。随后按[双头方案](KEEP_EDIT_ACCEPTANCE_CONSENSUS_PLAN_20260910.md)检查旧E3接受与新收益判断的交集，16个组合也全部未通过，被选失败候选为29/25/113。FINAL仍封存。该交集在TRAIN较好而DEV更差，不能将旧头当作未见来源上的可靠安全判断。

完整提案的供给诊断为：TRAIN有5个真正Stable晋升和7个SUN新增，DEV没有Stable晋升，只有962一个SUN新增。962为Rb2LiTlBr6，两端原本都Stable，SUN变化来自输入N。前后共同弛豫终态按同一冻结匹配器相同，hull约从−0.008637到−0.002941 eV/atom，未提高稳定性。因此原合同SUN计数的提高不能自动写成新稳定晶体发现。

第三阶段在上述失败后按[匹配监督方案](KEEP_EDIT_MATCHED_READOUT_PLAN_20260910.md)真正新增训练：把当前连续提案中467条可靠TRAIN配对加入历史5664条，组权重各0.5，DEV/FINAL不参加监督。冻结quality隐藏层，只更新末层接受输出行129参数，完成64遍、1536步；每条训练行覆盖64次，标准化折回原模型核验通过。其16个DEV策略正在评分，仍须双指标通过才冻结、打开FINAL并执行已登记的额外随机流。完整获得/损失、输入N与弛豫终态解释、工程失败和实际训练见[结果报告](KEEP_EDIT_FOCUS_RESULTS_20260910.md)及[回执](receipts/KEEP_EDIT_DEV_FAILURES_AND_MATCHED_READOUT_20260910.json)。

### 0.18 收尾及最新用户澄清

本次E专注工作现已完成。正式末层模型三流的FINAL SUN/MSUN增量为+1/+3、0/−1、+1/+2，未证明稳定提升。来源字段约化检查发现Mg2Pb2与Mg4Pb4跨组，原247结果保留，另报告剔除300后的246条严格组成隔离子集，增量不变。候选一次构造后按模型选择，实际模型分数、1000个输出和原流解码均已核验；派生xyz末位差异和失败开销保留。

用户随后明确：减少合法有益编辑的误拒，所有尚非SUN的状态都应在稳定性或新颖性上继续改善，失败/未收敛也要尝试。已追加TRAIN操作性收益训练（6156条、64遍、1600步、129参数），开放新流的999个合法视图，四候选池覆盖896个非SUN可编辑输入；只有无有效结构的837无法编辑，最高24次DLM调用仍低于80。

严格246子集基线SUN28/MSUN121，操作性单候选为29/123，两候选A为28/124，B为31/124，四候选为29/127。B包含493、974两个真正Stable晋升和761的N改善；四候选却误排掉493、974。该阶段复用了已经查看的来源，明确为探索性验证；B也没有在DEV同时提高双指标，不能以最好池宣布稳定成功。下一步需要SUN优先的排序监督与更多真实SUN内容正例，已准备20对TRAIN比较，其中只有4对区分新颖Stable，尚未训练新排序头。

全部37个登记任务已结束，分配3.1406 GPU小时，含5次失败。详细各组结果、得到/失去的样本、物理与N贡献、已知局限及模型/数据回执见[完整结果](KEEP_EDIT_FOCUS_RESULTS_20260910.md)和[最终回执](receipts/KEEP_EDIT_FOCUS_FINAL_20260910.json)。原1000主流程仍停在S1。

## 0.17 新增12小时：先尝试固定G/F的E训练，再决定1000方案（最新授权）

北京时间2026-09-10约03:24，用户在确认1000 TRAIN、S0＋三次G/E更新后，进一步标注§0.15.1建议并要求：“再给你12小时，把这个做个尝试，work了再1000条吧，不然就按照我们这次的来采1000条”。因此暂停1000正式启动，先执行固定G/F、补足E数据编译和T2T训练、以同输入KEEP为基线、区分N/U的有界尝试。有效后采用试验改进；不满足预登记条件则沿用已完成256实验的方法。用户随后再次明确：后续仍执行1000条S0＋三次G/E更新，不改为一次固定权重推理。

§0.16已经完成的局部修正保留。53项不同CPU检查已通过，包含真实AdamW状态下的内容逐位冻结、接受监督与mode标签分离、G保持KL停止，以及两进程中仅一个rank超出KL时的统一冻结与无死锁续训。首次回归的两处问题为旧测试fixture缺少新要求的requests字段，以及测试模块入口名称写错；修正后复测通过，不隐去首次失败日志。不可变科学源码为`55025f609f39a259eb9e3b051713e215ae1e40c4`，当前源清单哈希为`db6001c85a3d9c56b7491efa74b701e8556eb443348d3cff70ca3ccf35f69418`。

1000面板于19:22:39 UTC登记，Plan SHA256为`26318e6851ecc4976913641682dfc9da4ca8a6283afd84d2c9b5471eb7a4b971`，parents哈希为`423ec6dc83a5250ad4f82c863ae562b2c03e62dfe44dbc32f1066f43dff27d4b`。它保留原256完整前缀，从5119个合格独立TRAIN祖先中选择，排除1159个MAIN约化组成。登记当时`run_20260909T191900Z_fixed1000`只有CPU预检、代码快照、Plan与配置，未提交GPU或开始BUDGET计时。试验判定失败并冻结旧方法后，该运行已于22:01:15 UTC正式启动；最新进展及完整分母见[1000条逐轮报告](CLEAN_1000_RESULTS_20260910.md)。

接下来单独登记E尝试的固定G/F输入、训练/开发/验证来源划分、可信目标来源、T2T与头训练目标、曝光上限和有效性门槛。验证来源只保证不参与本次E更新，不能冒充从未出现在旧RSI中的全新测试集。主要比较同输入KEEP及旧E3；模型选择不得使用最终验证来源。多候选仅用于训练及N/U诊断，正式单次E输出继续由模型自身决策，不使用在线物理择优。具体登记与实际执行回执追加于本节。

真实历史教师CPU通路已在隔离的256探针中通过：固定旧S3 current，历史来源仅S0主路/第二RAW及S1/S2已经完成的current/edited。256个输出均得到token目标，其中15个成功采用同Plan的历史Stable目标，241个仍走原CHGNet终点量化。历史15个目标均通过元素槽位与严格几何支持检查；该探针未调用GPU、未产生新物理标签，也未用于正式训练。输入哈希`1aef647464fc6570397c4c52c7e04f53ae08d649fa5e112ed7338f5520937db3`，源哈希和协议绑定见新尝试根目录的`preflight/HISTORICAL_TEACHER_PROBE.json`。

用户随后强调：继续从KEEP/EDIT入手，争取更高SUN和MSUN，并详细分析指标提升来自哪里。为此正式尝试增加同数据、同顺序、同头学习率的“内容冻结、只训练决策头”对照。它与T2T＋头之间的差异用于评估内容更新的作用；历史数据扩充、标签归一化等同时改变的因素不分别作因果归因。主要指标拆解为Stable/MS保留、获得、损失，提案的有益/有害与接受/拒绝交叉表，以及N/U输入和共同弛豫终点诊断。

正式登记由`prepare_editor_t2t_trial.py`写入不可变`PREREGISTRATION.json`：按reduced_formula分组、固定哈希排序划分60%训练/20%开发/20%最终验证。所有新更新只使用训练来源。已完成S0至S3的同Plan实际token结构、已经共同物理协议评分的提案和教师可组成目标池；不直接拟合未经复评的新量化目标，不迁移历史U标签。每个current最多保留2个正候选及4个负候选、一个内容目标和一个mode标签；Stable current有KEEP与恒等内容锚点。

T2T与全mask M2T交替8个完整遍历，每个内容视图监督编辑范围内全部数值token，旧结构始终可见。内容参数统一学习率`2e-6`，四个决策头`1e-4`；头特征detach，头损失不能更改内容参数。内容梯度累积一个完整数据遍历才执行一次更新；KL超过`.02`或时间不足则丢弃该遍历尚未应用的梯度，头可继续在原8遍历上限内更新。KL用typed periodic分布衡量，T2T损失是稠密typed CE代理；运行时顺序几何硬支持保持原规则，不能将代理称作完整几何条件似然。首个模型若不足8个完整内容更新，只允许再试一次`2e-7`内容学习率，其他配置不变。

开发集只选择预登记的六个决策组合：是否尊重learned KEEP，以及接受阈值`.5/.65/.8`。最终权重和决策组合固定后才读取最终验证指标。有效性要求：全部Stable提升内容目标实际参与8次完整内容更新（其中4次T2T）；最终验证Stable数严格超过KEEP和旧E3，SUN/MSUN都不低于两者；Stable损失不超过旧E3且最多1条，已知物理失败不超过较差对照。若不满足则按用户要求回退到已完成旧方法做1000条S0＋三次G/E更新。四个固定E随机流用于同Plan的N/U诊断，不能物理择优后冒充单输出主指标。

**后续用户范围调整（原登记保留）：** 用户指出学习率过低未必有益，要求检查KL是否构成瓶颈；随后要求成功和失败都详细论证、本次后不能大改、1000条用干净数据训练、先写计划再执行并更新main.md，且主判断只关注Stable和SUN。新的执行顺序见`EDITOR_T2T_TRIAL_PLAN_20260910.md`；独立范围补充不覆盖原始登记。MSUN不再是强制门槛，多候选N/U扩展不再作为必做任务。任何KL校正先测量全量训练目标的KL与CE、写明证据和事前补充，再在原12小时预算内验证，最终验证继续封存。

当前真实训练数据：153个TRAIN来源、51个开发来源、52个最终验证来源；670个不同current，3294条候选判断，632个内容目标（Stable提升52、Stable恒等85、MS恒等154、其它已验证修复341）。首个内容学习率`2e-6`配置在第2次完整内容更新后停止，头完成552个小批次更新；首个触发批次是第三遍历第一批，最大卡KL`.04827698`，该卡只有2个内容样本，且其它几张卡也超过`.02`。因此尚不能认定只是单个离群点，需要全量分布及修复/保护组损失变化来判断。

2026-09-10北京时间约05:00追加：全量KL诊断已完成，原学习率模型T2T均值0.023775、M2T均值0.004118；Stable提升/保护目标的T2T CE平均下降0.117865/0.110622，低十倍学习率对应0.047776/0.045095。不能将KL越界直接等同于物理退化，也不能将CE下降当成Stable提升。按事前补充仅再运行一次保留原学习率和软KL、关闭硬停的对照；四个固定候选共24个DEV策略比较，冻结后FINAL只决定采用或回退，不再增加训练配置。实际规则和完整证据以[执行计划](EDITOR_T2T_TRIAL_PLAN_20260910.md)及[试验结果](EDITOR_T2T_TRIAL_RESULTS_20260910.md)为准；本段前面的八遍历/MSUN/K4规则保留为原始登记历史，已被独立补充限定。1000条来源与新初始化检查已通过，等待试验方法冻结。

**本节最终结果：未通过，回退旧方法。** 四组训练、五组提案物理比较、24个DEV策略与FINAL判定均已完成。各新模型最高接受概率都低于0.5，24个组合全部KEEP；所选heads/尊重KEEP/0.8只是并列规则结果。全256条为Stable32/SUN28，原E3和匹配E3均为33/32；FINAL52条均为5/5，没有新增Stable。旧匹配提案全是单原子局部XYZ，新候选有246—249个整胞提案；仅训练头也发生此变化，内容KL不是唯一原因。关闭硬停虽完成8次内容更新，FINAL仍无Stable晋升提案，降低接受阈值不能补出不存在的收益。完整范围/接受/提案供给归因、旧3得2失案例以及仅改变运行绑定的小bug回执见[试验结果](EDITOR_T2T_TRIAL_RESULTS_20260910.md)。后续按[执行计划](EDITOR_T2T_TRIAL_PLAN_20260910.md)用干净TRAIN和新B0/E初始化跑1000条S0＋三次G/E更新，保留旧方法并停止增加训练设置。

1000条运行进展：两路RAW均完成生成、共同物理标注与评分，主路Stable/SUN为25/25，对照19/19，分母各1000，当前继续F800。扩容准备遗漏了参考缓存覆盖，两个评分作业因缺失体系退出；按事前修复计划保留失败回执与原数据，使用同一2026.04.13参考版本扩展至925个体系，旧条目和输入/标签/配置字节不变。37项相关CPU检查及真实覆盖核验通过，仅重试评分成功；科学执行保持657985c，原18:01截止不变。该工程缺口不作为模型质量结果。后续逐轮收益和损失见[1000条结果](CLEAN_1000_RESULTS_20260910.md)，详情见[缓存恢复回执](receipts/CLEAN_1000_HULL_COVERAGE_RECOVERY_20260910.json)。

2026-09-10北京时间约08:45追加：主路F800、token与current评分已完整完成，为Stable122/SUN105，对照仍在精修，S0的E0已按B0与种子20260909新初始化并运行。F相对RAW新增97 Stable和80 SUN，原有25条均保留；Stable新增分别来自41条原明确失败、9条原未知、47条原可靠非Stable。全部SUN新增都同时发生Stable晋升，17条Stable因N为假未成为SUN，其中1条也未满足U。已知SUN的保留来自固定F分流，不能提前归因于E。完整998条可生成结构都通过终点验证，32条缺少官方参考，与2条生成失败一起留在34条未知分母。前256条新RAW与F/token的输出、种子、分流及回退逐条保持旧S0结果；没有把旧标签导入训练。绑定输入/评分/程序哈希的逐条分析和具体hull案例见[1000条结果](CLEAN_1000_RESULTS_20260910.md)及[F贡献回执](receipts/CLEAN_1000_S0_RAW_TO_TOKEN_ANALYSIS_20260910.json)。后续仍执行完整S0与三次G/E更新，不根据本中间结果改方法。

2026-09-10北京时间约09:16追加：S0主路五阶段全部完成并通过完整1000分母、输入/评分哈希、源码、E0检查点和最终决策绑定核验。E0最终Stable115/SUN106，相对current122/105是1得8失、Stable净−7，SUN1得0失。八条Stable损失均来自17条Stable但非SUN输入，105条SUN全部保留；固定SUN保护没有覆盖所有Stable。唯一晋升365为4站点XYZ、接受概率0.510412，hull由+0.053461降至−0.012032；8条损失概率0.502581—0.604699，含5次整胞、3次4站点XYZ，不能只归因于某一范围或用单阈值完美分离。三条维持Stable并满足SUN的提案761/792/795被拒绝，是N改善机会而非新Stable。最终50条明确物理失败、7条未收敛、32条官方参考缺失、2条原始生成失败均留完整分母。E0优化器更新数为0，基线损失不是KL训练停止造成。详见[完整回执](receipts/CLEAN_1000_S0_COMPLETED_MAIN_20260910.json)和[逐条诊断](receipts/CLEAN_1000_S0_EDITOR_DIAGNOSIS_20260910.json)；对照正在完成token评价，随后继续三次真实G/E更新，不因S0局部结果改方法。

2026-09-10北京时间约09:45追加：S0与对照均已完整完成并归档，对照F/current为122 Stable/106 SUN。首次G偏好数据由1000个Plan比较得到822个有效TRAIN来源，494条偏好监督与555条健康锚点监督可重叠；全部证据来自本次两次RAW及对应F/token结果。G1按原6卡、每卡batch32完成20个优化器步，822条记录各实际内容更新4次，六个rank均未触发KL停止，参数变化平方和0.007043263903且检查点验证通过。S1新1000条生成已开始，六次权重更新目前完成G1一次；实际覆盖与输入/数据/检查点哈希见[G1回执](receipts/CLEAN_1000_G1_WEIGHT_UPDATE_20260910.json)。本次G训练覆盖充分，S1质量仍要以完整生成/F/编辑评价判断，不用训练完成替代效果证据。

2026-09-10北京时间约11:10追加：S1生成999条可用结构、1条生成失败，RAW完整评分23 Stable/23 SUN，相对S0 RAW3得5失。一次N/U工作进程初始化超时已保留原失败，并以完全相同命令重试通过，输入、标签、模型和评价规则未改动。随后node99有3卡被长期作业占用，6卡F请求被排到数天后；按事前资源计划只取消尚未运行的41831，5卡F800的41861已开始。余下推理5卡、并行3+2，训练4×48保持全局源批量192；原6×32的G1回执保持不变。时间预留按已测1000条速度校正为未来完整轮140分钟、当前E后处理35分钟、收尾15分钟，仍由原12小时截止、单次1800秒和G4/E8上限约束。33项调度检查及远端实际配置校验通过，科学源码仍657985c。GPU重新分片改变rank样本划分，实际训练覆盖与停止原因必须单独记录。详见[当前容量回执](receipts/CLEAN_1000_CAPACITY_FIVE_GPUS_20260910.json)及[评分恢复](receipts/CLEAN_1000_S1_SCORE_STARTUP_RECOVERY_20260910.json)。

2026-09-10北京时间约11:55追加：S1的F/token与current已完成，为116 Stable/102 SUN。相对本轮RAW新增93 Stable、79 SUN，均无损失；相对S0同Plan F则为Stable14得20失、净−6，SUN15得18失、净−3。跨轮34条Stable变化的两端都可靠，不能归因于未知标签补齐；SUN另外包含136仅N变化获得、607仅N变化损失。G1已完成四遍并未触发KL停止，但本次G/F整体仍回退，训练覆盖不等于质量保证。365在S1的F阶段已经晋升Stable/SUN，后续不得再次记作E1贡献。E1正用本轮current、旧E0提案及本轮物理教师端点采集干净反馈，完整E1及后续两轮仍按既定方法继续。逐样本获得/损失、hull和哈希见[S1 F诊断](receipts/CLEAN_1000_S1_F_DIAGNOSTIC_20260910.json)。

2026-09-10北京时间约12:35追加（最新用户范围）：用户认为Edit效果尚不突出，要求至少多出一些SUN和meta SUN，完成本轮后先停止1000，再研究改进。已按此登记只完成当前S1，包括E1训练、正式最终评价及归档；新调度2941d5c通过37项检查，范围上限阻止G2，并在S1归档后写入用户暂停标记。E1数据1758条、965个TRAIN来源，其中4个Stable晋升目标；原允许的30分钟训练上限并未耗尽，E1在首步后因第二批次四rank KL均高于0.75而停止，4个晋升目标实际只覆盖1个，60个Stable损伤拒绝目标只覆盖7个。该结果首先暴露了更新幅度与监督覆盖瓶颈，不能以一次参数变化证明已学会编辑。原方法头损失未detach共享内容特征，KL停止的上游原因需结合参数组与共享梯度分析；此前试验中直接关闭硬KL停止也未产生最终Stable收益，因此后续还必须同时解决有效提案供给和接受判断。

重新汇总原完整评分，S0的F/current为122 Stable/105 SUN/482 MSUN，E0最终115/106/439；SUN净增1伴随MSUN净减43。S1 current为116/102/477，E1最终仍在评价。训练采集提案共有4个Stable、9个SUN及62个MSUN新增端点，但也有68个Stable和大量MSUN损伤，不能直接将候选机会当作模型接受后的增益。S1结束后按新范围做数据、动作、决策和训练诊断，再登记新的改进计划。见[最新执行计划](EDITOR_T2T_TRIAL_PLAN_20260910.md)、[停止范围回执](receipts/CLEAN_1000_STOP_AFTER_S1_REQUEST_20260910.json)及[E1实际更新](receipts/CLEAN_1000_E1_WEIGHT_UPDATE_20260910.json)。

## 0.16 固定方法、局部修正与1000固定Plan全流程（2026-09-10，用户已授权）

用户原话：“固定有效的方法，修正可能的错误（别影响大局），详细分析和记录，然后完成后开始1000固定plan的全流程”。执行解释为扩大固定TRAIN面板到1000，重新登记S0及三次G/E更新S1—S3，每个版本都完成draft→F800→token→Edit/KEEP→共同评价。新工作树为`DLM_periodic_self_repair_1000`，分支`codex/train-rsi-1000-20260910`。保留旧工作树和256全流程结果，旧完成时间、失败/重试成本及截止时间均不改写。

### 0.16.1 固定部分与证据边界

- 固定B0、词表和IO表；E继续由B0和种子20260909新初始化，不将旧E3或G3作为新S0起点。G后续更新实际LoRA，E更新现有内容适配与四个决策头。
- 固定§0.15已经运行的DLM几何恢复链：失败XYZ→邻域XYZ→全数值→最后Z放宽，保留原生完全重叠检查；不更改0.5 Å严格训练支持。
- 固定model494/tau800、raw已知SUN的F直通、F后离散化及失败回退、每轮一次E、四动作范围、温度0.7、80调用上限、接受阈值0.5和已知SUN KEEP先验9。
- 固定SUN＞严格Stable＞MetaStable＞普通改善的比较规则，普通同级能量差阈值0.01 eV/atom；联合力/应力弛豫停止及最多1000步不变。未知与物理失败继续分开，全部请求保留在分母中。
- G/E学习率、头学习率、beta、锚点和KL权重、KL上限0.02、G最多4遍/E最多8遍、每记录不重复的打乱遍历规则不变。此次不引入新T2T预训练、额外数据搜索或新的在线物理选择器。

256结果支持扩大验证恢复链，但并未证明RSI普遍提高稳定性。新S0最终30 SUN/32 Stable，S3最终32/33；S0未经E的F/token已有34 Stable。E3虽有140次实际修改，却只有3个Stable新增、2个Stable损失，并有23个已知物理失败。新1000实验必须继续同时报告Stable保住/损失/新增及SUN变化，不能仅把总编辑数或SUN/Stable比例当成收益。

### 0.16.2 本次修正及为何有界

1. **固定分母从代码常量改为登记值。** 原`prepare_fit`、TRAIN检查、配对编译、归档和汇总器分别写死256，会让1000任务被截断或失败。现在读取正整数`requests`，完整核对数量、source ID与顺序；不放宽失败保留和TRAIN排除规则。已有源池只读核验共有5119个合格且不与MAIN约化组成重叠的独立祖先。采用原`rsi_v1`哈希排序的前1000条，含987个不同约化组成；前256条完整Plan/prompt/噪声种子与旧面板一致。最终manifest在正式运行前冻结，失败不换Plan。

2. **补上已计划的历史Stable教师。** 旧S1/S2/S3在更早同Plan的current终态中分别有14/15/15个可用Stable目标，却没有进入教师编译。新流程在本次运行的更早完整current/edited端点中，为当前未Stable、可可靠比较的状态检索同祖先、完整prompt和plan_state一致的Stable候选；按Stable与hull排序，逐个检查元素槽位对齐及原动作/几何支持。每个当前态仍最多一个教师，历史目标不适用时沿用自身CHGNet终态量化。所有候选绑定源输入、评分、物理协议和哈希；历史U不作新标签，组装后的教师集合仍走原物理评价与整组N/U评分。物理缓存仅在端点及协议精确匹配时复用。

3. **mode按当前态监督，accept按候选监督。** 旧三轮分别有30/27/43个相同“Plan＋current”同时标为KEEP和EDIT，另有不同EDIT范围冲突。新编译在同条件候选中选一条质量最好的可靠正例作为mode/范围目标；没有正例时只保留一条已有KEEP监督。其余候选的接受正负标签和有效内容偏好仍保留，但不重复给mode贴标签。接受头训练显式支持`mode_target=null`；有限候选池中没有正例不被解释为“所有可能的编辑都失败”。

4. **内容KL停止不再截断后续决策头训练。** 旧E1/E2/E3均仅更新1步，实际Stable晋级记录覆盖0/1/1，且原日志把下一批检查访问也计入曝光。新E若任一rank观察到KL超过原0.02上限，全rank统一冻结LoRA、状态条件器及内容适配参数，清除这些参数的梯度并禁止AdamW继续更新；mode/site/count/quality四个现有头继续在原遍历数与时间预算内训练。G仍触发即停止。该停止保护保留已完成的最后一步，不声称回滚到KL必然低于阈值的权重。记录触发KL、内容/头更新次数，以及分别真正参与更新的pair ID；被检查但未做内容更新的批次不算内容偏好训练覆盖。它修复有效训练被截断的问题，不预先保证接受器校准或Stable改善。

**特意保留的推理语义：** 对旧E3重新核验发现19个非SUN输入的mode偏向KEEP，其中3个仍被接受头批准编辑：114的hull变差、129变为未知、214从MetaStable进入SUN。因此§0.15.1提出的“让正式执行立即尊重mode KEEP”仍是后续研究建议，本次不自动采用；原“已知SUN直接KEEP，其余生成候选后由接受头判断”的规则继续。若直接把mode=0变为全部状态的硬停止，会同时删除已经观察到的214号收益，无法称为中性的错误修复。

### 0.16.3 新实验登记、预算与验证

1000请求来自TRAIN，原MAIN及其约化组成排除继续生效；本轮不是独立测试集结果。主路和固定第二RAW对照各1000，后者只沿用预先固定的第二G seed供第一次G偏好，不能搜索seed或逐请求物理择优生成最终输出。各轮复用同一Plan/prompt和主路G/F seed。扩大到987种组成仍不足以证明模型已学会稳定结构的组级唯一性，N继续按原登记输入端点评分；不会将输入N变化自动解释成新的弛豫后多晶型。

新运行独立使用不超过12小时的绝对期限、最多6 GPU/3个同时运行或等待作业。起点按调度登记时间保守计时；不以本次授权延长旧运行截止。资源足够时S0与对照按3+3卡，单路及训练6卡，每卡真实batch容量32（全局192），E推理batch64、G每卡3进程、F/CHGNet每卡4进程，每卡申请4 CPU。完整数据遍历数不随batch容量增加。保留加载/报告的固定时间，对1000样本的剩余G/F/物理评价时间按256实测基准和GPU数缩放预留；该预留不是运行时长保证，绝对期限与完整回执决定实际可完成量。

当前本地21项针对性检查已通过：1000全量编译、扩大前缀与TRAIN排除、错误分母拒绝、同Plan/物理绑定、候选接受标签与mode去冲突，以及14项调度/预算检查。真实PyTorch优化器、分布式KL停止及GPU短验证仍在进行；正式1000作业尚未启动，不将静态通过写成实验效果。启动后追加不可变源码、Plan哈希、作业号、时间和预算回执。

## 0.15 完整流程与DLM构造三级恢复（2026-09-09，最新用户要求）

用户明确纠正了上一轮在G→F处提前终止的执行方式：必须走完 **draft→refine→token→Edit/KEEP** 才判断完整输出。旧修复轮S1的E1补跑及新协议S0—S3现已全部完成；§0.14中“E1未执行、当前作业为空”仅为此前停止时的历史状态。新协议从同Plan/种子的S0建立对照，G+F指标只作诊断，没有再单独阻止后续E。原12小时绝对截止时间、最多6张A800及3个并发作业没有重置。

**最终结果（全部以256请求为分母，Stable包含SUN）：**

| 轮次 | 可构图draft | raw SUN / Stable | F/token SUN / Stable | 最终SUN / Stable | E实际修改 | 最终已知物理失败 |
|---|---:|---:|---:|---:|---:|---:|
| S0 | 255 | 4 / 4 | 30 / 34 | 30 / 32 | 43 | 9 |
| S1 | 256 | 4 / 4 | 29 / 33 | 29 / 33 | 0 | 0 |
| S2 | 255 | 4 / 4 | 28 / 32 | 28 / 32 | 0 | 0 |
| S3 | 256 | 3 / 3 | 28 / 32 | 32 / 33 | 140 | 23 |

三级恢复起点相对旧完整S0使SUN27→30；随后三次RSI使新S0最终SUN30→32、Stable32→33。最终SUN为12.50%，相对新S0增加0.78个百分点；但相对S0未经E编辑的F/token端点，Stable34→33，不能仅凭相对随机初始化E0后的净数宣称已稳定保护Stable。S0最终→S3最终的配对为SUN保住26、失去4、新增6；Stable保住27、失去5、新增6。完整数字见[rsi/ranked_execution/CASCADE_FULL_FLOW_CLOSEOUT.json](rsi/ranked_execution/CASCADE_FULL_FLOW_CLOSEOUT.json)。

完整运行于北京时间2026-09-10 02:28:25结束，距原03:31:10截止时间尚余约63分钟，从原预算起点计10小时57分15秒。当前无遗留的本任务作业。按登记作业的实际分配时长计，新三级恢复目录14.353 GPU小时，连同前两次失败/修复目录合计36.515 GPU小时，最大同时6张GPU；不含无关用户/同账号其他任务。154个登记作业含148完成、3失败、3取消，成本没有隐藏重试。新协议实际优化器计算合计约1.051 GPU小时，不能把全部物理评价/加载成本称作神经网络训练时间。六次检查点参数变更与文件哈希已核验，四轮Plan逐字节一致，七份轨迹/训练归档的469,432,375字节已逐文件核验。

本次恢复仅发生在 **DLM生成时的几何限制**：初次失败后重新生成失败原子的XYZ；仍失败则扩大到周围原子的XYZ；仍失败则重新生成全部数值结构token（晶格与全体XYZ，保持Plan、元素和原子数）；最后仍失败时，仅在Z补全时放宽新增的0.5 Å周期距离硬限制。实现保留原生完全重叠检查，因为完全重叠无法构造refiner输入图。放宽生成不等于物理有效；后续几何、CHGNet与hull评价保持原口径，所有恢复阶段、seed、开放位置、失败原因及放宽标志保留。

首个32例定向探针覆盖旧B0的全部27例失败及5例成功对照：5例原生完成，7例在失败XYZ恢复、7例在邻域恢复、5例在全部数值恢复、8例在最后Z放宽完成；5个成功对照逐token复现。然而最后8例出现完全重叠，无法进refiner图。修正恢复原生重叠检查后，8例GPU复测全部生成且全部可构图；最小距离约0.060—0.177 Å，仍明确标记不满足原0.5 Å几何支持，等待refiner及物理评价。这个定向探针不进入正式256请求分母，也不能据此宣称SUN提升。

放宽后的结构可能不属于原严格内容训练支持，因此物理轨迹完整保留；编译时显式审计零支持内容目标，保留可训练的有效内容锚点及E动作/接受监督，不编造被mask目标的有限条件概率。当前源码为`d88e68a76007384173c76c0d4b94250618e0eeb8`；运行根为`run_20260909T132800Z_cascade_fullflow`。实现与小测试通过仅证明协议可执行，效果等待完整物理与编辑输出。

用户进一步要求：先完成手头工作，再结合此前讨论仔细分析Edit/KEEP，优先提升Stable，在保持Stable的前提下提升N与U。后续分析必须拆开候选产生、内容修改、接受/拒绝及停止，统计Stable保住/丢失/新增，并在Stable集合内分析N/U。当前轮次不因这项后续研究指向而偷偷更换已经登记的训练目标或增加未评价的选择器。

旧S1已于约14:03 UTC补完全部E1及评分归档，完整结果相对S0最终输出为可靠SUN **27→26**、严格Stable **29→29**、独占MetaStable **85→99**，256请求分母不变。E1仅完成1个优化器步，随后触发预设KL上限；训练回执约37.8秒。223个可用输入的模式头均输出KEEP，其中26个已知SUN直接保持，197个非SUN仍按既定策略生成提案；接受概率范围0.0221—0.0869，全部拒绝，实际修改数0。新三级恢复正式S0在旧S1归档后接续启动。机器可读回执见[rsi/ranked_execution/REPAIRED_S1_COMPLETE_FLOW.json](rsi/ranked_execution/REPAIRED_S1_COMPLETE_FLOW.json)；其中通用三轮汇总器的`complete=false`表示旧运行没有G2/E2/G3/E3，不否定单轮S1已完整结束。

旧S1的E1提案评分表明29个按登记质量顺序有益的提案均被拒绝，其中2个从MetaStable跨入SUN（ordinal18与85），接受概率分别约0.0620与0.0604。但整批提案也会损失3个非SUN的Stable（ordinal87、175、243），并出现45个无效物理终点。整批提案SUN为28、Stable为28；这不是实际输出，也不支持直接取消接受器。此处必须同时处理好提案召回与Stable保护，不能用SUN净数遮住稳定样本损失。

E1训练前的监督来源也不足：401条记录中34条为编辑正例，学生训练提案仅新增1个Stable；教师候选29个Stable全部保住但没有新增，只有3条登记为可靠改善。新一轮目标供应、接受器区分度及有效更新步数需要分别报告，不能把训练时间预算或普通降能正例数当作Stable学习已充分完成的证据。

只读接受分数诊断进一步得到29个有益、96个退步提案之间的AUROC约0.600；阈值0.06会接受48条，其中11条有益、24条退步，Stable新增2但损失1。这不是新的部署结果，不据此调部署阈值。现有Stable非SUN的3条均为N=false、U=true。本批256请求实际有254个不同约化组成，只有LiMn2O4及InMg各重复一次；这些重复组成在旧S1均未严格稳定，因此不能凭本面板证明Stable条件下的U改善。诊断脚本见[rsi/ranked_execution/analyze_editor_opportunities.py](rsi/ranked_execution/analyze_editor_opportunities.py)，摘要见[rsi/ranked_execution/REPAIRED_E1_ACCEPTANCE_SUMMARY.json](rsi/ranked_execution/REPAIRED_E1_ACCEPTANCE_SUMMARY.json)。

新S0的生成已完成：255/256可构图，220个原生成功对照逐token复现；失败XYZ、邻域XYZ、全数值、最终Z放宽分别完成11、8、8、8条，另1条退化晶格未恢复。raw结构有效247、组成有效203，可靠SUN/Stable仍为4/4。原27条构造失败均恢复，但旧1条晶格恢复成功样本本次仍失败，净增26个可构图样本。新增输入尚未直接增加raw稳定性，F与最终E结果继续评价。

资源适配记录：节点当时只能为本任务提供约4卡，原4+2调度使并行路交替等待。调度器提交`f96b697e4d4413e47b283e7566e238c902a2c099`将并行阶段主路改为2卡、对照/采集路2卡，单路生成/评价与训练仍用4卡；旧控制进程退出后复用已完成回执，正在运行的Slurm作业不动，仅取消未开始的4卡S0 refine并以2卡重提。科学代码继续固定为`d88e68a...`，控制器单独记录版本及哈希。修正末尾作业时长按剩余预算缩减、E训练不再预留已完成的当前G/F时间，原19:31:10 UTC绝对截止时间不变；4项预算回归测试通过。各作业实际资源及训练秒数仍以其提交/完成回执为准。

进一步核查发现一个明确的E实现缺口：旧S1中15个当前未达到严格Stable、且物理结果可靠并有完整token输入的请求，在已保存的S0主路或对照路中已有同Plan的可靠Stable终态。逐条核验了ancestor、完整prompt和plan_state一致，并保存输入/评分哈希；这15个目标包括ordinal18与85，两者也恰好是E1实际提案中被拒绝的Stable新增。§0.13原计划已经提出历史可信目标优先，但当前`teachers()`仅将自身CHGNet终点量化，没有把这些历史终态编译为E目标。因而“当前教师没有新增Stable”不能概括为整个已有数据池没有Stable目标。下一版E目标编译应先补足这项复用，再考虑扩充计算；现有池可用目标数量不等于学生实际学成数量，也不能直接拼接为新SUN成绩。证据见[rsi/ranked_execution/EXISTING_STABLE_TEACHER_OPPORTUNITIES.json](rsi/ranked_execution/EXISTING_STABLE_TEACHER_OPPORTUNITIES.json)。

新S0完整流程已结束：raw→F/token→Edit/KEEP的可靠SUN为 **4→30→30**，严格Stable为 **4→34→32**；最终独占MetaStable为90。相对旧S0的F/token端点，27个SUN、30个Stable全部保留，新增3个SUN和4个Stable，分别来自最终Z放宽（49、121）、失败XYZ恢复（187）及全数值恢复（136，仅Stable）。8个最终Z放宽样本均经F回到严格几何支持、物理终点均verified，2个成为SUN。220条raw未变化的对照，其F后token全部逐字节复现。E0权重三个主要文件与旧初始化哈希相同；225个提案、43次实际修改没有新增SUN或Stable，却损失2个Stable。最终SUN30/Stable32对比旧完整S0的27/29，初始增益来自恢复流程，尚不是RSI权重更新的收益。回执见[rsi/ranked_execution/CASCADE_S0_COMPLETE_FLOW.json](rsi/ranked_execution/CASCADE_S0_COMPLETE_FLOW.json)。

旧E1的有效训练覆盖已按实际`epoch_indices`、seed、world size及优化器步数重建：401条记录中只有前128条参与了唯一一次更新，包含12条EDIT正例，但Stable晋级与SUN编辑正例均为0；随后被检查的批次不能计作已更新样本。20条SUN KEEP监督和2条仅Stable KEEP监督不等于学习新增Stable。该证据进一步说明不能用编译总量或预算时长代替实际目标覆盖。见[rsi/ranked_execution/REPAIRED_E1_FIRST_UPDATE_EXPOSURE.json](rsi/ranked_execution/REPAIRED_E1_FIRST_UPDATE_EXPOSURE.json)，可复用分析器已增加实际更新批次统计。

资源进一步缩减时，节点只剩3张可用GPU及14个可分配CPU核。当前按3卡、每卡batch容量43（全局容量129）训练，保留学习率、4/8次G/E遍历上限及KL界限；单路3卡、并行2+1卡，每卡CPU申请上限4核，生成仍保留每卡3个进程。新G1已完成8步、4次遍历，训练201.0秒，峰值54.3GB，末步KL约0.00394；权重保持后重提尚未开始的生成申请，最终以3卡/12核/9进程启动。当前调度器与提交器为`7646e3b...`，科学执行源码仍为`d88e68a...`；10项调度回归测试通过，所有旧配置/取消的待运行申请均留档。资源恢复后可通过根配置调整后续阶段，已登记的训练配置仍绑定各自实际GPU数和batch。

约15:37 UTC节点资源释放后，后续阶段恢复为单路4卡、并行2+2卡、训练每卡batch 32及每卡4个CPU核；已完成G1仍绑定原3卡/batch 43配置，不重新训练。S1生成以原3卡申请完成256/256可构图：原生216、失败XYZ恢复10、邻域XYZ恢复9、全数值恢复9、最终Z放宽12，无最终构造失败。与S0相比恢复次数增加，不能据此称作生成器原生能力或Stable提升。当前控制进程为第五次调度续接，科学代码与绝对截止时间不变，S1 raw物理评价已用4卡/16核运行。

旧E1的动作监督进一步核验：401条训练记录对应218个不同的“Plan＋当前token”输入，其中33个输入出现不同mode标签，32个是KEEP与EDIT并存，另1个是两个不同EDIT范围。模式头在提案生成前只看同一当前输入，而编译器把各候选失败／无益直接标成该输入KEEP；这会混合候选质量与是否值得尝试编辑的监督。接受头看到实际提案，不能将它的正负标签也称为同一条件冲突。34个编辑正例的范围为22个局部XYZ、1个全部XYZ、11个全晶胞；教师的3个正例都是全晶胞，其中1个实际仅8个数值token变化但动作开放36个。不能泛称全部正例都是大范围重写，也不能把上述监督风险当作已被干预实验验证的单一原因。后续应按当前状态聚合候选机会来监督尝试／范围，并另行监督具体提案接受。证据见[rsi/ranked_execution/REPAIRED_E1_MODE_SCOPE_AUDIT.json](rsi/ranked_execution/REPAIRED_E1_MODE_SCOPE_AUDIT.json)。

新S1完整流程于约16:30 UTC完成：raw→F/token→Edit/KEEP的SUN为 **4→29→29**，Stable为 **4→33→33**；最终独占MetaStable113、已知物理失败0、未知5。相对新S0最终SUN30／Stable32，本轮SUN减少1、Stable增加1；拆分后，G/F的Stable实际34→33，而E1全部保持，避免了S0中E0损失2个Stable的情况，不能将最终Stable净增归为E1学会了修复。G/F端SUN保住27、失去3（51、125、148）、新增2（136、168）；Stable保住31、失去同3条、新增2（28、168）。256条均可构图；12条最终Z放宽经F均回到严格几何支持；252条完成显式F后几何检查，另4条为raw已知SUN直通，0条回退raw。完整回执见[rsi/ranked_execution/CASCADE_S1_COMPLETE_FLOW.json](rsi/ranked_execution/CASCADE_S1_COMPLETE_FLOW.json)。

新E1训练编译460条，其中38条EDIT正例，却没有任何Stable晋级正例；实际唯一更新覆盖128条、10条EDIT正例，Stable晋级及SUN编辑正例覆盖仍为0，训练38.7秒后触发KL上限。最终256个模式头均预测KEEP；29个已知SUN直通，其余227个仍按既定策略选择非零动作生成提案（82局部XYZ、136全部XYZ、9全晶胞），全部被接受器拒绝。227个提案中27个按质量顺序有益、117个退步，其余83个平局／不可比较；加上29条直通，完整256分母的平局／不可比较为112。候选仅新增1个Stable/SUN（18，接受概率0.0568），却损失4个Stable（28、87、175、243），其中2个物理终点无效。整批提案SUN30／Stable30，不是实际输出。接受分数对有益／有害提案的AUROC约0.533，阈值0.05会接受154条，含24有益、79有害，Stable新增1、损失2；这仅为只读诊断，不支持直接降低部署阈值。

新E1量化教师保留全部33个Stable，没有新增Stable；2个SUN（0、131）仍Stable但N变为false。训练数据251个不同当前条件中，30个同时存在KEEP与EDIT标签，另3个存在不同EDIT范围标签。以上继续支持分开修复目标供应、模式监督、内容训练覆盖与接受器，而不能仅增加盲目编辑次数。已归档的科学源码仍固定d88e68a；调度器836a039仅在教师分支结束后让后续尚未提交的提案物理评价恢复单路4卡，教师运行时仍共享2+2卡，旧已运行2卡申请保持，13项调度测试通过。控制器第六次续接复用完成回执，原19:31 UTC截止时间不变。

统一历史目标审计已在新S1确认14个当前可靠但未Stable的输入，在此前S0主路／对照路中已有同Plan、已验证SUN的终态；这些目标没有进入当前E1教师编译。脚本同时复现旧S1的15个机会。该计数只使用各轮开始训练前已存在的历史，不使用后续轮次；逐条核对ancestor、完整prompt、plan_state及输入顺序并保存哈希。14个可用目标不等于14个已学成晋级，也不能拼接为SUN成绩；但它说明优先补上已计划的历史目标复用，比立即假定需要扩大物理搜索更有依据。

新S2完整流程于约17:29 UTC完成：raw→F/token→Edit/KEEP的SUN为 **4→28→28**，Stable为 **4→32→32**；最终独占MetaStable112、已知物理失败0、未知6。G2完成16次更新、4次遍历、训练281.9秒，未触发KL停止。生成255/256可构图，211原生成功，失败XYZ／邻域／全数值／最终Z放宽完成8／4／9／23条；唯一失败为189号退化晶格。255个token输入中251个显式几何通过，4个raw SUN直通，0回退raw，23个最终Z放宽均经F回到严格支持。相对S0的F/token，SUN保住23、失去7、新增5；相对S1的F/token，保住24、失去5、新增4。失败189不在损失SUN集合中。最终相对S0的SUN30／Stable32为SUN减少2、Stable持平。回执见[rsi/ranked_execution/CASCADE_S2_COMPLETE_FLOW.json](rsi/ranked_execution/CASCADE_S2_COMPLETE_FLOW.json)。

E2的443条训练记录有35条EDIT正例，其中4条晋级Stable，分别是学生提案125、190、248及教师42。教师42从hull约+0.01688到−0.01195且N保持true，说明自身量化教师并非绝不可能跨Stable；本批教师新增1个Stable/SUN、无Stable损失，同时仍有0／131两个N损失。E2实际只更新1步、37.0秒即触发KL限制，128条实际更新记录中7条EDIT正例，仅125这一条Stable晋级正例得到更新，教师42未覆盖。最终255个可用输入模式均预测KEEP，28个已知SUN直通，227个提案全部拒绝，另1个上游失败。提案按登记质量有39有益、83有害，其余105平局／不可比较；其中4个Stable新增、1个Stable内N改善，同时3个Stable损失，整批提案SUN33／Stable33不是实际输出。接受AUROC约0.593。历史目标审计在本轮发现15个当前未Stable输入已有可用的同Plan历史Stable终态，仍未进入当前教师编译。

Stable条件下的N需明确端点：E2的87号提案按登记指标从Stable非SUN变成SUN，但编辑前后各自的共同物理弛豫终态，用同一StructureMatcher设置双向匹配均为true，归一化RMS约0.00473。它没有被实际接受；该诊断也没有重新计算终态相对完整训练参考的N。因此这里仅能称为提案输入端的N增益，不能据此声称发现新的稳定多晶型。若后续目标强调新材料，除保留登记输入端N外，应增加共同物理终态的新颖性与差异诊断。证据见[rsi/ranked_execution/CASCADE_E2_N_ENDPOINT_DIAG.json](rsi/ranked_execution/CASCADE_E2_N_ENDPOINT_DIAG.json)。另外，S0中SUN/Stable比例从30/34增至30/32，仅因为E0损失了2个Stable非SUN；条件比例提高本身也不等于在保住Stable的前提下改善N/U。

新S3最终E3与前两轮不同：209个模式预测local_xyz、47个预测KEEP；28个已知SUN均逐token保留，228个提案全部为local_xyz，接受140次。固定0.5阈值下接受概率均值约0.518，范围0.268—0.697。140次实际修改中36次按质量顺序有益、45次退步，其余59次平局／不可比较；所有提案有47次有益、79次退步，接受AUROC约0.577。E3新增Stable85、157、214，损失Stable136、243；87从Stable非SUN变成SUN，故本轮E使SUN28→32、Stable32→33，同时产生23个已知物理失败。不能把E1/E2的“全部KEEP”结论延伸到E3，也不能把140次修改都当成修复收益。详情见[rsi/ranked_execution/CASCADE_S3_COMPLETE_FLOW.json](rsi/ranked_execution/CASCADE_S3_COMPLETE_FLOW.json)。

E3训练478条记录中53条EDIT正例、3条Stable晋级记录，后者来自28号学生提案及52号学生/教师两个目标，仅涉及2个当前状态。实际唯一更新覆盖128条、16条EDIT正例，仅52号学生晋级提案参与更新；训练36.3秒后同样触发KL上限。三次G更新为8／16／20步、均完成4遍；三次E更新均为1步，实际Stable晋级记录覆盖0／1／1。E3量化教师有1个Stable新增和1个Stable损失，另有N损失，不能只看总Stable未变；本轮历史Stable目标机会仍有15个，当前编译未使用。训练覆盖不足、目标供应和接受器语义需要分别修正，不能将任何单一因素当作已经经过因果消融证实的全部原因。

E3实际接受的87号N转换也进行了独立端点核查：登记输入N由false变true，但共同弛豫前后终态仍双向结构匹配，归一化RMS约0.00460。这一登记SUN增益保留，但仍不能据此声称产生新的稳定多晶型；没有在本轮事后修改N定义，也没有重新对完整参考库计算终态N。证据见[rsi/ranked_execution/CASCADE_E3_N_ENDPOINT_DIAG.json](rsi/ranked_execution/CASCADE_E3_N_ENDPOINT_DIAG.json)。

对最终E3，固定阈值已接受全部3个Stable晋级提案及全部4个登记SUN增益，同时挡住3个Stable损失提案中的1个；因此不能将E1/E2漏接好提案的诊断直接套到E3，也不能期待单纯降阈值再找出本批不存在的额外Stable晋级。最终更直接的问题是好提案数量仍少、2个Stable损失仍被接受，以及大量非Stable输入被编辑到物理失败。后续应增加可学的晋级目标并提高Stable保护精度，同时校准接受器，而非只推动更多修改。

### 0.15.1 Edit/KEEP下一版建议：先Stable，再做Stable内的N/U（研究结论，尚未执行）

下一版建议暂时固定一个已归档的G/F输入分布，先把单次E做成有效修订器，再恢复联合RSI或增加编辑轮数。当前事实不足以支持“只需放宽接受阈值”或“只需多训练几轮”：已发现目标未复用、模式监督混合候选标签、有效更新覆盖不足，以及候选本身会损失Stable等不同问题。以下是后续研究建议，不改变本轮已登记的三个更新。

**1. 先修目标编译，使用已有证据。** 对每个真实F/token当前态，先检索相同ancestor、完整Plan和元素槽位下的历史可信终态，包括已评分但被拒绝的好提案；之后才补自身量化终态和新增离线搜索。新S1/S2仅在已审计的历史current终态中就分别有14/15个机会，因而应先解决已存在目标未进入训练的问题。候选须对齐原子槽位、检查动作支持与量化后完整结构，绑定实际物理标签和源哈希；不能把不同Plan的好结构配给当前输入，也不能把历史目标直接拼成当前SUN成绩。

训练标签分开表示：非Stable→Stable/SUN晋级、Stable保留、Stable损失、保持Stable的N/U改善，以及低等级普通改善。物理失败可以监督拒绝该提案；尚未确定的物理/参考状态保留未知。低等级降能记录可以辅助学习，但不能替代跨Stable监督。先确认高等级目标真实存在且参与更新，再扩大数据规模。此前MP20扰动/identity数据有可复用价值，但几何恢复标签不能自动当作物理Stable标签，恢复已知训练晶体也不是SUN正例。

**2. 分开“是否值得尝试”与“这个提案是否接受”。** 模式头只看当前态，应根据该当前态的可信候选集合学习尝试收益和可行范围；不能因为某一个候选无益，就给整个当前态贴上不可编辑的KEEP标签。接受头则看到当前态与实际提案，按这一对结构的可靠结果训练。候选不足时，避免把有限搜索未找到好目标解释成全局不可修复。当前非SUN输入即使模式预测KEEP仍生成非零动作提案；下一版应把强制探索明确限定在训练采集，在模式监督和校准可靠后，让正式执行尊重模型的KEEP/EDIT，再由接受器审核具体修改。不能用已知SUN直通规则替代自主Stable判断。

**3. 给E充分的编辑训练，而非从新初始化直接依赖稀疏偏好。** 沿用此前讨论的可见旧态、未修改上下文和真实学生错误，先对可信完整目标做条件T2T监督，再加入偏好/拒绝训练。MP20受控扰动可作为基础恢复能力来源，但需要与真实G→F状态混合，并区分几何恢复和物理晋级。仅有两个跨盆地终点时，不用坐标插值伪造能量下降轨迹。小范围或增量编辑值得研究，但投影/缩小范围后的完整候选必须重新核验；当前full_cell动作不能简单改名为“小编辑”。

训练控制上，将决策/接受头的学习与内容KL停止分开；内容参数使用更小的学习率和明确的KL约束，而不是直接放松KL保护。记录实际触发停止的KL值、参数组及真正完成的更新。按类别交错但不重复的数据排序，保证已有晋级正例和Stable保护样本进入实际更新批次，同时保留每条记录的曝光上限；不恢复此前把极少Stable样本反复高概率采样的做法。E2文件虽有4条晋级记录，实际仅1条更新，正说明预算、遍历上限、编译数量和有效覆盖必须分别计量。

现有动作已能提供一部分所需能力：E2生成的227个提案全部使用local_xyz；其4个Stable新增、3个Stable损失以及1个Stable内N转换，都只选择了1个原子的XYZ。因而不能将问题简单归结为“改了太多原子”。当前重生成的是绝对坐标，少量token仍可能对应较大物理位移；应优先核对内容训练、位移幅度与接受风险，避免未验证就新增一套复杂动作接口。

**4. Stable内的N与U分别处理。** 单看SUN/Stable比例会产生幸存者偏差，应同时报告Stable保住、损失、新增以及这三个集合内的N/U变化。对于N，优先使用确实保持Stable、且在目标物理端点仍不同的结构；保留本轮登记的输入端N，并增加共同物理终态的N/结构差异诊断，不事后改写本轮成绩。87号例子说明输入N改善尚不能证明新的稳定多晶型。对于U，当前256请求只有254个不同约化组成，重复组成没有形成足够的Stable去重场景；不能将这里近乎恒真的U当成已学会的能力。后续先构造同组成多候选组，在保持原U比较规则的同时评估新增Stable且独特的贡献；组级训练或模型自身生成历史的上下文是候选方案，不把U误当成孤立结构的固有属性。

建议的执行顺序是：固定G/F→补现有目标与正确监督→完成有实际晋级覆盖的E训练→在模型自己选择范围和接受的单次编辑中检验Stable新增/损失→再研究Stable内N/U及多轮编辑。首个验收重点是保住已有Stable并产生真实新增，同时展示全部失败和拒绝；在独立来源上复核接受器后再选择阈值，不能拿当前TRAIN阈值扫描当成部署验证。推理时仍由DLM自主判断和编辑，物理教师用于离线监督与评价，不新增在线专家选优来冒充模型能力。
...
- 从每卡8个训练样本、16个E推理请求开始实测后，正式固定为训练batch 32、E推理batch 64；G每卡3个独立构造进程，F每卡4个独立请求进程，CHGNet每卡4个worker。硬约束、逐请求随机种子、失败记录和评测分母保留；E批量浮点计算不保证与旧单条推理逐token相同，因此正式S0—S3统一使用新的批量协议。
...
重跑阶段性实测：主路与对照路各256条G、F输出均逐条复现旧S0。成功生成作业分别提速1.88倍／2.24倍，F分别提速2.25倍／2.44倍；这些速度比较不含主路首次加载失败的重试成本。加载问题来自多个进程把适配器临时权重放到默认cuda:0，已用模型导入前的逐进程设备隔离修复。新E0的主要权重文件哈希与原固定初始化相同；批量E0最终SUN27、严格稳定29、独占亚稳态85，F端点仍为27／30／97。相关回执见[rsi/ranked_execution/REPAIRED_RESTART_20260909.json](rsi/ranked_execution/REPAIRED_RESTART_20260909.json)。

修复后的G1已完成8步、4次遍历、752次全局样本访问，训练127.46秒，峰值显存42.40GB，末步记录KL为0.00339。同Plan/种子S1生成223成功、33失败，相比S0净少6个成功样本（14条成功转失败、8条失败转成功），原27个F后SUN请求在新G阶段全部成功。S1 raw SUN5、严格稳定5、独占亚稳态26；S0对应4／4／32。S1首次计分因新轮次预期缓存目录未建立而失败，已复制并逐文件校验原完整缓存后只重试计分，未重复训练或生成。

完整G→F验收结果：SUN27→26、严格稳定30→29、独占亚稳态97→99。SUN配对为保住22、失去5、新增4；失去的5条（ordinal 51、125、127、148、219）都成功生成并经F降为亚稳态，并非生成失败。G1因未达到事先设置的SUN不低于S0门槛而拒绝；E1／G2／E2／G3／E3未执行，当前作业为空。轨迹与物理标签已归档，另编译363条同prompt G偏好记录供未来明确标记的离策略复用。退化幅度收敛不能写成SUN提升，也不能据此认定唯一因果机制。结果见[rsi/ranked_execution/REPAIRED_G1_RESULT.json](rsi/ranked_execution/REPAIRED_G1_RESULT.json)。
...
## 0.19 SUN 晋升瓶颈与范围／排序对照（2026-09-10）

用户在 10:09 UTC 明确要求“详细论证后推进，目标更进一步提升 SUN”，并要求先找出提升不够的原因。承接此前对误拒绝、非 SUN/MSUN 以及失败/未收敛输入的修复要求，现推进新的有界阶段；不恢复 G/F 训练或采样。执行者设定阶段上限 3 小时、5 GPU、3 作业，原阶段预算和结果保持归档。

直接证据：旧三轮 E 各仅 1 次实际更新；1381 条名义监督中 1255 条 KEEP；897 条提案全部为单原子局部编辑，810 条集中 site 0；重复随机流不改变位置；四候选模型选择漏掉 493、974 两条已有 Stable 晋升。当前目标把新颖亚稳晋升 Stable 与新颖不稳定晋升亚稳等价奖励，与 SUN 首要目标不一致。此前延长头部训练仍失败，故不能认定 KL 是唯一原因；也未发现足以认定 site 索引实现错误的证据。

新阶段预先固定其它原子排名的候选以及独立 SUN 排序监督，按相同 G/F 输入、真实模型选择、同一物理协议比较。新终测从已有 B0/F800 来源中按组成与祖先排除、参考覆盖和固定哈希选择 256 个来源，不读取质量结果；它与 G1/F1000 数据域不同，单列报告。具体证据、预登记超参数、DEV 采用条件与失败处理见 [SUN_RANK_SCOPE_PLAN_20260910.md](SUN_RANK_SCOPE_PLAN_20260910.md)。此处记录的是已经获得推进授权的实验安排，尚不是取得 SUN 提升的结果。

## 0.20 SUN 范围／排序阶段完成（2026-09-10）

已完成§0.19安排的范围扩展、真实训练、DEV选择、新来源评价及完整模型核验。旧E每轮一次更新与位置集中得到进一步量化：重建三轮实际更新批次共384行，其中局部监督24行；新位置把TRAIN新颖Stable晋升机会从7个来源扩大到19个。两个新判别器各用相同1917行/492来源训练64遍、512步，300参数，内容和范围权重保持冻结。

原差值模型8个DEV策略均没有SUN净增。DEV诊断发现候选级MS增量≥0会过严限制本来已是MSUN的来源；随后在新来源编辑结果打开前，登记候选终态概率对照，DEV选择阈值0.2，SUN+5、MSUN+3、Stable持平。原差值模型的失败与固定诊断输出保留，不覆盖。

新组成隔离256来源的同输入KEEP为Stable20/SUN13/MSUN129。终态模型19/19/129，即SUN+6、MSUN持平、Stable-1；原差值对照22/15/139，即+2/+2/+10；旧操作性收益头配新候选为19/14/136。终态模型是新终测前选定的主候选，未按新终测结果改选差值模型。两者分别未通过完整DEV及新来源联合采用条件，默认策略未替换，真实权重均保留并导出。

SUN提升必须保留其含义：终态模型新增6条SUN有5条原本Stable，4条弛豫终态前后仍能匹配；现有N取弛豫前输入，不能称为新增6个稳定新物相。差值模型选中的Bi₂Se₃Br₆为更明显的结构及稳定性共同变化：hull+0.121355→-0.006238 eV/atom，终态双向匹配均false。新终测不再用于追调这一模型。

24个Slurm任务全部成功结束，4.4453 GPU小时、19.3056 CPU小时，峰值5GPU/28CPU/3任务；最后计算结束于12:19:16 UTC，早于13:54:41截止。两模型各5020个完整E判断视图重载分数差异为0、选择完全一致，三个新来源输出面板全部绑定原候选/KEEP字节。29项行为测试通过。旧1000仍停于S1，旧预算与G/F没有更改。

详细论证、置信区间、真实得失与后续瓶颈见[SUN_RANK_SCOPE_RESULTS_20260910.md](SUN_RANK_SCOPE_RESULTS_20260910.md)，[完整回执](receipts/SUN_RANK_SCOPE_FINAL_20260910.json)，[两组权重](artifacts/sun_rank_scope_20260910/README.md)。后续优先方向是增加并学习真实Stable晋升内容正例，以及单列物理Stable保留监督；这属于后续研究方向，不是本轮已经完成的内容训练。

<a id="release-final-run-20260910"></a>

## 0.21 最后四小时提升、发布仓库与最终三轮完整运行（用户最新要求，2026-09-10）

状态：用户已明确“开始推进”。最后分析窗口于2026-09-10 13:02:01 UTC开始，默认截止17:02:01 UTC（北京时间次日01:02:01）；发现很好的机制可按已有授权延长。用户对最终样本口径进一步明确：“按照合法plan的前1050条吧”，因此后续执行使用固定1200清单中原顺序前1050个合法Plan，三轮集合不变；原文中的1000仅保留为历史原文。上一轮已完成实验的预算与结果不被重写。以下要求优先于此前临时实验方案和人为流程门槛。

用户最新补充：“别忘了上一轮我们效果很好，好的内容记得保留”。上一轮有效组件、权重、策略配置和实测证据保留为发布基线，见[保留清单](receipts/PROVEN_COMPONENTS_TO_RETAIN_20260910.json)。本轮增加更新次数或分散位置本身不足以证明最终效果更好，实验结果与既有基线分别保留。

### 本阶段执行进度

本阶段实测进度：两档小学习率的E内容/定位训练均已完成248次内容更新和248次头部更新，492条可信TRAIN配对每条实际参与8遍。单独比较检查点确认LoRA、状态条件模块及位置头都有非零参数变化；尚待生成和物理评价判断收益。837定位为晶格角度的揭示顺序问题，条件重采样gamma后在47次DLM前向内得到合法结构和图；物理验证继续进行。549已产生全晶胞诊断编辑，不能在物理验证完成前称为修复。正式1050 Plan已按原顺序冻结，覆盖原始序号0至1061，三轮保持一致。

审计见[内容学习回执](receipts/FINAL_CONTENT_LEARNING_AUDIT_20260910.json)。本次验证训练由492个来源构成，其中313个原样保留目标、160个其它可靠改善目标、19个SUN晋升目标；不把313个保留目标当作正编辑。训练标签排除了物理不可靠的配对。另有2项内容训练回归检查和5项生成恢复检查通过。

### 用户原文

> 后面没什么时间了，你先把下述内容记录到文档中
>
> 最后给你4小时分析和寻找提升，后面开始整理repo（要求高复用，顶级水准，只保留现在的推理和训练流程，以及所有证明有效的改动和内容，然后清理失败的内容和冗余，杂乱的内容，然后推到新git https://github.com/YWDDLiang/DLM_ICLR ）后续要能在参数选择是否复用已有plan（比如256条的R03，1200条的H1A2,如果不复用则再使用plan模型），然后进入整个流程（也是代码高复用，可以通过.sh和slurm两种形式一键启动），这是要发布的git，禁止设置各种hard gate。后续训练代码也要整理，后续也要能一键使用代码进行训练（超参数先默认和本次一样），然后数据处理也是（现在只有mp20数据集，后面要支持好几个别的）
>
> 完成后你去做固定1200plan的完整链路采样（只看前1000条有效），自提升还是按照3轮来

### 已记录的执行顺序

1. **默认最后4小时：分析和寻找提升；发现很好的机制时允许延长。** 重点继续解决当前SUN提升、真实Stable晋升内容、排序漏选和不必要拒绝的问题；必要的小实验、修复与结果整理均在这一阶段中规划。用户补充：“如果发现了很好的机制可以延长时间”。因此四小时是默认窗口，不是不可延长的硬截止；发现有明确依据、值得继续验证的机制时，可在已有授权下延长，无须重新请求同一项授权。延长时记录具体机制、已有证据、下一步验证内容及预计新增用时，并向用户同步；这些记录用于说明工作安排，不是新增的指标hard gate。没有这样的机制线索时，按原顺序转入仓库整理，不仅因旧指标未达标而反复延长。之后的发布仓库整理、最终完整采样单独安排，也不把临时研究窗口写成公开代码的门禁。
2. **随后整理面向发布的新仓库。** 建立高复用、可维护、可复现的当前推理、训练与数据处理流程，筛选保留已经有有效性证据的改动及内容，清理失败实验实现、重复版本、临时脚本与杂乱依赖。完成后推送至 [YWDDLiang/DLM_ICLR](https://github.com/YWDDLiang/DLM_ICLR)。该地址是新的公开发布目标，不能只更新当前研究仓库便称为已完成发布。
3. **发布整理完成后做最终完整运行。** 复用固定1200 Plan，按原始顺序取前1050个合法Plan，执行完整生成、后续修订/评价、自提升训练及再采样链路；自提升固定3轮，不沿用旧1000实验“停在S1”的停止安排。

### 用户补充：四项已诊断瓶颈也是后续必须解决的工作

用户明确补充：“这也是你后面要解决的”，指向上一轮总结的有效训练不足、候选位置集中、接受条件误拒绝，以及失败输入修复四个问题。以下纳入最后分析提升阶段及后续实现的工作范围，不能仅保留为历史分析，也不能因为某个问题人数少而将其移出任务。

| 问题与已知证据 | 后续要推进的解决工作 | 当前仍未解决的部分 |
|---|---|---|
| 有效训练远少于名义轮数：旧E三轮各仅一次更新，实际384行中只有24行局部位置监督；延长头部训练也曾失败 | 查清提前停止、有效梯度和监督覆盖的原因，处理阻断充分训练的问题；补足有用内容与位置监督，开展实际内容/定位学习；记录实际参数更新和样本覆盖，以生成结果评价收益 | 最新两个排序器完成了训练，但E3内容和定位参数仍冻结，不能据此称为旧E有效训练不足已经解决；增加步数也不能代替验证 |
| 候选位置集中：897条旧提案全部为单原子编辑，810条集中site0，重复流只换坐标 | 改善模型的位置与编辑范围选择，增加有价值的候选差异和真实Stable晋升内容；利用已发现的正例继续学习，并检验候选覆盖能否转化为最终SUN收益 | 新位置调度已扩大机会，但定位头尚未重新学好；只增加候选数量不能视为问题完成 |
| 接受条件误拒绝SUN晋升：已是MSUN的输入增量上界为0，候选级预测增量≥0会过严；已有真实晋升仍被漏选 | 继续修正判断目标、排序和KEEP/EDIT决策，减少合法有益编辑的误拒绝；同时识别实际损伤，处理SUN、MSUN、Stable之间的取舍，避免把问题重新包装成额外hard gate | 终态概率模型已提高SUN，但新来源上MSUN持平、Stable少1；差值模型仍有DEV与新来源表现不一致，尚无全面稳定占优的决策方式 |
| 失败入口开放不等于失败已修复：549四条合法提案仍全部物理失败，837缺少有效结构视图 | 对有结构的失败/未收敛输入分析失败机制并改善修复内容；对缺少有效视图的上游失败，在完整链路中处理生成或恢复能力，明确其责任阶段；保持失败记录和固定样本映射 | 549、837目前均未修复。不能只展示“已尝试”或以失败人数少为由略过，也不能伪造结构或静默剔除失败来源 |

四项工作的共同目标是进一步提高SUN，并准确报告MSUN、Stable及真实结构变化；尚未解决的部分要保留明确状态。它们是研究和实现任务，不是要求全部达到某个手写指标后才能整理仓库、发布或进入下一轮的hard gate。时间安排沿用默认四小时、发现好机制可延长的最新授权；发布整理和最终三轮完整运行的顺序不变。

### 发布仓库的结构与复用要求

- 只保留当前需要的推理与训练流程，以及有有效性证据的改动和内容；不能将历次研究目录整体复制为发布仓库。
- 推理、采样、自提升、训练和数据处理各有清晰入口，共用核心实现；`.sh` 与 Slurm 只是两种启动方式，不能各自维护一套科学逻辑。
- 配置、模型/数据资产定位、运行目录和结果输出有统一约定。公开代码不依赖当前机器的绝对路径、历史任务号、旧研究目录或私有执行状态。
- 配套必要的依赖说明、配置示例、运行说明和能检验核心行为的测试，达到可由新使用者复用的发布标准。
- 失败实验实现、旧审批/调度包装、一次性修复脚本和重复入口从发布主线清理。原始研究证据可留在原研究仓库或独立归档，避免将“清理发布内容”变成删除或改写真实结果；公开说明仍准确保留有效性的适用范围与限制。

### Plan 参数与完整推理入口

- 通过参数选择是否复用已有 Plan。
- 已有 Plan 预设包括用户指定的 **R03 256条**、**H1A2 1200条**；预设需对应真实既有资产、来源及原始顺序，不能以其它同数量集合冒充。
- 不复用时调用 Plan 模型生成，然后进入同一条完整流程。
- 支持显式传入 Plan 文件或资产配置，避免将现有两个预设写死为仅有的数据来源。
- 后续处理共享同一实现；不能因为Plan来自缓存或新生成而复制生成、修订、评价、自提升的代码。
- 参数命名与目录布局在实现时确定；以上是要求的参数语义，不表示这些公开CLI已经实现。

### 禁止各种人为 hard gate

- 这是发布仓库的明确要求，不是可忽略的优化建议。
- 盘点现有hard gate，避免把研究时期的指标准入、审批状态、回执要求、固定Git版本/目录状态、特定实验成功标记等门禁原样带入公开推理或训练入口。
- 不设置“某项SUN/MSUN/Stable结果达到手写门槛后，才允许下一轮训练、采样或完成流程”的阻断规则；三轮流程不能因此自动缩为一轮。
- 模型本身的KEEP/EDIT等决策与额外人为流程门禁需要清楚区分；现有质量硬否决也必须纳入盘点，不能换个名字继续隐式阻断流程。
- 输入文件无法读取、依赖缺失、数据格式损坏等实际错误应有明确诊断；错误处理不能被扩展成额外科学指标准入条件。

### 一键训练与默认超参数

- 整理当前需要的训练代码，通过统一入口一键启动；同样提供本地`.sh`和Slurm方式，共用训练实现。
- 默认超参数先采用本次对应模块实际使用的配置，完整显式化，不以重构为由悄悄更改训练设置。
- 本次不同模块已有不同训练配置，需分别保存相应默认值，不能将一个排序器的超参数误用于所有G/E训练。
- 已实际验证的两个排序器公共训练设置为：Adam学习率`1e-3`、batch`256`、`64`遍、L2系数`0.1`、SUN排序项系数`0.2`、种子`20260910`。其各自目标、特征和模型结构按对应运行回执区分。
- G/E、Plan及完整三轮自提升所需训练配置，以对应实际训练回执/配置为依据整理，保留配置覆盖能力；不在这次登记中虚构未核对的默认值。
- 数据准备、训练、检查点保存和后续推理的接口衔接完整，避免需要人工拼接零散研究产物才能继续。

### 数据处理与多数据集支持

- 数据处理也提供可复用的一键入口。
- 当前先支持已有MP20处理链；架构要便于后续接入多个其它数据集。
- 使用统一数据表示和数据集适配接口，将数据读取、字段映射、预处理、划分和资产配置与核心模型/训练逻辑解耦；接入新数据集不复制训练或推理代码。
- 数据划分、组成/来源标识、必要单位和评估参考由数据集配置提供，不在核心流程中散布MP20专用路径和假设。
- 用户尚未指定其它数据集名称；此时记录可扩展要求，不宣称已实现未接入的数据集。

### 固定1200 Plan的最终运行要求

- 在仓库整理完成后执行，使用整理后的统一代码和一键入口。
- 固定1200 Plan的来源、顺序和版本；结合上文现有资产，优先对应H1A2 1200 Plan，并核对真实清单。
- 按用户后续明确口径，取原始顺序前1050个合法Plan。启动前记录合法性定义以及原始序号到所用集合的映射；不以生成成功、SUN/MSUN、能量或某轮结果来后验筛选。
- 同一选定集合贯穿对照和三轮自提升，避免后续轮次更换样本导致不可比。无效或未完成项保留明确记录，不静默缩小分母。
- 自提升完整执行3轮，保存每轮模型版本、实际训练更新、采样输出和共同评价结果。用户明确的“前1050个合法Plan”不意味着可以把采样、训练或轮次简化为部分流程。

本节已进入执行，阶段状态以分析窗口和后续运行记录为准；仓库发布、最终三轮运行仍须实际完成后报告。后续按上述顺序推进，无须重新逐项请求已经给出的工作授权。


### 执行中补充：可行性验证与干净的正式训练数据

用户进一步明确：“后续只要证明keep/edit机制可行就行（要求不要太苛刻），验证实验完成后的正式实验还是要训练数据要干净些”。验证重点调整为机制确实能训练、产生有效编辑，并在同输入对照中表现出实际收益；不再要求每次SUN、MSUN、Stable或所有随机流同时全面占优。仍如实报告损伤、统计不确定性及未解决项，不把降低采用苛刻程度理解为降低结果真实性。此处评价记录不构成阻断发布和三轮执行的hard gate。

正式实验单独形成干净的训练数据：明确原始来源、数据角色、组成/祖先映射、去重、可靠的监督标签和完整数据处理配置；剔除工程错误、来源不明、冲突或损坏记录，不把未知物理标签当负例。验证及最终评价集合与其后代不混入正式训练，除非未来用户明确改变数据角色并重新划分评估；对应评估隔离边界要在正式训练前固定。


### 2026-09-10 16:16 UTC：追加研究与三次正式运行

用户原文：最后本次keep-edit部分完成后（你还有3小时研究怎么更进一步提升SUN和MSUN），后续允许你多跑2次3轮的1050条的全流程，保留最好的那次记录到主表，剩下两次作为私有文档记录储存。

用户最新追加：从2026-09-10 16:16:40 UTC起继续研究3小时，当前截止为19:16:40 UTC（北京时间次日03:16:40）。整理发布后，总共执行三次独立的1050合法Plan全流程，每次完整三轮；预先按最终S3的SUN、MSUN、Stable依次择优，主表注明三次中的最佳并保留该次S0至S3完整过程。其余两次完整结果与分析只保存在私有文档及私有产物中，不提交公开仓库。三次共用固定Plan集合和方法配置，使用预先固定的独立随机流。



### 最后研究阶段完成与自主输入纠正

本阶段自主KEEP/EDIT研究已结束，当前固定方案为已更新E、8候选及离线蒸馏价值头。在禁止读取物理结果的选择审计下，已查看的247/246开发来源分别得到SUN/MSUN增量+2/+8、+1/+6。新96组成验证为SUN6→6、MSUN49→54、Stable6→6；有一个阈值附近的Stable晋升，也损失一个原SUN，尚不能称SUN稳定提升。模型已冻结，不再使用该96来源调参。现在进入公开仓库整理与后续三次×三轮1050合法Plan正式运行。详见[自主结果与局限](docs/r03_paper_story_20260907/AUTONOMOUS_KEEP_EDIT_RESULTS_20260911.md)。

原有带当前物理反馈的排序结果保留为离线对照，不作为自主推理结果。正式推理不设置真实RAW/SUN门控，也不向E提供真实当前态物理指标。物理反馈只供离线训练和输出后的评价。
