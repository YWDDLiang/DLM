...
状态：先按用户本条消息完成要求登记；以下是后续工作的明确顺序与约束。四小时提升窗口尚未因这次文档登记而开始，实际开始和截止时间将在启动时单独记下。上一轮已完成实验的预算与结果不被重写。以下要求优先于此前临时实验方案和人为流程门槛。
...
1. **最后4小时：分析和寻找提升。** 重点继续解决当前SUN提升、真实Stable晋升内容、排序漏选和不必要拒绝的问题；必要的小实验、修复与结果整理均放在这一有界阶段中规划。时间到后结束该阶段，不因某项指标未达到旧的采用条件而无限延长。四小时是最后提升阶段的上限，不把之后的发布仓库整理、最终完整采样混入这四小时，也不把临时研究窗口写成公开代码的门禁。
