...
状态：先按用户本条消息完成要求登记；以下是后续工作的明确顺序与约束。四小时提升窗口尚未因这次文档登记而开始，实际开始和默认截止时间将在启动时单独记下；用户随后补充，发现很好的机制时允许延长。上一轮已完成实验的预算与结果不被重写。以下要求优先于此前临时实验方案和人为流程门槛。
...
3. **发布整理完成后做最终完整运行。** 复用固定1200 Plan，按原始顺序只看前1000条有效记录，执行完整生成、后续修订/评价、自提升训练及再采样链路；自提升固定3轮，不沿用旧1000实验“停在S1”的停止安排。
...
- 只看按原始顺序得到的前1000条有效记录。启动前写明“有效”的具体定义以及原始序号到所用集合的映射，不能按后验SUN/MSUN、能量或某轮是否成功来挑选有利样本。
- 同一选定集合贯穿对照和三轮自提升，避免后续轮次更换样本导致不可比。无效或未完成项保留明确记录，不静默缩小分母。
- 自提升完整执行3轮，保存每轮模型版本、实际训练更新、采样输出和共同评价结果。用户“只看前1000条有效”的要求不意味着可以把采样、训练或轮次简化为部分流程。

本节是用户要求的登记与执行语义整理，不是仓库已发布或最终三轮运行已完成的声明。后续按上述顺序推进，无须重新逐项请求已经给出的工作授权。
