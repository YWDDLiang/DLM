...
创建日期：2026-09-07。更新日期：2026-09-09。状态：用户已批准§0.13并要求开始执行，代码实现与回归验证进行中。本次仅TRAIN，12小时、4—6张A800，固定基座初始化，训练侧允许CHGNet指导；最新质量顺序为SUN＞严格Stable＞Meta Stable＞普通更稳定＞不稳定。轨迹按可复用数据集保留；GPU开始时间与绝对截止时间以正式运行回执为准。§0.11／§0.12为此前讨论与诊断，与最新计划冲突时以§0.13为准。
...
## 0.14 后续提案：先用MP20训练KEEP／EDIT基础能力（2026-09-09）

用户在本轮运行中询问：后续能否先在MP20上训练KEEP／EDIT，为它提供基础能力。建议采用，作为后续起点方案；本轮仍按已经启动的§0.13完成S0和三次更新，不把新预训练权重插入本轮已登记的起点。
...
建议的训练顺序为：**MP20基础恢复预训练 → 实际G＋F状态上的物理判断训练 → 分级偏好RSI**。基础恢复提供较多、明确的编辑监督，降低只用256个在线Plan同时学习动作语义、定位、内容和物理偏好的负担；收益仍需后续运行验证。
