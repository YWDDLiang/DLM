...
创建日期：2026-09-07。更新日期：2026-09-09。状态：用户已认可最后一次修正版闭环方案，明确本次只在TRAIN上进行、MAIN不参与新评测，并给出12小时、4—6张A800额度。§0.11已写入详细流程和资源安排，当前未启动新作业。已完成的原因诊断见§0.10，原三轮结果见§0.9.1。
...
沿用已固定的256个TRAIN Plan及其顺序，保留原训练来源身份，不另抽一批更容易成功的Plan。新实验从旧θ2的G/E权重开始，记为新S0，后续三次更新为S1、S2、S3，避免与旧θ0—θ3混淆。选择θ2是利用已有观察作工程起点；本次不追加MAIN评分。已有G、F、E各自的seed保持固定，跨轮变化来自权重和预先登记的流程修正。

沿用B0输入／输出词表、原Plan元素计数、K8硬几何支持和model494 F800。每个版本的实际输出仍是256条，失败保留在分母。历史池仅来自TRAIN；模型输出与历史教师样本分别存储，历史最优结构不得直接覆盖本轮输出。

### 0.11.2 数据修复与可靠反馈

所有教师端点与历史回放按原Plan元素顺序排列，整体移动“元素＋三个坐标”原子块，保持胞参数和几何等价性，并保存原端点、新token、标签来源的绑定。训练入口检查N、元素槽位及动态数值支持。旧的整胞E编辑不能改个名称就作为局部编辑目标：其改动若超出新动作范围，不进入局部内容训练；可复用其当前状态、可信物理记录或KEEP锚，新的局部目标必须另有对应标签。
...
每个Plan保存若干历史raw候选及它们各自真实的F/token端点。新旧候选采用同一有效物理协议与F设置时，比较后端可靠质量，把胜负赋给对应的两条raw：`Plan → raw_A/raw_B → 各自F/token质量 → raw偏好对`。新的raw比历史更差时，历史raw仍是胜者；同等级只有明确可靠的改善才产生偏好，质量未知或仅由批次去重代表变化造成的差异不提供物理胜负。
...
本次具体实现建议进一步收敛动作集：F负责晶胞及全局调整，E固定晶胞、N、元素及未选中坐标，仅允许`local_xyz`，模型从已有count头的1／2类选择原子数，由site头选择位置。上一版提到的胞参数限幅修改不在本次新增分支。限制原子数还不够，坐标允许集合也要限制实际周期位移；建议单次每原子位移上限0.2Å，与K8支持同时满足。该数值为启动前固定的工程参数，超出范围的教师目标不能先剪裁再沿用旧标签。
...
训练输入显式包含同一当前结构与真实active mask。内容损失针对所选原子的数值位置，在多个mask位置或同一编辑块上提供监督，不再每次仅抽一个全局数值位置。site/count标签来自真实有益的局部目标，不能把“力最大”直接当成必然有益的位置。最终候选选择学习的是`当前状态＋具体提案 → 是否采用`；只有可靠健康保留监督用于提前KEEP，某次随机提案失败不再等价于这个状态永远不值得编辑。

### 0.11.5 离线教师与回放池

每轮首先用当前学生在新G的token-F状态上产生局部提案，并为TRAIN记录实际成功、失败及被拒提案的物理反馈。缺少有益提案的状态，再利用既有力／轨迹或短程CHGNet指导补一个满足相同局部动作范围的候选。优先使用已有信息；新候选仍须量化并独立评分，不能复用另一结构或另一个mask的奖励。

教师候选只进入离线训练，不直接作为本轮E输出。为控制费用，每轮最多补64个教师候选，按符合条件的Plan固定哈希顺序安排；这是TRAIN数据采集上限，不改变256条结果面板。三个E训练阶段最多补192个新教师候选，不在S3完成后再为不存在的S4采集数据。只对实际缺少正例的状态补充，不为达到64而增加调用；最后2小时不再新增教师采集。

按Plan维护可靠成功池，保存优良raw→F轨迹、兼容新动作范围的E正例、真实反例和KEEP锚。建议历史回放与本轮新数据各约50%；若某类兼容数据不足，按实际可用量登记，不伪造正例或大量重复少数Plan。旧整胞编辑正例不能直接混入新局部提案分布。参考模型为本次更新前的冻结版本，训练使用已有可训练模块，B0输入／输出表保持冻结，不新开模型结构或超参数搜索。

### 0.11.6 每一轮的实际顺序

先得到修正规则下的S0记录：能证明检查点、种子、输入和执行协议一致的G/F物理记录可以复用；门控或输入已改变的请求须执行相应阶段。E局部提案属于新行为，不能把旧整胞编辑成绩直接填入S0。

| 顺序 | 本轮工作 | 输出及用途 |
|---|---|---|
| A | 用截至上一轮的可靠同Plan raw偏好更新G | 得到G_r，保留更新前参考与真实权重变化 |
| B | G_r在固定256 TRAIN生成raw，执行健康门控、F800、token回写和评分 | 得到本轮新当前结构x_r，刷新G历史轨迹 |
| C | E_(r−1)在x_r上产生局部训练提案，必要时补少量离线教师候选 | 生成同一x_r条件下的局部内容、位置、范围及KEEP/EDIT监督 |
...
主问题是：相对修正后的S0，TRAIN最终SUN/MSUN能否提高；E能否在保留当前成功的同时新增MSUN/SUN；在G可生成覆盖已接近饱和后，F/token端点还能否改善。正式指标与可靠反馈诊断一同解释，不把模型只学会KEEP、生成失败减少或异常能量下降混为同一种成功。

训练按样本／mask覆盖组织，不照抄旧128／96步；先为每轮G分配最多35分钟、E最多45分钟的有效训练窗口，均优先使用2张A800。三轮优化训练窗口合计约4小时串行墙钟、约8 GPU小时；模型加载、保存及步边界收尾开销另计入整体12小时和实际作业账目。实际优化步数由新mask损失的吞吐决定，在完整优化步边界保存并如实报告；不预先声称必达某个步数，不因临时看到好结果而延长或截取训练。全程保留固定256 TRAIN、三次真实G更新和三次真实E更新，以及每个版本的完整输出评分。

| 墙钟时间安排目标 | 工作 |
|---|---|
| 0—1.5小时 | 初始化修正后的S0与有效历史池，复用兼容的TRAIN记录和相图参考，补齐规则变化所需的阶段 |
| 1.5—4.25小时 | 第一轮G更新、新当前结构、E数据与训练、本轮实际输出 |
| 4.25—7小时 | 第二轮相同闭环 |
| 7—9.75小时 | 第三轮相同闭环 |
| 9.75—12小时 | 完成末轮／尚在执行的必要评分、处理失败请求、汇总所有版本与成本，不新增实验分支 |

以上是调度目标，生成与物理阶段的时长估算不等于已测得的新实现耗时。4卡时，2卡训练与剩余就绪工作配合；6卡时额外2卡优先用于生成、F和物理标签吞吐。没有独立工作就不占卡等待；最多3个本轮Slurm作业同时运行或等待。必要依赖仍按顺序执行，不把依赖尚未产生标签的训练提前启动。

保留最后约2小时收尾；不追加MAIN、消融、额外seed、全1000扩展或S4数据采集。完成优先级为完整三轮与实际结果评分，其次是达到教师候选上限或训练窗口上限。若某阶段实际变慢，减少尚未开始的补充教师采集或在既定训练窗口内控制覆盖，不从结果面板删除失败／耗时样本。代码和数据一致性检查在占用GPU前完成，属于实现正确性工作，不新建一组验证实验；不把未完成的循环计为完成。
