...
### 0.9.1 初始实测记录（2026-09-09 03:40北京时间）
...
| θ0 raw来源 | 分母 | comp_valid | Struct_valid | SUN | MSUN |
|---|---:|---:|---:|---:|---:|
| MAIN固定评测子集 | 256 | 199（77.73%） | 223（87.11%） | 5（1.95%） | 32（12.50%） |
| TRAIN固定反馈集 | 256 | 184（71.88%） | 229（89.45%） | 4（1.56%） | 36（14.06%） |

MAIN的5个SUN跳过F800，218个可用非SUN结构进入F800，33个构造失败保留在分母。TRAIN同样按完整raw cohort的SUN评分门控。当前F800在两套来源上运行；这些数值是训练前的初始测量，尚不能说明模型自提升。

编辑输入始终采用token-F；“raw优于refine”决定训练偏好方向，不在部署时另加逐条物理选优。已知SUN的KEEP先验与模式／接受头监督分开保存。完全相同端点可复用已核验的物理标签；结构发生变化后重新计算N/U。

可复核证据：[初始阶段四项指标、门控及来源哈希](execution/post_refine_v08/rsi/INITIAL_PHASE.json)。42项相关CPU行为检查已通过，包括token分界、TRAIN隔离、硬支持梯度、编辑模型和SUN评分；真实G/E的CUDA反向检查与后续优化器更新分别记账，前者不计作训练轮次。
