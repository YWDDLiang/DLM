...
创建日期：2026-09-07。更新日期：2026-09-09。状态：三组G/E训练及评测已完成；随后完成现有数据的CPU原因诊断，发现G训练表示与固定元素槽位不一致、无效弛豫终点进入健康反馈，以及E保守决策与提案破坏性问题。最新诊断见§0.10，原协议结果见§0.9.1；下一轮计算预算待诊断讨论后确定。
...
## 0.11 最后一次实验：完整修正方案建议（2026-09-09）

用户最新约束：只剩最后一次实验额度，不再安排验证性实验与消融，重点是KEEP/EDIT与自提升机制本身的改进。因此§0.10.6的多组消融顺序不再作为下一轮执行建议。当前建议是一个统一配方、一次完整三轮闭环；每轮实际生成结果的逐阶段评分属于本实验结果记录，不另开对照分支。本节为方案建议，未启动新作业；具体额度尚未给出，先保证能够完整完成三轮，再按额度确定固定面板规模。

**推荐起点为现有θ2的G/E。** 它在已观察MAIN面板上是最佳版本，且E在TRAIN上有保留已有SUN/MSUN、净增5个MSUN的行为。这个选择基于已观察结果，不把它当作独立测试结论。新实验应标识为从θ2继续训练的修正版闭环，完整记录起点和三次真实权重更新。

**一、先修正表示与物理反馈。** 所有训练端点按原Plan的元素槽位对齐，移动完整原子块，保留胞参数与几何等价性；历史回放也必须经过相同修正。无效终点、能量协议矛盾和异常评分不得提供健康标记、正偏好或成功锚。失败状态可作为独立失败监督，但不把异常低能量当成奖励；未知标签保持未知。原正式SUN/MSUN继续保留，以便说明实际结果，同时明确记录可靠健康标记的定义和异常数量。

**二、保留KEEP/EDIT，但取消非健康状态的重复否决。** 对可靠已知SUN保留强KEEP先验和保留监督；KEEP仍是模型决策。其余状态先产生一个局部候选，再由已有候选判别头比较当前状态与候选，统一决定最终KEEP/EDIT。当前状态单独给出的mode不再提前否决这类候选。不能把“这一条随机提案不好”训练成“这个当前状态永远不值得编辑”。非法候选回退当前状态是执行约束；真正的改善接受由学习到的判别器完成，推理时不新增CHGNet选优。

**三、把整胞重生成改为有范围限制的修改。** 复用现有site/count头预测位置与范围，默认只修改1—2个原子的坐标并保留其余token；胞参数调整单独限制范围，不再每次同时mask六个胞参数及全部坐标。训练和推理采用同一局部mask与数值支持，偏好损失覆盖实际改动位置，保留位置另有保持监督。改动较小并不自动代表物理更好，候选仍需训练标签与判别器支持。

**四、G按精修后的价值学习选择生成起点。** 优先复用同Plan跨轮已评分的新旧raw轨迹，比较它们各自经相同F/token流程后的可靠质量，将偏好赋给对应raw候选。这样优化的是“生成什么起点更容易得到好结果”，而不是只比较raw与一次精修终点后直接模仿终点。旧轨迹只有物理协议、F设置、来源和标签有效性一致时才复用；仅由cohort去重代表变化造成的优劣不作为局部物理负反馈。几何对齐后的好终点可以保留少量SFT监督，但不取代上述G目标。新结果变差时，历史更好的raw仍作为胜者，不因轮次较新就自动晋级为教师。

**五、用离线CHGNet补充E的正例，并加强成功回放。** 新模型自己的提案继续提供真实成功与失败；对缺乏有益提案的TRAIN状态，利用既有力/弛豫轨迹或少步CHGNet指导补充一个局部教师候选。保留完整原子对应关系，量化回学生token后再按相同物理协议评分，生成当前状态条件下的偏好、修改位置及范围标签。不能把教师浮点能量直接贴给量化后的输出。优先复用已有轨迹，把新物理调用集中在正例不足的训练状态；推理阶段由学生完成局部提案和KEEP/EDIT。

跨轮维护按Plan平衡的可靠成功历史池，同时保留真实失败与反向偏好。建议回放与新数据各占约一半，参考模型保持上一轮冻结版本，采用保守权重更新；修正现有每次只监督一个位置的稀疏训练，按局部编辑块或多个mask位置组织监督。具体步数与batch按最后额度一次确定，不开展学习率、权重或seed搜索。上述比例是本次方案的工程选择，尚无独立消融证明最优。

本方案要争取的是：G产生的候选更适合F，E既保留好结构也提供小范围改善，下一轮持续学习可信的历史成功。不能把历史样本池中的最佳结构直接替换新模型评测输出来制造逐轮上升；每轮表格必须来自对应新模型的真实输出。固定TRAIN和MAIN边界、Plan及随机源，完成三轮后报告全部阶段与成功/失败变化。若最后额度无法覆盖固定1000的完整流程，优先缩小统一面板并保留三轮训练闭环，不把未完成的第三轮或未评分的输出计为成功。
