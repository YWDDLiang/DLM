...
创建日期：2026-09-07。更新日期：2026-09-10（北京时间）。状态：256请求的三级恢复S0—S3已完整结束，最终SUN32、Stable33。用户随后授权固定有效方法、修正局部错误并开始1000固定Plan全流程；当前实施与验证见§0.16。新实验独立登记，不改写旧12小时预算。最多6张A800、3个并发作业，固定基座初始化、训练侧CHGNet指导及原质量口径延续。用户要求优先提升并保护Stable，再改善Stable内N/U；轨迹按可复用数据集保留。
