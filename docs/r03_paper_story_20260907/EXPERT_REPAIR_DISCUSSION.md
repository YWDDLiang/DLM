...
首个32例定向探针覆盖旧B0的全部27例失败及5例成功对照：5例原生完成，7例在失败XYZ恢复、7例在邻域恢复、5例在全部数值恢复、8例在最后Z放宽完成；5个成功对照逐token复现。然而最后8例出现完全重叠，无法进refiner图。修正已恢复原生重叠检查，并对这8例重新做GPU验证。这个定向探针不进入正式256请求分母，也不能据此宣称SUN提升。
