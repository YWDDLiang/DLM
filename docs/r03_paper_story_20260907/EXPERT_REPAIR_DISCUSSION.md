...
创建日期：2026-09-07。更新日期：2026-09-10（北京时间）。状态：旧S1补跑、新三级恢复S0—S3全部完整流程、六次真实G/E更新及归档审计均已完成；最终SUN32、Stable33，后续Edit/KEEP建议见§0.15.1，尚未启动新训练。本次仅TRAIN，沿用原12小时预算及最多6张A800，固定基座初始化，训练侧允许CHGNet指导；执行质量顺序为SUN＞严格Stable＞Meta Stable＞普通更稳定＞不稳定。用户要求后续重点先提升并保护Stable，再提升Stable内N/U。轨迹按可复用数据集保留；与历史状态冲突时以§0.15最终结果为准。
