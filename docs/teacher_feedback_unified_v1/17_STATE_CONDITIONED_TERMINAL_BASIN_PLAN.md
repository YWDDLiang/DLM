...
日期：2026-09-05原稿，2026-09-06数据预算修订。当前主线已恢复，自动化已迁到新任务。
warmup、条件准备和首轮1024×K4路径均已完成，全量终态标注正在运行。最新用户要求
优先提效、取消追加小数据消融；**唯一刷新改为同1024条件×K8**，详见
[20数据充分性与交付](20_DATA_SUFFICIENCY_AND_DELIVERY_20260906.md)。其余科学对象、
双目标、两遍训练和单次刷新结构保留。截止仍为2026-09-06 19:19 Asia/Shanghai，
最多6A800、每卡4CPU，不自动续期。
...
以下保留原K4/K4完整覆盖预算，不代表当前实际训练量；执行中的K4/K8预算已在
[20数据充分性与交付](20_DATA_SUFFICIENCY_AND_DELIVERY_20260906.md)更新。不能把
完整DLM后训练写成一次348-step小头训练：
