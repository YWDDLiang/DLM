...
进一步的数学论证见[23 连续路径、双目标投影与可行性边界](23_CONTINUOUS_DIFFUSION_MATHEMATICAL_FEASIBILITY.md)。
其中给出了实际PC链的可评分方案、确定性末步处理，以及弛豫对蒸馏不能自动降低B的原因。
用户要求的独立审查已完成于[24 真实训练与SDE边界](24_CONTINUOUS_DIFFUSION_TRAINING_AND_SDE_AUDIT.md)：
从联合score回归出发，明确区分分布教师存在、连续控制可实现和实际效果，修正过强的可行性判断。

**用户04:00确认：此扩展放到下一轮，本轮K4/K8完成后优先评测。**当前不新增refiner
训练，不改变model494对照；保留以下设计作为后续方案。
