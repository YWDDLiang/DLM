...
当前 [RAW_LLADA_DESIGN.md:3](../../../../../periodic_self_repair_v1/RAW_LLADA_DESIGN.md#L3) 明确覆盖旧 K4/K8 warm-start 方案：只加载原始 LLaDA-8B-Instruct/tokenizer，晶体输入/输出行增量和 LoRA 为新训练参数。因此 [DESIGN_AND_AUDIT.md](../DESIGN_AND_AUDIT.md) 中 K4/K8 终态修复数据、旧 LoRA 初始化和参考 KL 不能再当作当前 raw 模型已使用的训练机制。

当前文档实际定义的是：

- MP20 teacher-only 数据，train/validation 为 `27136/9047` 个 source；每个 source 每 epoch 有一个随机 mask 构造视图和一个 scalar full-cell repair 视图。[当前数据定义](../../../../../periodic_self_repair_v1/RAW_LLADA_DESIGN.md#L9)
- `family-balanced schema denoising CE + 0.25 × actual-legal-support CE`；各项内部 `90% exact CE + 10% numerical CE`，数值带宽从 `0.75` 到 `0.25` bins；schema/部署概率温度分别为 `1/0.7`，目标不在合法支持时记录冲突并使合法项为零。[当前目标定义](../../../../../periodic_self_repair_v1/RAW_LLADA_DESIGN.md#L11)
- 修复输入是结构化合成扰动及正确的 clean prefix；自提升已暂停，没有当前模型自身生成错误或 K4/K8 反馈进入当前训练。[输入状态](../../../../../periodic_self_repair_v1/RAW_LLADA_DESIGN.md#L7)、[范围边界](../../../../../periodic_self_repair_v1/RAW_LLADA_DESIGN.md#L19)
...
合法集合的归纳证明应只声称保持所声明的有限支持：精确组成、数值 token 类型、非退化晶格及规定周期像集合下的距离条件。它不证明低能、近平衡，亦不自动证明任意 unreduced 晶格的全局最近周期像；当前新设计也明确保留该边界。[新设计的支持声明](../DESIGN_AND_AUDIT.md#L43)、[有限周期像边界](../../../../../periodic_self_repair_v1/RAW_LLADA_DESIGN.md#L17)
