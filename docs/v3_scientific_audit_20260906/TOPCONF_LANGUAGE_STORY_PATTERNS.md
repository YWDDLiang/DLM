...
**我们的 What 必须是 de novo 联合生成。** 输入不提供目标化学式或参考结构，系统自主选择 N、元素/计数，再生成晶格与坐标。几何子模块可以使用组成条件，但这只是联合模型的内部因子。论文主表评估完整请求；缓存同一批上游自主抽样后得到的组成—结构仍是de novo系统样本，后端配对差值用于归因。外部给定组成的重建只用于条件诊断，不能替代主任务。[当前cohort的两种含义区分](DE_NOVO_COHORT_ACCOUNTING.md)。
