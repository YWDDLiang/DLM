...
2026-09-06。**仅作条件触发准备，未改代码、训练或提交 GPU。** 当前40064健康训练，既定 G float raw/refined 与 secondary Q raw 实验继续完整执行。待两主端 SUN 出齐并按用户条件判断效果不足后，才启用下列顺序；不根据中间 loss 或 Q 的较好结果提前切换主端点。
