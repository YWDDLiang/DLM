...
This is the fully de novo route: its Plans are sampled from the learned
Planner, not selected from MP-20 rows. Training may use deterministically
extracted Plan labels, but inference samples from the learned Plan prior.
...
Because the Plan source is frozen, this route reproduces the conditional
body/refiner calculation rather than fully de novo Plan generation. It is a
fast control for the downstream stages, not the source of the de novo claim.
...
requirements. Launch, asset, and result handling do not use Python `assert`
statements or historical file-hash gates. Vendored upstream numerical code
retains its original tensor-shape assertions.
