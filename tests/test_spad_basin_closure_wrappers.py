...
class SPADBasinClosureWrapperTest(unittest.TestCase):
    ...
    def test_preflight_generation_uses_registered_closure_only(self):
        ...
        self.assertIn('--num-samples "${EXPECTED_GROUPS}"', self.preflight_generate)
        self.assertIn("SPAD_ROLLOUT_WORLD_SIZE", self.preflight_generate)
        self.assertIn("materialize_spad_basin_preflight_states.py", self.preflight_generate)
