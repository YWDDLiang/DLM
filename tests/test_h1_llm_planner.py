...
class H1LlmPlannerTests(unittest.TestCase):
    ...
    def test_canonical_plan_record_derives_body_prompt(self):
        ...
        self.assertEqual(
            ...
            hashlib.sha256(b"formula: Li2O\nend: plan").hexdigest(),
        )
        self.assertEqual(record["raw_plan_text_sha256"], record["plan_text_sha256"])
        self.assertEqual(
            record["body_prompt_sha256"],
    ...
    def test_nocharge_rich_prompt_and_teacher_answer_keep_formula_first_schema(self):
        plan = {
            "elements": ["Li", "O"],
            "counts": [2, 1],
            "N": 3,
        ...
        prompt = build_planner_user_prompt(sample_idx=9, prompt_style=H1_PLANNER_PROMPT_STYLE_RICH_NOCHARGE)
        self.assertIn("exactly six lines", prompt)
        self.assertIn("formula:", prompt)
        self.assertNotIn("charge: <", prompt)
        self.assertIn("Do not emit charge", prompt)
        self.assertIn("sample_id: 9", prompt)
        ...
        self.assertIn(record["plan_state"]["charge_bucket"], {"neutral_plausible", "validator_unavailable"})
        self.assertTrue(record["plan_state"]["derived_charge_bucket_from_formula"])
        self.assertNotIn("charge:", record["plan_text"])
        self.assertIn(f'"charge_bucket":"{record["plan_state"]["charge_bucket"]}"', record["prompt"])
