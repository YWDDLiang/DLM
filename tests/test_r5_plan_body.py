...
class R5PlanBodyTests(unittest.TestCase):
    ...
    def test_h1_nocharge_rich_plan_round_trip_derives_charge_without_emitting_it(self):
        _arrays, plan = self.make_arrays_and_plan()
        text = format_composition_plan(plan, plan_style=H1_RICH_NOCHARGE_PLAN_FORMAT)
        self.assertEqual(len(text.splitlines()), 6)
        self.assertNotIn("charge:", text)
        self.assertIn("formula: Li2O", text)
        self.assertIn("end: plan", text)

        parsed = parse_composition_plan(text, plan_style=H1_RICH_NOCHARGE_PLAN_FORMAT)
        self.assertEqual(parsed["formula"], "Li2O")
        self.assertIn(parsed["charge_bucket"], {"neutral_plausible", "validator_unavailable"})
        self.assertTrue(parsed["derived_charge_bucket_from_formula"])
        self.assertNotIn("charge", parsed["generated_rich_fields"])
        self.assertIn(parsed["validator"]["reason"], {"charge_neutral_pauling_valid", "validator_unavailable"})

    def test_h1_nocharge_rich_plan_rejects_generated_charge(self):
        with self.assertRaisesRegex(ValueError, "must not emit a charge field"):
            parse_composition_plan(
                "formula: Li2O\nanion: oxide\ncharge: neutral_plausible\n"
                "lattice: cubic\nspacegroup: sg_195_230\nvolume: volpa_005_009\nend: plan",
                plan_style=H1_RICH_NOCHARGE_PLAN_FORMAT,
            )

    def test_h1_nocharge_plan_body_record_keeps_exact_length_body(self):
        arrays, plan = self.make_arrays_and_plan()
        record = build_plan_body_record(plan_state=plan, arrays=arrays, plan_style=H1_RICH_NOCHARGE_PLAN_FORMAT)
        self.assertEqual(record["r5_representation"], H1_RICH_NOCHARGE_PLAN_BODY_REPRESENTATION)
        self.assertEqual(record["plan_format"], H1_RICH_NOCHARGE_PLAN_FORMAT)
        self.assertNotIn("charge:", record["plan_text"])
        plan_text, body_answer = split_plan_body_answer(record["answer"])
        parsed_plan = parse_composition_plan(plan_text, plan_style=H1_RICH_NOCHARGE_PLAN_FORMAT)
        parsed_body = validate_answer_matches_plan(parsed_plan, body_answer)
        self.assertEqual(parsed_body["num_atoms"], 3)
        self.assertEqual(record["block_spans"]["body"]["semantic_length"], 7 + 4 * 3)
