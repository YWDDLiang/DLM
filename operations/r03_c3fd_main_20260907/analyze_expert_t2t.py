"""Paired source-cluster analysis of fixed-checkpoint masked versus visible OLD proposals and deployment."""
...
def main(directory):
    root = Path(directory)
    names = ['T2T64_COMPACT.json','GAUGE64_OLD_COMPACT.json']
    repeated, old_capsule = [json.loads((root/name).read_text()) for name in names]
    old = indexed(old_capsule['dev']['rows'])
    seeds = [2026090813,2026090913,2026091013,2026091113]
    arms = ['masked','old_values']
    panels = {(arm,seed):indexed(repeated['labels'][arm+':'+str(seed)]['rows']) for arm in arms for seed in seeds}
    assert all(set(rows)==set(old) for rows in panels.values())
    tasks = {gid:x['trajectory_id'].split(':')[-1] for gid,x in panels[('masked',seeds[0])].items()}
    ...
    output = {'schema':'expert_t2t64_analysis_v1','seeds':seeds,
    ...
    for task, name, fn in metrics:
        ...
        diff = values['old_values']-values['masked']
        output['paired_effects'][task+':'+name] = {
            'independent_sources':len(gids),'denominator':len(gids)*len(seeds),
            'masked':int(values['masked'].sum()),'old_values':int(values['old_values'].sum()),
            'old_values_minus_masked_pp':float(diff.mean()*100),
    ...
    (root/'T2T64_ANALYSIS.json').write_text(json.dumps(output,indent=2)+'\n')
