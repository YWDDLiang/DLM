"""Collect complete T2T comparisons with input identity and learned-action evidence."""
import hashlib
import json
...
paths = [root/('mechanism_t2t64_'+arm)/'autonomous/old_physics.jsonl' for arm in ['masked','old_values']]
assert inputs(paths[0]) == inputs(paths[1])
reference_autonomous = root/'mechanism_gauge64_repeat_control/autonomous/old_physics.jsonl'
assert inputs(paths[0]) == inputs(reference_autonomous)
data['input_identity']['autonomous_reference_sha256'] = sha(reference_autonomous)
