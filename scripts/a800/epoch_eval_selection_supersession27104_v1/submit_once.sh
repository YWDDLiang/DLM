...
BATCH=configs/experiments/wyckoff_codiffusion/epoch_eval_selection_supersession27104_v1_dag.json
BATCH_SHA=0220f54125ddb5d6d09beb7833805797cf111323465ad4c12b5c939d2e5e3a73
AUTHORIZATION=runs/remote_audit/20260723_user_authorization_epoch_eval_selection_supersession27104_v1.json
FAILURE_AUDIT=runs/remote_audit/20260723_refiner27103_pass_epoch_eval27104_identity_failure_v1.json
RECORD="$RUN/notes/epoch_eval_selection_supersession27104_v1_submission.json"
CLAIM="$RECORD.claim"
JOB_DIR=scripts/a800/epoch_eval_selection_supersession27104_v1
...
EVIDENCE="$RUN/notes/epoch_checkpoint_evidence_sup27104_v1.json"
SELECTION="$RUN/outputs/epoch_checkpoint_selection_lock_sup27104_v1.json"
...
test "$(sha256sum "$AUTHORIZATION" | awk '{print $1}')" = b0b74bac3802e7e2319a3e86ced4799987f516040f44350d207b556368e477b1
test "$(sha256sum "$FAILURE_AUDIT" | awk '{print $1}')" = e5f61b38d664fa18b86f2bf6be6a46b25f25642e7ae91356c2248f0d88b49b0f
...
if (
    batch.get("schema")
    != "crysllmgen_epoch_eval_selection_supersession_dag_v1"
...
write_record() {
  ...
  python - "$RECORD" "$status" "$failed_stage" "$failure_message" \
    "$EVALUATION_PATCH_SHA256" "$ADAPTER_TRAINING_PATCH_SHA256" \
    "$REFINER_TRAINING_PATCH_SHA256" "$BATCH_SHA" "$eval_job" \
    "$selection_job" "$eval_command" "$selection_command" \
    "$(printf '%s\n' "${existing_rows[@]}")" <<'PY'
import json, os, sys
from pathlib import Path
...
(
    ...
    adapter_patch, refiner_patch, batch_sha, eval_job, selection_job,
    eval_command, selection_command, existing,
) = sys.argv[1:]
payload = {
    "schema": "crysllmgen_epoch_eval_selection_supersession_submission_v1",
    "status": status,
    ...
    "retry_or_replacement_policy": (
        "one transparent evaluation-and-selection supersession; "
        "no task resubmission or attempt replacement"
    ),
...
payload = {
    "schema": "crysllmgen_epoch_eval_selection_submission_claim_v1",
