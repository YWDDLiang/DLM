...
JOB=scripts/a800/wq_formula_plan_sft_one_epoch_v2/train_and_eval.sbatch
JOB_NAME=wq-fplan-1ep-p64-v2
CONTRACT=configs/experiments/wyckoff_codiffusion/wq_formula_plan_sft_one_epoch_v2.json
CONTRACT_SHA256=f50b934cb24b026815caea6ac387e4a0604ae32310ee9c46b67e0cc6e1cb0e46
AUTHORIZATION_LABEL=user_wq_formula_plan_sft_one_epoch_v2_2026-07-27
...
: "${WQ_FORMULA_PLAN_ONE_EPOCH_PATCH_SHA256:?caller must export WQ_FORMULA_PLAN_ONE_EPOCH_PATCH_SHA256}"
if [[ ! "$WQ_FORMULA_PLAN_ONE_EPOCH_PATCH_SHA256" =~ ^[0-9a-f]{64}$ ]]; then
  echo "invalid formula-plan one-epoch execution patch identity" >&2
  exit 2
fi

cd "$ROOT"
test -x "$EXACT_PYTHON"
test -f ".artifacts/source_sync/authorized_patch_${WQ_FORMULA_PLAN_ONE_EPOCH_PATCH_SHA256}.json"
...
if (
    ...
    or one(r"^#SBATCH --time=(\S+)$") != "05:00:00"
    or "#SBATCH --array" in job
):
    raise SystemExit("formula-plan one-epoch resource gate failed")
print("wq_formula_plan_one_epoch_preclaim_resource_gate=PASS")
PY

PYTHONPATH="$ROOT" "$EXACT_PYTHON" - \
  "$ROOT" "$GATE" "$PROTOCOL" "$WQ_FORMULA_PLAN_ONE_EPOCH_PATCH_SHA256" \
...
if authorization not in PATCH_ALLOWED_AUTHORIZATIONS:
    raise SystemExit("formula-plan one-epoch authorization is not registered")
...
if (
    ...
    raise SystemExit("formula-plan one-epoch installed patch failed Gate A")
print("wq_formula_plan_one_epoch_preclaim_gate_a=PASS")
PY
...
if printf '%s\n' "$existing_rows" |
  ...
  echo "same formula-plan one-epoch job already exists" >&2
  exit 3
fi

mkdir -p "$RUN/notes" "$RUN/logs"
"$EXACT_PYTHON" - "$CLAIM" "$WQ_FORMULA_PLAN_ONE_EPOCH_PATCH_SHA256" \
  "$CONTRACT_SHA256" <<'PY'
import json
import os
import sys
...
payload = {
    "schema": "wq_formula_plan_sft_one_epoch_submission_claim_v2",
    ...
    "training_epochs": 1.0,
    "expected_training_updates": [959, 960],
    "formula_attempts": 64,
...
command="sbatch --parsable --export=ALL,WQ_FORMULA_PLAN_ONE_EPOCH_PATCH_SHA256=$WQ_FORMULA_PLAN_ONE_EPOCH_PATCH_SHA256 $JOB"
set +e
output="$(
  sbatch --parsable \
    --export="ALL,WQ_FORMULA_PLAN_ONE_EPOCH_PATCH_SHA256=$WQ_FORMULA_PLAN_ONE_EPOCH_PATCH_SHA256" \
    "$JOB" 2>&1
)"
rc=$?
set -e
...
"$EXACT_PYTHON" - "$RECORD" "$status" "$job_id" "$failure_message" \
  "$WQ_FORMULA_PLAN_ONE_EPOCH_PATCH_SHA256" "$CONTRACT_SHA256" "$command" \
  "$existing_rows" <<'PY'
import json
import os
import sys
...
payload = {
    "schema": "wq_formula_plan_sft_one_epoch_submission_v2",
    "status": status,
...
echo "wq_formula_plan_one_epoch_job_id=$job_id"
