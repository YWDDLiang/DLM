...
def main():
    ...
    all_rows=[]
    for split,expected in (('main1000',983),('remainder159',156)):
        selected=[x for x in accepted if x['split']==split];
    ...
    (out/'manifest.json').write_text(json.dumps({'schema':'known1200_refined_accepted_v1','steps':a.steps,'requested':1159,'default_denominator':1139,'main_denominator':983,'remainder_denominator':156,'invalid_cif_excluded':20},indent=2,sort_keys=True)+'\n'); (out/'_SUCCESS').touch(); print(out)
