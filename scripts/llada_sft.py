...
def normalize_planned_corruption_policy(name: str | None) -> str:
    normalized = str(name or "none").strip().lower()
    if normalized not in PLANNED_CORRUPTION_POLICIES:
        choices = ", ".join(sorted(PLANNED_CORRUPTION_POLICIES))
        raise ValueError(
            f"Unknown planned corruption policy {normalized!r}; expected one of {choices}"
        )
    return normalized


def validate_corruption_mix(
    *,
    iid_fraction: float,
    ...
    iid = float(iid_fraction)
    planned = float(planned_fraction)
    policy = normalize_planned_corruption_policy(planned_policy)
    if not math.isfinite(iid) or not math.isfinite(planned):
        raise ValueError("corruption fractions must be finite")
    if iid < 0.0 or planned < 0.0 or iid + planned <= 0.0:
        raise ValueError(
            "iid_fraction and planned_fraction must be non-negative with positive sum"
        )
    if planned > 0.0 and policy == "none":
        raise ValueError(
            "planned_fraction is positive but planned_corruption_policy is 'none'"
        )
    if planned > 0.0 and representation not in (None, "dynamic_v1"):
        raise ValueError(
            "planned corruption currently requires representation='dynamic_v1'"
        )
    return {
        "iid_fraction": iid,
        "planned_fraction": planned,
        "planned_probability": planned / (iid + planned),
        "planned_policy": policy,
    }


def _planned_groups_for_record(
    row: Dict[str, Any],
    *,
    policy: str,
    ...
    if not isinstance(answer, str):
        raise ValueError("planned corruption record is missing string answer")
    parsed = parse_dynamic_answer(answer, strict=True)
    num_atoms = int(parsed["num_atoms"])
    declared_num_atoms = row.get("num_atoms")
    if declared_num_atoms is not None and int(declared_num_atoms) != num_atoms:
        raise ValueError(
            "planned corruption num_atoms disagrees with the dynamic-v1 answer"
        )
    answer_length = 7 + 4 * num_atoms
    for field in ("answer_semantic_length", "answer_token_count"):
        declared_length = row.get(field)
        if declared_length is not None and int(declared_length) != answer_length:
            raise ValueError(
                f"planned corruption {field}={declared_length} disagrees with "
                f"dynamic answer length {answer_length}"
            )
    if policy == "d1":
        ...
        raise ValueError(f"cannot build planned groups for policy {policy!r}")
    return (
        position_group_ids(groups, answer_length=answer_length),
        len(groups),
        answer_length,
    )


def _planned_metadata_for_record(
    row: Dict[str, Any],
    *,
    policy: str,
    tokenizer,
    ...
    metadata: Dict[str, Any] = {
        "corruption_key": corruption_key_for_record(row),
        "planned_group_ids": torch.empty((0,), dtype=torch.long),
        "planned_group_count": 0,
    }
    if policy == "none":
        return metadata

    group_ids, group_count, answer_length = _planned_groups_for_record(
        row,
        policy=policy,
    ...
    untruncated_ids = tokenizer(
        prompt_text + str(row["answer"]),
        add_special_tokens=False,
    )["input_ids"]
    if list(untruncated_ids) != [*prompt_ids, *answer_ids]:
        raise ValueError(
            "planned corruption requires additive prompt/answer tokenization; "
            "the tokenizer changed tokens at the boundary"
        )
    if len(untruncated_ids) > int(max_length) or list(input_ids) != list(
        untruncated_ids
    ):
        raise ValueError(
            "planned corruption refuses truncated training examples: "
            f"model_length={len(untruncated_ids)} max_length={max_length}"
        )
    if int(prompt_length) != len(prompt_ids):
        raise ValueError("planned corruption prompt length is inconsistent")
    if len(answer_ids) != answer_length:
        raise ValueError(
            "planned corruption requires one tokenizer token per dynamic-v1 "
            f"semantic token: tokenizer={len(answer_ids)} semantic={answer_length}"
        )
    metadata["planned_group_ids"] = torch.tensor(group_ids, dtype=torch.long)
    metadata["planned_group_count"] = int(group_count)
    return metadata
...
class JsonlSftDataset(Dataset):
    def __init__(
        ...
        tokenizer,
        max_length: int,
        planned_corruption_policy: str = "none",
        planned_corruption_seed: int = 0,
    ):
        ...
        self.planned_corruption_seed = int(planned_corruption_seed)
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        item = {
            ...
            "mask_policy_id": mask_policy_id(
                row.get("mask_policy") or row.get("sft_mask_policy")
            ),
            "loss_profile_id": loss_profile_id(row.get("loss_profile")),
            "module_id": int(
                row.get("module_id") or MODULE_TO_ID.get(str(row.get("module", "")), 0)
            ),
            "sample_weight": float(row.get("sample_weight", 1.0) or 1.0),
        }
        item.update(
            _planned_metadata_for_record(
                row,
                policy=self.planned_corruption_policy,
                tokenizer=self.tokenizer,
                prompt_text=prompt_text,
                input_ids=input_ids,
                prompt_length=prompt_length,
                max_length=self.max_length,
                corruption_seed=self.planned_corruption_seed,
            )
        )
        return item
...
class CsvCrystalSftDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        if self.answer_representation == "dynamic_v1":
            answer, _ = structure_to_dynamic_answer(
                structure, separator=self.answer_separator
            )
        else:
        ...
        planned_record = {
            ...
            "answer_semantic_length": 7 + 4 * len(structure),
            "answer_token_count": 7 + 4 * len(structure),
        }
        item.update(
            _planned_metadata_for_record(
                planned_record,
...
class DataCollator:
    ...
    def __call__(self, batch: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        max_planned_len = max(
            (
                int(item["planned_group_ids"].shape[0])
                if item.get("planned_group_ids") is not None
                else 0
            )
            for item in batch
        )
        ...
        planned_group_ids = torch.full(
            (len(batch), max_planned_len),
            -1,
            dtype=torch.long,
        )
        planned_group_counts = torch.zeros((len(batch),), dtype=torch.long)
        corruption_keys = torch.zeros((len(batch),), dtype=torch.long)
        for i, item in enumerate(batch):
            ids = item["input_ids"]
            input_ids[i, : ids.shape[0]] = ids
            attention_mask[i, : ids.shape[0]] = 1
            prompt_lengths[i] = item["prompt_length"]
            mask_policy_ids[i] = int(
                item.get("mask_policy_id", MASK_POLICY_TO_ID["normal"])
            )
            loss_profile_ids[i] = int(
                item.get("loss_profile_id", LOSS_PROFILE_TO_ID["fixed_slot"])
            )
            module_ids[i] = int(item.get("module_id", 0))
            sample_weights[i] = float(item.get("sample_weight", 1.0) or 1.0)
            sample_group_ids = item.get("planned_group_ids")
            if sample_group_ids is not None and int(sample_group_ids.shape[0]) > 0:
                planned_group_ids[i, : sample_group_ids.shape[0]] = sample_group_ids
            planned_group_counts[i] = int(item.get("planned_group_count", 0))
            corruption_keys[i] = int(item.get("corruption_key", 0))
        return {
            "input_ids": input_ids,
            ...
            "planned_group_ids": planned_group_ids,
            "planned_group_counts": planned_group_counts,
            "corruption_keys": corruption_keys,
        }
...
class DistributedNoPaddingSampler(Sampler[int]):
    """Deterministic rank-strided sampler with no padding or duplication."""
    ...
    def __init__(
        ...
        *,
        ...
        self.indices = strided_shard_indices(
            len(dataset),
...
def _forward_candidate_mask(
    input_ids: torch.Tensor,
    answer_mask: torch.Tensor,
    positions: torch.Tensor,
    prompt_lengths: torch.Tensor,
    *,
    mask_policy_ids: torch.Tensor | None,
    empty_token_id: int | None,
    prefill_slot_tokens: bool,
    fixed_slot_body_offset: int,
) -> torch.Tensor:
    bsz = input_ids.shape[0]
    candidate_mask = answer_mask.clone()
    rel_positions = positions - prompt_lengths.unsqueeze(1)
    body_rel_positions = rel_positions - int(fixed_slot_body_offset)
    ...
    if (
        mask_policy_ids is not None
        and empty_token_id is not None
        and int(empty_token_id) >= 0
    ):
        ...
        for i in range(bsz):
            policy = ID_TO_MASK_POLICY.get(
                int(mask_policy_ids[i].detach().item()),
                "normal",
            )
            ...
            if sample_empty.any():
                first_empty_abs = int(
                    torch.nonzero(sample_empty, as_tuple=False)
                    .flatten()[0]
                    .detach()
                    .item()
                )
...
def _stateless_uniform_tensor(
    corruption_keys: torch.Tensor,
    *,
    step: int,
    seed: int,
    stream: int,
    ...
    """PyTorch counterpart of planned_corruption.stateless_uniform."""

    if int(step) < 0:
        raise ValueError("corruption_step must be non-negative")
    if int(stream) < 0:
        raise ValueError("corruption stream must be non-negative")
    keys = corruption_keys.to(device=positions.device, dtype=torch.long)
    position_values = positions.to(device=positions.device, dtype=torch.long)
    while keys.ndim < position_values.ndim:
        keys = keys.unsqueeze(-1)
    modulus = STATELESS_MODULUS
    values = (
        keys.remainder(modulus)
        ...
        + (position_values + 1).remainder(modulus) * STATELESS_POSITION_MULTIPLIER
    ).remainder(modulus)
    values = (
        (
    ...
    values = (
        torch.bitwise_xor(values, torch.bitwise_right_shift(values, 13))
        * STATELESS_FINAL_MULTIPLIER
        + 1
    ).remainder(modulus)
    return (values.to(torch.float32) + 0.5) / float(modulus)


def _ensure_stateless_supervision(
    masked_indices: torch.Tensor,
    candidate_mask: torch.Tensor,
    fallback_scores: torch.Tensor,
) -> torch.Tensor:
    """Guarantee one supervised token without batch-order-dependent RNG."""

    result = masked_indices.clone()
    needs_fallback = candidate_mask.any(dim=1) & ~result.any(dim=1)
    if needs_fallback.any():
        scores = torch.where(
            candidate_mask,
            fallback_scores,
            torch.full_like(fallback_scores, float("inf")),
        )
        fallback_positions = scores.argmin(dim=1)
        rows = torch.nonzero(needs_fallback, as_tuple=False).flatten()
        result[rows, fallback_positions[rows]] = True
    return result
...
def forward_process(
    ...
    planned_fraction: float = 0.0,
    planned_group_ids: torch.Tensor | None = None,
    planned_group_counts: torch.Tensor | None = None,
    corruption_keys: torch.Tensor | None = None,
    corruption_step: int = 0,
    corruption_seed: int = 0,
    stateless_iid: bool = False,
    ...
    # This branch deliberately preserves the original random-call order and
    # masking logic. With planned_fraction=0, callers get the prior D0 path.
    if float(planned_fraction) == 0.0 and not stateless_iid:
        t = torch.rand(bsz, device=device)
        p_mask = ((1 - eps) * t + eps).unsqueeze(1).expand(bsz, seq_len)
        random_mask = torch.rand((bsz, seq_len), device=device) < p_mask
        candidate_mask = _forward_candidate_mask(
            input_ids,
            answer_mask,
            positions,
            prompt_lengths,
            ...
            prefill_slot_tokens=prefill_slot_tokens,
            fixed_slot_body_offset=fixed_slot_body_offset,
        )
        ...
        return {
            ...
            "input_masked_indices": masked_indices,
            "p_mask": p_mask,
            "answer_mask": answer_mask,
            "candidate_mask": candidate_mask,
            "planned_sample_mask": torch.zeros(
                (bsz,),
                dtype=torch.bool,
                device=device,
            ),
            "active_group_indices": torch.full(
                (bsz,),
                -1,
                dtype=torch.long,
                device=device,
    ...
    if float(planned_fraction) == 0.0:
        if corruption_keys is None:
            raise ValueError("stateless iid corruption requires stable corruption keys")
        keys = corruption_keys.to(device=device, dtype=torch.long)
        if keys.shape != (bsz,):
            raise ValueError("corruption_keys must have shape [batch]")
        candidate_mask = _forward_candidate_mask(
            input_ids,
            answer_mask,
            positions,
        ...
        rel_positions = positions - prompt_lengths.unsqueeze(1)
        scalar_positions = torch.zeros((bsz,), dtype=torch.long, device=device)
        probability_uniform = _stateless_uniform_tensor(
            keys,
        ...
        sample_probabilities = (1.0 - eps) * probability_uniform + eps
        p_mask = sample_probabilities.unsqueeze(1).expand(bsz, seq_len)
        token_uniform = _stateless_uniform_tensor(
            keys,
            step=corruption_step,
            seed=corruption_seed,
            stream=2,
            positions=rel_positions.clamp_min(0),
        )
        masked_indices = (token_uniform < p_mask) & candidate_mask
        fallback_scores = _stateless_uniform_tensor(
            keys,
            step=corruption_step,
            seed=corruption_seed,
            stream=5,
        ...
        masked_indices = _ensure_stateless_supervision(
            masked_indices,
            candidate_mask,
            fallback_scores,
        )
    ...
    mix = validate_corruption_mix(
        iid_fraction=iid_fraction,
        planned_fraction=planned_fraction,
        planned_policy="d1",
    )
    if planned_group_ids is None or planned_group_counts is None:
        raise ValueError(
            "planned corruption requires collated group IDs and group counts"
        )
    if corruption_keys is None:
        raise ValueError("planned corruption requires stable corruption keys")
    group_ids = planned_group_ids.to(device=device, dtype=torch.long)
    group_counts = planned_group_counts.to(device=device, dtype=torch.long)
    keys = corruption_keys.to(device=device, dtype=torch.long)
    if group_ids.ndim != 2 or group_ids.shape[0] != bsz:
        raise ValueError("planned_group_ids must have shape [batch, answer_length]")
    if group_ids.shape[1] == 0:
        raise ValueError("planned_group_ids cannot be empty")
    if group_counts.shape != (bsz,) or (group_counts <= 0).any():
        raise ValueError("planned_group_counts must be positive for every sample")
    if keys.shape != (bsz,):
        raise ValueError("corruption_keys must have shape [batch]")
    ...
    rel_positions = positions - prompt_lengths.unsqueeze(1)
    within_group_width = (rel_positions >= 0) & (rel_positions < group_ids.shape[1])
    safe_rel_positions = rel_positions.clamp(
        min=0,
        max=group_ids.shape[1] - 1,
    )
    full_group_ids = torch.gather(group_ids, 1, safe_rel_positions)
    full_group_ids = torch.where(
        answer_mask & within_group_width,
        full_group_ids,
        torch.full_like(full_group_ids, -1),
    )
    if ((full_group_ids < 0) & answer_mask).any():
        raise ValueError(
            "planned group metadata does not cover every non-padding answer token"
        )
    if ((full_group_ids >= group_counts.unsqueeze(1)) & answer_mask).any():
        raise ValueError("planned group metadata contains an out-of-range group ID")

    scalar_positions = torch.zeros((bsz,), dtype=torch.long, device=device)
    mixture_uniform = _stateless_uniform_tensor(
        keys,
        step=corruption_step,
        seed=corruption_seed,
        stream=0,
    ...
    planned_sample_mask = mixture_uniform < float(mix["planned_probability"])
    probability_uniform = _stateless_uniform_tensor(
        keys,
        step=corruption_step,
        seed=corruption_seed,
    ...
    random_mask = token_uniform < p_mask
    active_uniform = _stateless_uniform_tensor(
        keys,
        step=corruption_step,
        seed=corruption_seed,
        stream=3,
    ...
    active_group_indices = torch.floor(
        active_uniform * group_counts.to(torch.float32)
    ).to(torch.long)
    active_group_indices = torch.minimum(
    ...
    active_candidate_mask = (
        answer_mask
        & candidate_mask
        & (full_group_ids == active_group_indices.unsqueeze(1))
    )
    if (planned_sample_mask & ~active_candidate_mask.any(dim=1)).any():
        raise ValueError(
            "planned corruption selected an active group with no candidate token"
        )
    future_mask = answer_mask & (full_group_ids > active_group_indices.unsqueeze(1))
    planned_loss_mask = random_mask & active_candidate_mask
    planned_fallback_scores = _stateless_uniform_tensor(
        keys,
        step=corruption_step,
        seed=corruption_seed,
        stream=4,
    ...
    planned_loss_mask = _ensure_stateless_supervision(
        planned_loss_mask,
        active_candidate_mask,
        planned_fallback_scores,
    )

    iid_loss_mask = random_mask & candidate_mask
    iid_fallback_scores = _stateless_uniform_tensor(
        keys,
        step=corruption_step,
        seed=corruption_seed,
        stream=5,
    ...
    iid_loss_mask = _ensure_stateless_supervision(
        iid_loss_mask,
        candidate_mask,
        iid_fallback_scores,
    )
    ...
    masked_indices = torch.where(
        planned_sample_mask.unsqueeze(1),
        planned_loss_mask,
        iid_loss_mask,
    )
    input_masked_indices = torch.where(
        planned_sample_mask.unsqueeze(1),
        planned_loss_mask | future_mask,
        iid_loss_mask,
    )
    effective_candidate_mask = torch.where(
        planned_sample_mask.unsqueeze(1),
        active_candidate_mask,
        candidate_mask,
    )
    ...
    return {
        "noisy": noisy,
        "masked_indices": masked_indices,
        "input_masked_indices": input_masked_indices,
        "p_mask": p_mask,
        "answer_mask": answer_mask,
        "candidate_mask": effective_candidate_mask,
        "planned_sample_mask": planned_sample_mask,
        "active_group_indices": torch.where(
            planned_sample_mask,
            active_group_indices,
            torch.full_like(active_group_indices, -1),
        ),
    }
...
def build_loss_config(tokenizer, args) -> Dict[str, Any]:
    ...
    if (
        getattr(args, "representation", "fixed_slot") == "fixed_slot_compressed_v1"
        and "<C_PAD>" in vocab
    ):
        pad_coord_token_ids = [optional_token_id("<C_PAD>")]
    else:
        pad_coord_token_ids = [
            optional_token_id(token) for token in ("<X_PAD>", "<Y_PAD>", "<Z_PAD>")
        ]
    return {
        "representation": getattr(args, "representation", "fixed_slot"),
        "answer_token_count": int(
            getattr(args, "answer_token_count", ANSWER_TOKEN_COUNT)
        ),
        "iid_fraction": float(getattr(args, "iid_fraction", 1.0)),
        "planned_fraction": float(getattr(args, "planned_fraction", 0.0)),
        "planned_corruption_policy": normalize_planned_corruption_policy(
            getattr(args, "planned_corruption_policy", "none")
        ),
        "corruption_seed": int(getattr(args, "corruption_seed", 0)),
        "fixed_slot_body_offset": (
            PHYSICAL_HEADER_BODY_OFFSET
            if getattr(args, "representation", "fixed_slot")
            == "fixed_slot_physical_header"
            else 0
        ),
        "physical_header_loss_weight": float(
            getattr(args, "physical_header_loss_weight", 2.0)
        ),
        ...
        "train_prefill_slot_tokens": bool(
            getattr(args, "train_prefill_slot_tokens", False)
        ),
        "empty_token_id": optional_token_id("<EMPTY>"),
        "pad_coord_token_ids": pad_coord_token_ids,
        "composition_module_loss_weight": float(
            getattr(args, "composition_module_loss_weight", 2.0)
        ),
        "lattice_module_loss_weight": float(
            getattr(args, "lattice_module_loss_weight", 1.0)
        ),
        "sites_module_loss_weight": float(
            getattr(args, "sites_module_loss_weight", 1.25)
        ),
        "crysllmgen_lattice_loss_weight": float(
            getattr(args, "crysllmgen_lattice_loss_weight", 1.0)
        ),
        "crysllmgen_composition_loss_weight": float(
            getattr(args, "crysllmgen_composition_loss_weight", 2.5)
        ),
        "crysllmgen_species_loss_weight": float(
            getattr(args, "crysllmgen_species_loss_weight", 2.0)
        ),
        "crysllmgen_coords_loss_weight": float(
            getattr(args, "crysllmgen_coords_loss_weight", 1.1)
        ),
        "crysllmgen_site_coord_loss_weight": float(
            getattr(args, "crysllmgen_site_coord_loss_weight", 1.0)
        ),
        "fixed_plain_count_loss_weight": float(
            getattr(args, "fixed_plain_count_loss_weight", 3.0)
        ),
        "fixed_plain_lattice_loss_weight": float(
            getattr(args, "fixed_plain_lattice_loss_weight", 1.0)
        ),
        "fixed_plain_elements_loss_weight": float(
            getattr(args, "fixed_plain_elements_loss_weight", 2.0)
        ),
        "fixed_plain_coords_loss_weight": float(
            getattr(args, "fixed_plain_coords_loss_weight", 1.1)
        ),
        "dynamic_lattice_length_loss_weight": float(
            getattr(args, "dynamic_lattice_length_loss_weight", 1.0)
        ),
        "dynamic_lattice_angle_loss_weight": float(
            getattr(args, "dynamic_lattice_angle_loss_weight", 1.0)
        ),
        "dynamic_coord_loss_weight": float(
            getattr(
                args,
                "dynamic_coord_loss_weight",
                getattr(args, "coordinate_loss_weight", 1.0),
            )
        ),
    }
...
def build_answer_position_weights(
    ...
    if loss_config.get("representation") == "fixed_plain":
        if module_ids is not None:
            ...
            sample_weights = torch.where(
                ...
                torch.full_like(
                    sample_weights,
                    float(loss_config["fixed_plain_elements_loss_weight"]),
                ),
                sample_weights,
            )
            sample_weights = torch.where(
                ...
                torch.full_like(
                    sample_weights, float(loss_config["fixed_plain_coords_loss_weight"])
                ),
                sample_weights,
            )
            weights = weights * sample_weights.unsqueeze(1)
        if text_profile_mask is not None:
            weights = torch.where(
                text_profile_mask & answer_mask, torch.ones_like(weights), weights
            )
        return weights
    ...
    if body_offset > 0:
        ...
        weights = torch.where(
            header_mask,
            torch.full_like(
                weights, float(loss_config.get("physical_header_loss_weight", 2.0))
            ),
            weights,
        )
    ...
    if loss_config.get("representation") == "dynamic_v1":
        weights = torch.where(
            ...
            torch.full_like(
                weights, float(loss_config["dynamic_lattice_length_loss_weight"])
            ),
            weights,
        )
        weights = torch.where(
            ...
            torch.full_like(
                weights, float(loss_config["dynamic_lattice_angle_loss_weight"])
            ),
            weights,
        )
...
def compute_loss(
    ...
    *,
    ...
    processed = forward_process(
        ...
        iid_fraction=float(loss_config.get("iid_fraction", 1.0)),
        planned_fraction=(
            0.0
            if force_iid_baseline
            else float(loss_config.get("planned_fraction", 0.0))
        ),
        planned_group_ids=batch.get("planned_group_ids"),
        planned_group_counts=batch.get("planned_group_counts"),
        corruption_keys=batch.get("corruption_keys"),
        corruption_step=int(corruption_step),
        corruption_seed=int(loss_config.get("corruption_seed", 0)),
        stateless_iid=bool(force_iid_baseline),
    )
    ...
    token_loss = F.cross_entropy(
        outputs.logits[masked], input_ids[masked], reduction="none"
    )
    ...
    if loss_config.get("representation") in {
        "cif_lite_modular",
        "crysllmgen_text",
        "fixed_plain",
    }:
        ...
        sample_weight_sums = (
            processed["candidate_mask"].sum(dim=1).to(torch.float32).clamp_min(1.0)
        )
    else:
        sample_weight_sums = (
            (loss_weights * processed["candidate_mask"]).sum(dim=1).clamp_min(1.0)
        )
...
def evaluate(
    ...
    for idx, batch in enumerate(loader):
        ...
        if "sample_weights" in batch:
            ...
            batch_weight = float(batch["input_ids"].shape[0])
        weighted_loss_sum += float(loss.detach().cpu()) * batch_weight
        evaluation_weight += batch_weight
    if distributed:
        payload = torch.tensor(
            [weighted_loss_sum, evaluation_weight],
            dtype=torch.float64,
            device=device,
        )
        ...
        result = weighted_loss_sum / max(1.0, evaluation_weight)
    model.train()
    return result
...
def main() -> None:
    ...
    parser.add_argument(
        "--model-path", default="/public/home/jiaosz/ywliang/models/LLaDA-8B-Instruct/"
    )
    parser.add_argument(
        "--checkpoint-path",
        type=Path,
    ...
    parser.add_argument(
        "--data-dir", type=Path, default=PROJECT_ROOT / "data/dlm_sft/mp_20"
    )
    parser.add_argument(
        "--representation",
        choices=[
    ...
    parser.add_argument(
        ...
        choices=sorted(PLANNED_CORRUPTION_POLICIES),
        default="none",
        help=(
            "Dependency grouping for mixed DLM corruption. D1 follows the "
            "current N/elements/lattice/x/y/z schedule; D2 requires PlanGraph "
            "body records."
        ),
    )
    parser.add_argument(
        ...
        default=1.0,
        help=(
            "Non-negative iid corruption mixture weight. Weights are "
            "normalized with --planned-fraction; primary registration uses 2."
        ),
    )
    parser.add_argument(
        "--planned-fraction",
        ...
        default=0.0,
        help=(
            "Non-negative planned corruption mixture weight. Zero preserves "
            "the original iid forward path; primary registration uses 1."
        ),
    )
    parser.add_argument(
        "--corruption-seed",
        ...
        default=20260731,
        help=(
            "Counter-based corruption seed. Mixed masks depend only on this "
            "seed, the training-pair key, microbatch step, stream, and position."
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Evaluate the frozen validation panel once at optimizer step 0. "
            "This is required by registered checkpoint noninferiority screens."
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Label outputs as bounded engineering evidence that is ineligible "
            "for checkpoint selection or later initialization."
        ),
    )
    parser.add_argument(
        "--skip-final-save",
        action="store_true",
        help=(
            "Skip checkpoint/final model serialization. This is allowed only "
            "with --engineering-only for bounded runtime smoke tests."
        ),
    )
    ...
    if args.skip_final_save and not args.engineering_only:
        raise ValueError("--skip-final-save requires --engineering-only")
    corruption_mix = validate_corruption_mix(
        iid_fraction=args.iid_fraction,
        planned_fraction=args.planned_fraction,
        planned_policy=args.planned_corruption_policy,
        representation=args.representation,
    )
    args.iid_fraction = float(corruption_mix["iid_fraction"])
    args.planned_fraction = float(corruption_mix["planned_fraction"])
    args.planned_corruption_policy = str(corruption_mix["planned_policy"])
    ...
    args.output_dir = assert_writable_output_path(
        args.output_dir,
    ...
    if is_main:
        ...
        write_json(
            str(args.output_dir / "tokenizer_report.json"),
            {
                ...
                "pad_token_id_ne_mask_token_id": tokenizer.pad_token_id
                != MASK_TOKEN_ID,
    ...
    model.train()
    model_ready_seconds = time.perf_counter() - main_started
    if device.type == "cuda":
        torch.cuda.reset_peak_memory_stats(device)

    training_planned_policy = (
        args.planned_corruption_policy
        if float(args.planned_fraction) > 0.0
        else "none"
    )
    if args.train_csv_dir is None:
        train_ds = JsonlSftDataset(
            ...
            tokenizer,
            args.max_length,
            planned_corruption_policy=training_planned_policy,
            planned_corruption_seed=args.corruption_seed,
        )
    ...
    val_ds = JsonlSftDataset(
        ...
        tokenizer,
        args.max_length,
        planned_corruption_policy="none",
    )
    if args.limit_train:
    ...
    if args.weighted_sampling_power != 1.0:
        raw_train_weights = [
            math.pow(max(weight, 0.0), args.weighted_sampling_power)
            for weight in raw_train_weights
        ]
    train_weight_summary = summarize_sample_weights(raw_train_weights)
    multiplier_summary = summarize_sample_weight_multipliers(
        train_ds.rows, sample_weight_multipliers
    )
    weighted_sampling_active = bool(
        args.weighted_sampling and train_weight_summary["positive_count"] > 0
    )
    ...
    local_val_indices = (
        list(val_sampler.indices)
        if val_sampler is not None
        else list(range(len(val_ds)))
    )
    if dist_info["distributed"]:
        gathered_val_indices: List[List[int] | None] = [
            None for _ in range(dist_info["world_size"])
        ]
        dist.all_gather_object(gathered_val_indices, local_val_indices)
        validation_shards = [
            indices if indices is not None else []
            for indices in gathered_val_indices
        ]
    else:
        validation_shards = [local_val_indices]
    validation_sampler_report = audit_distributed_index_shards(
        len(val_ds),
        validation_shards,
    )
    validation_sampler_report.update(
        {
            "sampler": (
                type(val_sampler).__name__
                if val_sampler is not None
                else "SequentialSampler"
            ),
            "batch_size_per_rank": int(args.batch_size),
            "eval_max_batches_per_rank": int(args.eval_max_batches),
            "automatic_downstream": False,
        }
    )
    if is_main:
        write_json(
            str(args.output_dir / "validation_sampler_report.json"),
            validation_sampler_report,
        )
    if dist_info["distributed"]:
        dist.barrier()
    if not validation_sampler_report["gate_passed"]:
        raise RuntimeError(
            "Validation sampler failed exact no-padding distributed coverage audit"
        )
    ...
    train_microbatches = 0
    local_task_loss_values: List[float] = []
    local_gradient_norm_values: List[float] = []
    local_evaluation_loss_values: List[float] = []
                ...
                json.dumps(
                    {
                        ...
                        "weighted_sampling": {
                            ...
                            else type(train_sampler).__name__,
                        ...
                        "element_token_alignment": {
                            ...
                            "matched_count": (
                                ...
                                else element_alignment_targets.get("report", {}).get(
                                    "matched_count"
                                )
                            ),
        ...
        if args.eval_before_train:
            ...
            if not math.isfinite(val_loss) or val_loss <= 0.0:
                raise FloatingPointError(
                    f"non-finite or non-positive pre-training validation loss: {val_loss}"
                )
            local_evaluation_loss_values.append(float(val_loss))
            if is_main:
                log_handle.write(
                    json.dumps(
                        {
            ...
            for micro_step, batch in enumerate(train_loader):
                ...
                with sync_context:
                    task_loss = compute_loss(
                        ...
                        batch,
                        loss_config,
                        corruption_step=epoch * len(train_loader) + micro_step,
                    )
                    task_loss_value = float(task_loss.detach().cpu())
                    if not math.isfinite(task_loss_value) or task_loss_value <= 0.0:
                        raise FloatingPointError(
                            "non-finite or non-positive task loss at "
                            f"rank={dist_info['rank']} microbatch={train_microbatches}: "
                            f"{task_loss_value}"
                        )
                    local_task_loss_values.append(task_loss_value)
                ...
                if (
                    ...
                    raise FloatingPointError(
                        "non-finite or non-positive gradient norm at "
                        f"rank={dist_info['rank']} step={global_step + 1}: "
                        f"{gradient_norm_value}"
                    )
                local_gradient_norm_values.append(gradient_norm_value)
                optimizer.step()
                scheduler.step()
                optimizer.zero_grad(set_to_none=True)
                global_step += 1
                        ...
                        json.dumps(
                            {
                                ...
                                "pre_clip_gradient_norm": gradient_norm_value,
                ...
                if global_step % args.eval_steps == 0:
                    ...
                    if not math.isfinite(val_loss) or val_loss <= 0.0:
                        raise FloatingPointError(
                            f"non-finite or non-positive validation loss: {val_loss}"
                        )
                    local_evaluation_loss_values.append(float(val_loss))
                    if is_main:
                        log_handle.write(
    ...
    local_runtime = {
        ...
        "optimizer_updates": int(global_step),
        "task_loss_count": len(local_task_loss_values),
        "task_loss_min": min(local_task_loss_values)
        if local_task_loss_values
        else None,
        "task_loss_max": max(local_task_loss_values)
        if local_task_loss_values
        else None,
        "gradient_norm_count": len(local_gradient_norm_values),
        "gradient_norm_min": min(local_gradient_norm_values)
        if local_gradient_norm_values
        else None,
        "gradient_norm_max": max(local_gradient_norm_values)
        if local_gradient_norm_values
        else None,
        "evaluation_loss_count": len(local_evaluation_loss_values),
        "evaluation_losses": local_evaluation_loss_values,
    ...
    if dist_info["distributed"]:
        gathered_runtime: List[Dict[str, Any] | None] = [
            None for _ in range(dist_info["world_size"])
        ]
        dist.all_gather_object(gathered_runtime, local_runtime)
        rank_runtime = [
            item if item is not None else {}
            for item in gathered_runtime
        ]
    else:
        rank_runtime = [local_runtime]
    evaluation_expected = bool(
        args.eval_before_train
        or (
            int(args.eval_steps) > 0
            and int(global_step) >= int(args.eval_steps)
        )
    )
    runtime_gate_passed = (
        len(rank_runtime) == int(dist_info["world_size"])
        and validation_sampler_report["gate_passed"]
        and all(
            int(item.get("optimizer_updates", -1)) == int(global_step)
            and int(item.get("train_microbatches", 0)) > 0
            and int(item.get("task_loss_count", 0)) > 0
            and int(item.get("gradient_norm_count", 0)) == int(global_step)
            and (
                not evaluation_expected
                or int(item.get("evaluation_loss_count", 0)) > 0
            )
            and all(
            ...
            and all(
                ...
                for value in item.get("evaluation_losses", [])
            )
            for item in rank_runtime
        )
    )
    runtime_report = {
        "schema": "llada_sft_distributed_runtime_v1",
        "status": "complete" if runtime_gate_passed else "failed",
        ...
        "global_effective_batch": int(
            args.batch_size * args.grad_accum * dist_info["world_size"]
        ),
        "global_train_sequences": sum(
            int(item.get("train_microbatches", 0)) * int(args.batch_size)
            for item in rank_runtime
        ),
        "optimizer_updates": int(global_step),
        "model_ready_seconds": model_ready_seconds,
        "training_seconds": training_seconds,
        "total_seconds_before_checkpoint_save": time.perf_counter() - main_started,
        "rank_runtime": rank_runtime,
        "validation_sampler_report": "validation_sampler_report.json",
        "rank0_only_checkpoint_and_report_publication": True,
        "final_checkpoint_saved": not bool(args.skip_final_save),
        "engineering_only": bool(args.engineering_only),
        "eligible_for_checkpoint_selection": (
            False if args.engineering_only else None
        ),
        "eligible_for_later_initialization": (
            False if args.engineering_only else None
        ),
        "automatic_downstream": False,
    ...
    if is_main:
        write_json(
            str(args.output_dir / "distributed_runtime_report.json"),
            runtime_report,
        )
    if dist_info["distributed"]:
        dist.barrier()
    if not runtime_gate_passed:
        raise RuntimeError("Distributed runtime audit failed")
