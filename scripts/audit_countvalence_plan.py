...
def audit_dataset(name: str, path: Path) -> dict[str, Any]:
    rows = 0
    plans = 0
    serialized = 0
    parsed = 0
    ...
    for row in iter_jsonl(path):
        ...
        if isinstance(oxidation, (list, tuple)) and len(oxidation) == len(plan.get("elements") or ()):
        ...
        plan_counts = [int(value) for value in (plan.get("counts") or ())]
        oxidation_list = oxidation if isinstance(oxidation, (list, tuple)) else [None] * len(elements)
        if len(oxidation_list) != len(elements):
            oxidation_list = [None] * len(elements)
        for element, count, ox in zip(elements, plan_counts, oxidation_list):
...
def write_report(results: list[dict[str, Any]], output_dir: Path) -> dict[str, Any]:
    train_like = [row for row in results if row["name"] in {"train", "val"}]
    gate = all(
    ...
    payload = {
        "schema": "h1a2_planner_countvalence_coverage_audit_v1",
        "datasets": results,
        "gate": {
            "train_val_roundtrip_95pct": gate,
            "candidate_training_authorized": gate,
        },
    }
    ...
    lines = [
        ...
        "",
        f"Training gate: **{gate}**",
        "",
        "| Dataset | Plans | Serialize | Parse | Composition RT | Soft-field RT | Oxidation known | Charge neutral |",
        "|---|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in results:
        rates = row["rates"]
        lines.append(
            ...
            f"{rates['source_oxidation_known']:.2%} | {rates['generated_charge_neutral']:.2%} |"
        )
...
def main() -> None:
    ...
    for item in args.dataset:
        ...
        results.append(audit_dataset(name.strip(), Path(path)))
